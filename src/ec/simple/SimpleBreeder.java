...
public class SimpleBreeder extends Breeder
    ...
    public Population breedPopulation(EvolutionState state)
        ...
        //modified by mengxu 2023.01.30
        if(this instanceof SPEA2Breeder && state.generation == 0){
            for(int i = 0; i < state.population.subpops.length; i++){
                int prospectiveSize = state.population.subpops[i].individuals.length + numElites(state, i);
                newpop.subpops[i].individuals = new Individual[prospectiveSize];
            }
        }
    ...
    protected void loadElites(EvolutionState state, Population newpop)
        ...
        if(individualEvaluationImmediate) {
            ...
            if (state.population.subpops.length == 1 && ((GPIndividual) state.population.subpops[0].individuals[0]).trees.length == 2) {
                for (int sub = 0; sub < state.population.subpops.length; sub++) { //0  1
                    for (int x = newpop.subpops[sub].individuals.length - numElites(state, sub); x < newpop.subpops[sub].individuals.length; x++) {//start from 510, because numElites(state,sub)
...
//                    System.out.println("individual fitness before: " + newpop.subpops[sub].individuals[x].fitness.fitness());
//                    System.out.println("problem seed before: " + ((DynamicSimulation)((MultipleRuleEvaluationModel)problem.getEvaluationModel()).getSchedulingSet().getSimulations().get(0)).getSeed());
                        SimpleProblemForm prob = (SimpleProblemForm) (state.evaluator.p_problem.clone());
                        prob.evaluate(state, newpop.subpops[sub].individuals[x], 0, 0);
//                    System.out.println("individual fitness after: " + newpop.subpops[sub].individuals[x].fitness.fitness());
//                    System.out.println("problem seed after: " + ((DynamicSimulation)((MultipleRuleEvaluationModel)problem.getEvaluationModel()).getSchedulingSet().getSimulations().get(0)).getSeed());
                        ...
                        if (state instanceof GPRuleEvolutionStateUpdateLate) {
                            ...
                            if (mctsUpdateLate) {
                                if (mctsPolicy.equals("visitCount")) {
                                    GPTree gpTree0 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[0];
                                    GPTree gpTree1 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[1];
                                    ((GPRuleEvolutionStateUpdateLate) state).mctses[0].frontPropagateVisitCount(gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                    ((GPRuleEvolutionStateUpdateLate) state).mctses[1].frontPropagateVisitCount(gpTree1, newpop.subpops[sub].individuals[x].fitness.fitness());
                                }
                            }
                            ...
                            if (mctsUpdateLate) {
                                if (mctsPolicy.equals("visitCount")) {
                                    GPTree gpTree0 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[0];
                                    GPTree gpTree1 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[1];
                                    ((GPRuleEvolutionStateUpdateImmediate) state).mctses[0].frontPropagateVisitCount(gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                    ((GPRuleEvolutionStateUpdateImmediate) state).mctses[1].frontPropagateVisitCount(gpTree1, newpop.subpops[sub].individuals[x].fitness.fitness());
                                } else if (mctsPolicy.equals("totalReward-div-visitCount-linked") ||
                                        mctsPolicy.equals("totalReward-div-visitCount")) {
                                    GPTree gpTree0 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[0];
                                    GPTree gpTree1 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[1];
                                    ((GPRuleEvolutionStateUpdateImmediate) state).mctses[0].frontPropagateTotalReward(gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                    ((GPRuleEvolutionStateUpdateImmediate) state).mctses[1].frontPropagateTotalReward(gpTree1, newpop.subpops[sub].individuals[x].fitness.fitness());
...
//                            }
                                    ((GPRuleEvolutionStateUpdateImmediate) state).mctses[0].frontPropagateAllReward(state.generation, gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                    ((GPRuleEvolutionStateUpdateImmediate) state).mctses[1].frontPropagateAllReward(state.generation, gpTree1, newpop.subpops[sub].individuals[x].fitness.fitness());
...
//                System.out.println("Is CCGP!");
                for (int sub = 0; sub < state.population.subpops.length; sub++) { //0  1
                    for (int x = newpop.subpops[sub].individuals.length - numElites(state, sub); x < newpop.subpops[sub].individuals.length; x++) {//start from 510, because numElites(state,sub)
                        //modified by mengxu for MCTS to evaluate individual immediatelly!
                        ((MultiPopCoevolutionaryEvaluator)state.evaluator).evaluateIndividual(state,newpop.subpops[sub].individuals[x],sub);
                        if (state instanceof GPRuleEvolutionStateMCTS) {
                            String mctsPolicy = ((GPRuleEvolutionStateMCTS) state).mctsPolicy;
                            boolean mctsUpdateLate = state.mctses[0].isMctsUpdateLate();
                            if (mctsUpdateLate) {
                                if (mctsPolicy.equals("visitCount")) {
                                    GPTree gpTree0 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[0];
                                    state.mctses[sub].frontPropagateVisitCount(gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                }else if (mctsPolicy.equals("totalReward-div-visitCount-linked") ||
                                        mctsPolicy.equals("totalReward-div-visitCount")) {
                                    GPTree gpTree0 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[0];
                                    state.mctses[sub].frontPropagateTotalReward(gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                } else if (mctsPolicy.equals("num-infinity-reward") ||
                                        mctsPolicy.equals("mean-non-infinity-reward") ||
                                        mctsPolicy.equals("mean-non-infinity-reward-v2") ||
                                        mctsPolicy.equals("mean-non-infinity-reward-pro") ||
                                        ...
                                        mctsPolicy.equals("mean-non-infinity-reward-linked-pro-v2")) {
                                    GPTree gpTree0 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[0];
                                    state.mctses[sub].frontPropagateAllReward(state.generation, gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                } else if (mctsPolicy.equals("hybird")) {//20210518
                                    GPTree gpTree0 = ((GPIndividual) newpop.subpops[sub].individuals[x]).trees[0];
                                    state.mctses[sub].frontPropagateTotalReward(gpTree0, newpop.subpops[sub].individuals[x].fitness.fitness());
                                } else if (mctsPolicy.equals("hybirdv1")) {//20210518
