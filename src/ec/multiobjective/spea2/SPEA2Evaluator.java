...
public class SPEA2Evaluator extends SimpleEvaluator
    ...
    public void evaluatePopulation(final EvolutionState state)
        ...
        for(int x = 0;x<state.population.subpops.length;x++)
            ...
            computeAuxiliaryData(state, inds); //Computes the strength of individuals, then the raw fitness (wimpiness) and kth-closest sparsitymeasure. 
            //Finally, computes the final fitness of the individuals.
            }
        }
    ...
    public void computeAuxiliaryData(EvolutionState state, Individual[] inds)
        {
        double[][] distances = calculateDistances(state, inds);    /** Returns a matrix of sum squared distances from each individual to each other individual. */
        ...
        for(int y=0;y<inds.length;y++)
            ...
            for(int z=0;z<inds.length;z++)
                if (((SPEA2MultiObjectiveFitness)inds[y].fitness).paretoDominates((MultiObjectiveFitness)inds[z].fitness)) //the strength---the larger the better
                    myStrength++;
        ...
        for(int y=0;y<inds.length;y++)
            ...
            for(int z=0;z<inds.length;z++)
                ...
                if ( ((SPEA2MultiObjectiveFitness)inds[z].fitness).paretoDominates((MultiObjectiveFitness)inds[y].fitness) )
                    {
                    fitness += ((SPEA2MultiObjectiveFitness)inds[z].fitness).strength; //the fitness is the sum of strength of individuals that dominate it
                    }
            ...
            double kthDistance = Math.sqrt(orderStatistics(distances[y], kTH, state.random[0])); //Returns the kth smallest element in the array
            ...
            indYFitness.fitness = fitness + indYFitness.kthNNDistance;  //fitness is the sum of strength of the individuals that dominate it and K-near Neighbors distance
            if(Double.isNaN(indYFitness.fitness)){//add by mengxu 2023.01.23
                indYFitness.fitness = Double.POSITIVE_INFINITY;
            }
            }
        }
