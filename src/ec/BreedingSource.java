...
public abstract class BreedingSource implements Prototype, RandomChoiceChooserD
    ...
    //fzhang 20.7.2018  after preGeneration, change the rate of mutation and crossover
    public static final String P_PRE_GENERATIONS = "pre-generations";
    private int preGenerations;

    //mengxu 2021.05.07 for cluster selection methods
    public static final String P_CLUSTER_SELECTION = "cluster-selection";
    public boolean clusterSelection;

    //mengxu 2021.12.23 for DPS selection methods
    public static final String P_DPS_SELECTION = "DPS-selection";
    public boolean DPSSelection;
    ...
    */
    public void setup(final EvolutionState state, final Parameter base)
        ...
        //===================================original=====================================
        //make sure have a valid probability
        ...
        clusterSelection = state.parameters.getBoolean(new Parameter(P_CLUSTER_SELECTION), null, false);

        //add 2021.12.23 mengxu
        DPSSelection = state.parameters.getBoolean(new Parameter(P_DPS_SELECTION), null, false);


    /*    //F: fzhang 21.7.2018   if write it here, only use once, do not match requirement
        //fzhang 20.7.2018 after generation 15, mutation change from 0.8 to 0.15 and crossover change from 0.15 to 0.8
        ...
        if(state.generation < preGenerations) {
        	System.out.println("generation less than 10");
     	 //make sure have a valid probability
            if (!state.parameters.exists(base.push(P_PROB),def.push(P_PROB)))
                probability = NO_PROBABILITY;
            else
        	...
        	System.out.println("generation larger than 10");
        	 //make sure have a valid probability
               if (!state.parameters.exists(base.push(P_PROB),def.push(P_PROB)))
                   probability = NO_PROBABILITY;
               else
                   ...
                   //==========================start==================
                   if(probability == 0.8) {
                	   probability = 0.15 + (0.8-0.15)*(state.generation-preGenerations)/(state.numGenerations-1-preGenerations);
                   }
                   else if(probability == 0.15) {
                	   probability = 0.8 - (0.8-0.15)*(state.generation-preGenerations)/(state.numGenerations-1-preGenerations);
                   }
                   else
                	   probability = 0.05;
                   //========================end======================================
    ...
    */
    ...
    public void preparePipeline(final Object hook)
        ...
        //fzhang 2019.6.15 change the selection method---select from specific individuals as parents
        public abstract int produceFrac(int min, int max, int start, int subpopulation, Individual[] inds, EvolutionState state, int thread);
    }
