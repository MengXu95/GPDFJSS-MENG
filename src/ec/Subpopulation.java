...
//    public Species species;
    public Species species; //modified by mengxu
    ...
    //modified by mengxu 2021.05.18
    public static final String P_USE_OBL = "use-obl";
    public String useOBL;
    ...
    */
    ...
    public void setup(final EvolutionState state, final Parameter base)
        {
        Parameter def = defaultBase();  // Returns an instance of Subpopulation just like it had been before it was populated with individuals.

        int size;

        //modified by mengxu 20210518
        useOBL = state.parameters.getStringWithDefault(new Parameter(P_USE_OBL), null, "no");
        ...
        // step 1: do we load from a file?
        file = base.push(P_FILE);  //no
        loadInds = state.parameters.exists(file,null);


        // step 2: what species do we use?

        species = (Species) state.parameters.getInstanceForParameter(
            base.push(P_SPECIES),def.push(P_SPECIES),
            Species.class);
        species.setup(state,base.push(P_SPECIES)); //pop.subpop.0.species = ec.gp.GPSpecies   also linked to MultiBreedingPipeline, BreedingPipeline, BreedingSource

        // step 3: how big should our subpopulation be?
        ...
        // step 4: How often do we retry if we find a duplicate?
        numDuplicateRetries = state.parameters.getInt(
            base.push(P_RETRIES),def.push(P_RETRIES),0);
        //System.out.println(numDuplicateRetries);  //2  2
    ...
    public void populate(EvolutionState state, int thread)
        ...
        //System.out.println(len);  //512   512
        ...
        if (loadInds)
            ...
            //System.out.println(stream);  // nothing be printed out
        ...
        for(int x=start;x<individuals.length;x++)
            ...
            for(int tries=0;
                ...
                /**modified by mengxu 2021.03.24 generate OBLIndividual **/
                if(useOBL.equals("yes")){
                    if(x<individuals.length/2){
                        individuals[x] = species.newIndividual(state, thread); //important step for new individuals  //use species to generate new individual
                    }
                    else{
                        individuals[x] = species.OBLIndividuals((GPIndividual) individuals[x-individuals.length/2],state, thread);
                    }
                }
                else{
                    individuals[x] = species.newIndividual(state, thread); //important step for new individuals  //use species to generate new individual
                ...
                //modified by mengxu 2021.03.24 the following is the original one
//                individuals[x] = species.newIndividual(state, thread); //important step for new individuals  //use species to generate new individual
                ...
                //modified by mengxu for MCTS to evaluate individual immediatelly! 2021.07.06
                if(individualEvaluationImmediate && ((GPIndividual)individuals[x]).trees.length==2){
                    SimpleProblemForm prob = (SimpleProblemForm)(state.evaluator.p_problem.clone());
                    prob.evaluate(state,individuals[x], 0, 0);
                }
                else if(individualEvaluationImmediate && ((GPIndividual)individuals[x]).trees.length==1){
                    if(state.generation != 0){
                        int i=0;
                        for(i=0; i<state.population.subpops.length;i++){
                            if(state.population.subpops[i].equals(this)){
                                break;
                            }
                        }
                        ((MultiPopCoevolutionaryEvaluator)state.evaluator).evaluateIndividual(state,individuals[x],i);
                    }
                }

                //modified by mengxu 20210520---------------------
//                System.out.println("individual fitness: " + individuals[x].fitness.fitness());
                if(state instanceof GPRuleEvolutionStateUpdateLate) {
                    ...
                    if (mctsUpdateLate) {
                        if (mctsPolicy.equals("visitCount")) {
                            GPTree gpTree0 = ((GPIndividual) individuals[x]).trees[0];
                            GPTree gpTree1 = ((GPIndividual) individuals[x]).trees[1];
                            ((GPRuleEvolutionStateUpdateLate) state).mctses[0].frontPropagateVisitCount(gpTree0, individuals[x].fitness.fitness());
                            ((GPRuleEvolutionStateUpdateLate) state).mctses[1].frontPropagateVisitCount(gpTree1, individuals[x].fitness.fitness());
                        }
                    }
                }
                    ...
                    if (mctsUpdateLate) {
                        if(mctsPolicy.equals("visitCount")){
                            GPTree gpTree0 = ((GPIndividual)individuals[x]).trees[0];
                            GPTree gpTree1 = ((GPIndividual)individuals[x]).trees[1];
                            ((GPRuleEvolutionStateUpdateImmediate) state).mctses[0].frontPropagateVisitCount(gpTree0, individuals[x].fitness.fitness());
                            ((GPRuleEvolutionStateUpdateImmediate) state).mctses[1].frontPropagateVisitCount(gpTree1, individuals[x].fitness.fitness());
                        }
                        else if(mctsPolicy.equals("totalReward-div-visitCount-linked") ||
                                mctsPolicy.equals("totalReward-div-visitCount")){
                            GPTree gpTree0 = ((GPIndividual)individuals[x]).trees[0];
                            GPTree gpTree1 = ((GPIndividual)individuals[x]).trees[1];
                            ((GPRuleEvolutionStateUpdateImmediate) state).mctses[0].frontPropagateTotalReward(gpTree0, individuals[x].fitness.fitness());
                            ((GPRuleEvolutionStateUpdateImmediate) state).mctses[1].frontPropagateTotalReward(gpTree1, individuals[x].fitness.fitness());
                        }
                            ...
                            GPTree gpTree0 = ((GPIndividual)individuals[x]).trees[0];
                            GPTree gpTree1 = ((GPIndividual)individuals[x]).trees[1];
//                            if(individuals[x].fitness.fitness() > ((GPRuleEvolutionStateUpdateImmediate) state).mctses[0].getMctsRootNode().INFINITY){
//                            }
                            ((GPRuleEvolutionStateUpdateImmediate) state).mctses[0].frontPropagateAllReward(state.generation, gpTree0, individuals[x].fitness.fitness());
                            ((GPRuleEvolutionStateUpdateImmediate) state).mctses[1].frontPropagateAllReward(state.generation, gpTree1, individuals[x].fitness.fitness());
                        }
    ...
    */
    ...
    /** Reads a subpopulation from the format generated by printSubpopulation(....).  If the number of individuals is not identical, the individuals array will
     be deleted and replaced with a new array, and a warning will be generated as individuals will have to be created using newIndividual(...) rather
     than readIndividual(...).  */
    // add by mengxu to do not read fitness but only read individual
    public void readSubpopulationWithoutFitness(final EvolutionState state, final LineNumberReader reader) throws IOException
        {
            // read in number of individuals and check to see if this appears to be a valid subpopulation
            int numIndividuals = Code.readIntegerWithPreamble(NUM_INDIVIDUALS_PREAMBLE, state, reader);
            ...
            if (numIndividuals != individuals.length)
            {
                ...
                for(int i = 0 ; i < individuals.length; i++)
                {
                    ...
                    if (j!=i) state.output.warnOnce("On reading subpopulation from text stream, some individual indexes in the subpopulation did not match.  " +
                            "The first was individual " + i + ", which is listed in the file as " + j);
                    individuals[i] = species.newIndividualWithoutFitness(state, reader);
                }
            }
            else for (int i = 0 ; i < individuals.length; i++)
            {
