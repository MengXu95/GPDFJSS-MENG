 ...
 *     <ol><li>Terminals can be printed either as variables "a" or as zero-argument function "a()"
 *     <li>One-argument nonterminals are printed as function "a(b)"
 *     <li>Two-argument nonterminals can be printed either as operators "b a c" or as function "a(b, c)"
 *     <li>Nonterminals with more arguments are printed as function "a(b, c, d, ...)"
 * </ol>
 ...
 * to try out this example):
 ...
 <td valign=top>(when printing using c, print two-argument function operators "b a c"?  The alternative is function "a(b, c)."</td></tr>
 <tr><td valign=top><i>base</i>.<tt>c-variables</tt><br>
 <font size=-1>bool = <tt>true</tt> (default) or <tt>false</tt></td>
 <td valign=top>(when printing using c, print zero-argument function as variables "a"?  The alternative is function "a()".)</td></tr>
 </table>
...
public class GPTree implements GPNodeParent, Prototype
    ...
    /** When using c to print for humans, do we print terminal as variables?
        (as opposed to zero-argument function)? */
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        // in C, treat terminal as variables?  By default, yes.
        printTerminalsAsVariablesInC = state.parameters.getBoolean(base.push(P_USEVARS),def.push(P_USEVARS),true);

        // in C, treat two-child function as operators?  By default, yes.
    ...
    */
    ...
    public void readTree(final EvolutionState state,
        ...
        if (s==null)  // uh oh
            ...
            {
            GPInitializer initializer = ((GPInitializer)state.initializer);
            //revised by mengxu 2025.1.10
            child = TerminalERCUniform.readRootedTree(linenumber,new DecodeReturn(s),
...
//                    constraints(initializer).functionset,this,0,state);
    ...
    */
    ...
    public void printTreeForHumans(final EvolutionState state, final int log,
        ...
        public String printTreeForHumansNoLog(final EvolutionState state)
        {
            return child.printRootedTreeForHumansNoLog(state,0,0) + "";
        }
    ...
    public void buildTree(final EvolutionState state, final int thread) 
        {
        GPInitializer initializer = ((GPInitializer)state.initializer);
        //fzhang constraint here, get the min and max depth for generating individuals---for example, minDepth = 2 and maxDepth = 6
        GPTreeConstraints constraints = constraints(initializer); //fzhang 20.6.2018 here, constraint type is 0, name = "tc0"
        child = constraints.init.newRootedTree(state,
            constraints.treetype,  //fzhang 21.6.2018 tree type has two elements.  name = "nil"  type = 0
            thread,
            this,
             constraints.functionset, // functionset and treetype are from constraints, need to check "constraint"
            0,
        ...
        public void buildTreeMcts(final EvolutionState state, final int thread, int treeNum)
        {
            GPInitializer initializer = ((GPInitializer)state.initializer);
            //fzhang constraint here, get the min and max depth for generating individuals---for example, minDepth = 2 and maxDepth = 6
            GPTreeConstraints constraints = constraints(initializer); //fzhang 20.6.2018 here, constraint type is 0, name = "tc0"
            child = constraints.init.newRootedTreeMcts(state,
                    constraints.treetype,  //fzhang 21.6.2018 tree type has two elements.  name = "nil"  type = 0
                    thread,
                    this,
                    constraints.functionset, // functionset and treetype are from constraints, need to check "constraint"
    ...
    /** 2021.3.23 modified by mengxu. Builds a new randomly-generated rooted tree and attaches it to the GPTree. */
    public void OBLTree(GPTree tree, final EvolutionState state, final int thread)
        {
//            GPTree oblTree = new GPTree();
//            oblTree.setup(state, defaultBase());
            GPInitializer initializer = ((GPInitializer)state.initializer);
            //fzhang constraint here, get the min and max depth for generating individuals---for example, minDepth = 2 and maxDepth = 6
            GPTreeConstraints constraints = constraints(initializer); //fzhang 20.6.2018 here, constraint type is 0, name = "tc0"
            child = opposition(state, this,tree.child);
//            return oblTree;
        }

    public GPTree OBLTree2(GPTree tree, final EvolutionState state, final int thread)
        {
            GPTree oblTree = new GPTree();
    //            oblTree.setup(state, defaultBase());
            GPInitializer initializer = ((GPInitializer)state.initializer);
            //fzhang constraint here, get the min and max depth for generating individuals---for example, minDepth = 2 and maxDepth = 6
            GPTreeConstraints constraints = constraints(initializer); //fzhang 20.6.2018 here, constraint type is 0, name = "tc0"
            oblTree.child = opposition(state, oblTree,tree.child);
            return oblTree;
        }

//    public GPNode getOBLTree(final EvolutionState state, GPTree oblTree, GPTree o1){
//        return opposition(state, oblTree.child, o1.child);
//    }

    //modified by mengxu. do not finish
    public GPNode opposition(final EvolutionState state, GPNodeParent oblNode, GPNode o1)
        {
            GPNode n;
            if(o1.toString().equals("+")){
                n = new Sub();
                n.children = new GPNode[2];
                n.argposition = (byte)o1.argposition;
                n.parent = oblNode;
                GPType[] childtypes = n.constraints(((GPInitializer)state.initializer)).childtypes;
                for(int x=0;x<o1.children.length;x++){
                    n.children[x]=opposition(state, n, o1.children[x]);
                }
                return n;
            }
            ...
            else if(o1.toString().equals("Max")){
                n = new Min();
                n.children = new GPNode[2];
                n.argposition = (byte)o1.argposition;
                n.parent = oblNode;
            ...
            else if(o1.toString().equals("Min")){
                n = new Max();
                n.children = new GPNode[2];
                n.argposition = (byte)o1.argposition;
                n.parent = oblNode;
