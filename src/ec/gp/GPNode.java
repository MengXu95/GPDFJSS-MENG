 ...
 * all GP function nodes in trees.  GPNode contains quite a few function
 ...
 * ECJ in a distributed fashion.  Both of these function are called by GPNode's readRootedTree/writeRootedTree
 ...
 * files for inspection, slight modification, then reading back in later on.  Both of these function are called by GPNode's readRootedTree/writeRootedTree
...
public abstract class GPNode implements GPNodeParent, Prototype
    ...
    public boolean Using_PSL = false;
    ...
    */
    ...
    //modified by mengxu 2021.04.09
    public GPNode lightCloneMcts()
    {
        try
        {
    ...
    */
    public int printNodeForHumans(final EvolutionState state,
        ...
        public String printNodeForHumansNoLog(final EvolutionState state,
                                      final int log,
                                      final int verbosity)
        {
            String n = toStringForHumans();
    ...
    */
    ...
    /** Producess a String consisting of the tree in pseudo-C form, given that the parent already will wrap the
        expression in parentheses (or not).  In pseudo-C form, function with one child are printed out as a(b),
        function with more than two children are printed out as a(b, c, d, ...), and function with exactly two
        children are either printed as a(b, c) or in operator form as (b a c) -- for example, (b * c).  Whether
        or not to do this depends on the setting of <tt>useOperatorForm</tt>.  Additionally, terminal will be
    ...
    */
    ...
    public int printRootedTreeForHumans(final EvolutionState state, final int log,
        ...
        //modified by mengxu 2021.05.08
        public String printRootedTreeForHumansNoLog(final EvolutionState state,
                                            int tablevel, int printbytes)
        {
            return printRootedTreeForHumansNoLog(state, Output.V_VERBOSE, tablevel, printbytes);
        }
...
//            }
            ...
            treeString = treeString + printNodeForHumansNoLog(state,tablevel,printbytes);

            for (int x=0;x<children.length;x++)
                treeString = treeString + children[x].printRootedTreeForHumansNoLog(state,tablevel,printbytes);
            if (children.length>0) {
                treeString = treeString + ")";
            }
            return treeString;
