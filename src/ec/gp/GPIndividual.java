...
public class GPIndividual extends Individual
    ...
    public Double[] decisionVector;

    public void setDecisionVector(List<Integer> sequencingVector, List<Integer> routingVector) {
        decisionVector = new Double[sequencingVector.size()+routingVector.size()];
        for(int dec=0;dec<sequencingVector.size();dec++){
            decisionVector[dec]=((double)sequencingVector.get(dec));
            decisionVector[dec+sequencingVector.size()]=((double)sequencingVector.get(dec));
        }
//        for(int dec=0;dec<routingVector.size();dec++){
//            decisionVector.add((double)routingVector.get(dec));
//        }
    }

        public void setDecisionVector(List<Integer> AllVector) {
            decisionVector = new Double[AllVector.size()];
            for(int dec=0;dec<AllVector.size();dec++){
                decisionVector[dec]=((double)AllVector.get(dec));
            }
        }
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        //fzhang 2019.5.29 to show the individual in string
        @Override
        public String toString()
        {
            ...
            {
                if(trees[i].child == null)
                    continue;
                retval += "T" + i + ": " + trees[i].child.makeCTree(true, true, true) + "\n";
            }
            return retval;
        }

        //modified by mengxu 2021.05.08
        public String toStringEnsemble(EvolutionState state)
        {
            if (trees == null || trees.length == 0)
                return "null";
            ...
            for (int i = 0; i < trees.length; i++)
            {
                if(trees[i].child == null)
                    continue;
                retval += "Tree" + i + ": " + "\n" + " " + trees[i].printTreeForHumansNoLog(state) + "\n";
            }
            return retval;
        }
    ...
    //fzhang   the size of individual is the number of nodes
    public long size()
        {
        long size = 0;
        for(int x=0;x<trees.length;x++)
