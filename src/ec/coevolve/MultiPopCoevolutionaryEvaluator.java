...
import ec.simple.*;
import ec.util.*;
import yimei.jss.ruleoptimisation.RuleCoevolutionProblem;
...
public class MultiPopCoevolutionaryEvaluator extends Evaluator
   ...
   //public double fracAdapted;
    ...
    public void evaluatePopulation(final EvolutionState state)
        ...
        //modified by mengxu 2021.07.15 for MCTS, this is used except for generation 0
        public void evaluateIndividual(final EvolutionState state, Individual individual, int subpopIndex)
        {
            // determine who needs to be evaluated
            boolean[] preAssessFitness = new boolean[state.population.subpops.length];
            boolean[] postAssessFitness = new boolean[state.population.subpops.length];
            ...
            // do evaluation except for generation 0, no need this part.
//            beforeCoevolutionaryEvaluation( state, state.population, (GroupedProblemForm)p_problem );

            ((RuleCoevolutionProblem)p_problem).preprocessIndividual(state,individual, preAssessFitness[subpopIndex], false);
            performIndividualCoevolutionaryEvaluation( state, subpopIndex, individual, (GroupedProblemForm)p_problem );

            //change the elites for cooperative coevolution
            ((RuleCoevolutionProblem)p_problem).postprocessIndividual(state, individual, postAssessFitness[subpopIndex], false);
//          //todo: need to do the following function after every individual was evaluated. Remember!!!
//            afterCoevolutionaryEvaluation( state, state.population, (GroupedProblemForm)p_problem );//change the elites
        }
...
//        }
        ...
        //modified by mengxu 2021.07.15 for MCTS
        public void performIndividualCoevolutionaryEvaluation( final EvolutionState state,
                                                     int subpopIndex,
            ...
            //==========================useful and important part=======================================
            //step 4: for each subpopulation, j means subPopulation   2*512*4*2= 8192  cost
                if (shouldEvaluateSubpop(state, subpopIndex, 0)){ // don't evaluate this subpopulation

                // for this individual
//                    System.out.println("subpopIndex: " + subpopIndex);
                    // Test against all the elites
                    for (int k = 0; k < eliteIndividuals[subpopIndex].length; k++) { //2
                        for (int ind = 0; ind < inds.length; ind++) { //2
                            if (ind == subpopIndex) {   //j = 0, 1  (ind j) ---> (0 0) or (1 1) that is to say, this is the subpopulation1
                                inds[ind] = individual; //inds[0] = individual = state.population.subpops[0].individuals[0];
                                //inds[1] = individual = state.population.subpops[1].individuals[1];
                                //the individuals to evaluate together
                                updates[ind] = true;   // updates[0] = true    updates[1] = true   evaluate
                    ...
                    /*    System.out.println("ind "+ ind);
                        ...
                        if(inds.length == 1){
                            System.out.println("inds.length == 1!");
                        }
                        prob.evaluate(state, inds, updates, false, subpops, 0);
                    }
                   /* System.out.println("Evaluated finished: population "+ j);
...
//            state.output.message("Evaluations: single individual!");
        }
