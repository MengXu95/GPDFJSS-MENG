...
public class EvolutionState implements Singleton
	...
	//modified by fzhang  29.4.2018  in order to get generation information
	public EvolutionState state;

	//modified by mengxu 2021.04.08
	public Mcts[] mctses = new Mcts[2];

    public int totalEvaluationTime = 0; //added by mengxu 2023.04.15 to stop with the same evaluation times
    ...
    */
    public void setup(final EvolutionState state, final Parameter base)
        ...
        //=======checkpoint: checkpoint, checkpointprefix, modulo==========
        // we ignore the base, it's worthless anyway for EvolutionState
        // here, we have no checkpoint
        p = new Parameter(P_CHECKPOINT);  //false
        checkpoint = parameters.getBoolean(p,null,false);

        p = new Parameter(P_CHECKPOINTPREFIX);//null
        ...
        p = new Parameter(P_CHECKPOINTMODULO); //1
        ...
        /* Set up the singletons: a single object, as distinguished from a pair */
        p=new Parameter(P_INITIALIZER);
        initializer = (Initializer)
            (parameters.getInstanceForParameter(p,null,Initializer.class));
        initializer.setup(this,p);
        //System.out.println(initializer); //ec.gp.GPInitializer
        //GPInitializer is a SimpleInitializer which sets up all the Cliques,
        //(the initial [tree/node]constraints, types, and function sets) for the GP system. <p>Note that the Cliques must be set up in a very particular order:

        p=new Parameter(P_FINISHER);
        finisher = (Finisher)
            (parameters.getInstanceForParameter(p,null,Finisher.class));
        finisher.setup(this,p);  // do nothing,do not care
        p=new Parameter(P_BREEDER);
        breeder = (Breeder)
            (parameters.getInstanceForParameter(p,null,Breeder.class));
        breeder.setup(this,p);

        //System.out.println(breeder);  //ec.simple.SimpleBreeder

        //==============here is different between before!!!!!================
    ...
    //fzhang 21.7.2018 reset   not finished
	public void setupResetProb(final EvolutionState state, final Parameter base) {
		Parameter p;
		p=new Parameter(P_INITIALIZER);
        initializer = (Initializer)
            (parameters.getInstanceForParameter(p,null,Initializer.class));
  ...
  //modified by fzhang  29.4.2018  in order to get generation information
  	public Integer getStateGeneration() {
