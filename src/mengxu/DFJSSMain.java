...
//            AbstractRule sequencingRule = new HEFT(RuleType.SEQUENCING);
//            AbstractRule routingRule = new HEFT(RuleType.ROUTING);
            //baseline 17
//            AbstractRule sequencingRule = GPRule.readFromLispExpression(RuleType.SEQUENCING, "(* (+ (Max (* (Min OWT MWT) (* PT PT)) (* PT PT)) (Max (Max (* PT PT) (/ PT TIS)) (/ (Min (Max (Min WIQ TRANT) (- NIQ W)) (Max NOR PT)) (Max MWT (/ PT TIS))))) (+ PT WKR))");
//            AbstractRule routingRule = GPRule.readFromLispExpression(RuleType.ROUTING, "(+ (+ PT (- (+ (+ (+ WIQ TRANT) PT) WIQ) (Min (- W W) (Min MWT NOR)))) (+ (/ (+ (/ TIS OWT) (* TIS NOR)) (* (Max OWT TRANT) (Max WIQ PT))) (Max TRANT W)))");

            //multi-case NSGAII 17
//            AbstractRule sequencingRule = GPRule.readFromLispExpression(RuleType.SEQUENCING, "(+ PT (Min (+ (* NPT (+ TRANT PT)) (* WKR OWT)) (Min PT (+ OWT (* WIQ PT)))))");
//            AbstractRule routingRule = GPRule.readFromLispExpression(RuleType.ROUTING, "(Max (+ (* (* (Min TIS WKR) (+ OWT MWT)) (Min (Min WKR WIQ) (Min W PT))) (Max (* (- TIS NOR) (- TRANT NIQ)) (Max (- (/ TIS PT) (- NOR NIQ)) WIQ))) (- (/ (/ (- NIQ NPT) (Min NOR WIQ)) (Min (+ PT NPT) (- W OWT))) (/ TIS OWT)))");

            AbstractRule sequencingRule = new SPT(RuleType.SEQUENCING);
...
//                utilLevel, dueDateFactor, false);
        ...
        Simulation simulation = new HeterogeneousSimulation(seed,
                    null, null, numMachines, numJobs, warmupJobs,
                    minNumOperations, maxNumOperations,
                    utilLevel, dueDateFactor, false);
