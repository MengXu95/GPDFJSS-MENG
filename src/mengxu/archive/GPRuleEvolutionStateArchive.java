...
public class GPRuleEvolutionStateArchive extends GPRuleEvolutionState {
...
//		System.out.println("Generation " + generation + " bad run: " + badRun);
		...
		//2021.07.13 modified by mengxu to save the top 32 individuals and divide two kinds of tree in two archive
		PopulationUtils.sort(population);
		double bestFitness = population.subpops[0].individuals[0].fitness.fitness();
		System.out.println("bestFitness: " + bestFitness);
		System.out.println("bestFitness/baselineFitness: " + bestFitness/baselineFitness[generation]);

		List<GPTree> archiveTree0 = new ArrayList<>();
		List<GPTree> archiveTree1 = new ArrayList<>();
//		for(int i=0; i<Math.sqrt(population.subpops[0].individuals.length); i++){
//			archiveTree0.add(((GPIndividual)population.subpops[0].individuals[i]).trees[0]);
//			archiveTree1.add(((GPIndividual)population.subpops[0].individuals[i]).trees[1]);
//		}
		for(int i=0; i<10; i++){
			archiveTree0.add(((GPIndividual)population.subpops[0].individuals[i]).trees[0]);
			archiveTree1.add(((GPIndividual)population.subpops[0].individuals[i]).trees[1]);
		}
		ArchiveEvaluator.evaluateArchive(this,archiveTree0,archiveTree1);
		//-----------------------------------------------------------------
