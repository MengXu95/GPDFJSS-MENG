...
public class InvertedGenerationalDistance {
  public mengxu.util.qualityIndicator.util.MetricsUtil utils_;  //utils_ is used to access to the
   ...
   */
  public double invertedGenerationalDistance_MaxMin(EvolutionState state,
                                             double [][] front,
...
//    minimumValue = utils_.getMinimumValues(trueParetoFront, numberOfObjectives);
    maximumValue = ((PSLInitializer)((GPRuleEvolutionStatePSL)state).initializer).nadirPoint;
    minimumValue = ((PSLInitializer)((GPRuleEvolutionStatePSL)state).initializer).idealPoint;
  ...
  public double invertedGenerationalDistance_Manual(EvolutionState state,
                                                    double [][] front,
                                                    double [][] trueParetoFront,
                                                    double [] manualObj,
                                                    int numberOfObjectives) {
...
//    minimumValue = utils_.getMinimumValues(trueParetoFront, numberOfObjectives);
    ...
    // STEP 2. Get the normalized front and true Pareto fronts
    normalizedFront       = utils_.getNormalizedFront_Manual(front,manualObj);
    normalizedParetoFront = utils_.getNormalizedFront_Manual(trueParetoFront,manualObj);

    // STEP 3. Sum the distances between each point of the true Pareto front and
    // the nearest point in the true Pareto front
    double sum = 0.0;
    for (double[] aNormalizedParetoFront : normalizedParetoFront){
      if(aNormalizedParetoFront[0] >= Double.MAX_VALUE || aNormalizedParetoFront[0] >= Double.POSITIVE_INFINITY){
        sum += 0;
      }
      else{
        sum += Math.pow(utils_.distanceToClosedPoint(aNormalizedParetoFront,
  ...
  public static void main(String args[]) {
    ...
    double value = qualityIndicator.invertedGenerationalDistance_MaxMin(null,
    		                                 solutionFront,
    		                                 trueFront,
