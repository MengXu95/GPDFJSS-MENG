...
package mengxu.util.qualityIndicator.util;
...
public class MetricsUtil {
  ...
  public double distance(double [] a, double [] b) {
    ...
    if(a[0] >= Double.POSITIVE_INFINITY || a[0] >= Double.MAX_VALUE || b[0] >= Double.POSITIVE_INFINITY || b[0] >= Double.MAX_VALUE)
    {
  ...
  public double [][] getNormalizedFront(double [][] front,
    ...
    for (int i = 0; i < front.length;i++) {
      normalizedFront[i] = new double[maximumValue.length];
      for (int j = 0; j < maximumValue.length; j++) {
        //add by mengxu 2024.5.21
        if(front[i][j] >= Double.POSITIVE_INFINITY || front[i][j] >= Double.MAX_VALUE || front[i][j] >= maximumValue[j]){
          normalizedFront[i][j] = 1.0;
        }
        else{
...
//                (maximumValue[j] - minimumValue[j]);
        ...
        if((maximumValue[j] - minimumValue[j]) == 0){
          normalizedFront[i][j] = 1.0;
        }
      }
    }
...
//  } // getNormalizedFront
  ...
  public double [][] getNormalizedFront_Manual(double [][] front,
                                               double [] manualObj) {

    double [][] normalizedFront = new double[front.length][];

    for (int i = 0; i < front.length;i++) {
      normalizedFront[i] = new double[manualObj.length];
      for (int j = 0; j < manualObj.length; j++) {
        //add by mengxu 2024.5.21
        if(front[i][j] >= Double.POSITIVE_INFINITY || front[i][j] >= Double.MAX_VALUE){
          normalizedFront[i][j] = front[i][j];
        }
        else{
          normalizedFront[i][j] = front[i][j]/manualObj[j];
        }
      }
    }
    return normalizedFront;
