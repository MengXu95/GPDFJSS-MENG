...
public class MultiTaskLearningMultipleTreeRuleTestSingleRun {
	 ...
	 public static final long simSeed = 968356;
	    ...
	    public MultiTaskLearningMultipleTreeRuleTestSingleRun(String trainPath, RuleTypeV2 ruleType, int indexOfRun,
	    ...
	    public MultiTaskLearningMultipleTreeRuleTestSingleRun(String trainPath, RuleTypeV2 ruleType, int indexofRun, String testScenario,
...
//				result.setGenerationalParentSelectionDiversityStat(ResultFileReader.readDiversitiesFromFile(diversityFile,8));
				...
				for (int j = 0; j < result.getGenerationalRules().size(); j++) {
					...
					if (numTrees == 2) {
						EvolutionState state = new GPRuleEvolutionStateMultiTaskLearningSurrogate();
						((GPRuleEvolutionStateMultiTaskLearningSurrogate) state).utilisationLevels = new double[]{utilLevel};
						((GPRuleEvolutionStateMultiTaskLearningSurrogate) state).currentSubpop = 0;

						generationalRules[0].calcFitnessForMultiTask(  //in calcFitness(), it will check which one is routing/sequencing rule
								result.getGenerationalTestFitness(j), state,
...
//									result.getGenerationalParentSelectionDiversityStat(j));
	...
	public void writeToCSVMultiTaskNoSurrogate() {
		System.out.println("Run "+ indexOfRun);
		for(int subpop=0; subpop< testSetNames.size(); subpop++){
			System.out.println("Subpop " + subpop);
			for(int task=0; task< testSetNames.size(); task++) {
				System.out.println("Task " + task);
				String testSetName = testSetNames.get(task);
				SchedulingSet testSet = generateTestSet(testSetName);
				...
				File csvFile = new File(targetPath + "/" + testSetName + "_ind_" + subpop + "_" + indexOfRun + ".csv"); //create a .csv to save the test result
				List<TestResult> testResults = new ArrayList<>();

				File sourceFile = new File(trainPath + "job." + indexOfRun + ".out.stat");  //this file keeps the rule
				TestResult result = MultipleTreeTestResult.readSubPopFromFile(sourceFile, ruleType, numTrees, subpop);
	...
	public void writeToCSVManualRuleBaseline() {
		System.out.println("Run "+ indexOfRun);
		for(int subpop=0; subpop< testSetNames.size(); subpop++){
			System.out.println("Subpop " + subpop);
			for(int task=0; task< testSetNames.size(); task++) {
				...
				File csvFile = new File(targetPath + "/" + testSetName + "_ind_" + subpop + "_" + indexOfRun + "_manualRule.csv"); //create a .csv to save the test result
				List<TestResult> testResults = new ArrayList<>();

				testSet.lowerBoundsFromBenchmarkRule(objectives);
				RealMatrix lowerBoundMtx = testSet.getObjectiveLowerBoundMtx();

				long start = System.currentTimeMillis();

				System.out.println("Manual rule test fitness = " +
						lowerBoundMtx.getEntry(0,0));
		...
		public static void main(String[] args) {
			...
			MultiTaskLearningMultipleTreeRuleTestSingleRun multipletreeruleTest = new MultiTaskLearningMultipleTreeRuleTestSingleRun(trainPath, ruleType, indexofRun, testScenario,
...
//			multipletreeruleTest.writeToCSVMultiTaskNoSurrogate();
//			multipletreeruleTest.writeToCSVManualRuleBaseline();
		}
}
