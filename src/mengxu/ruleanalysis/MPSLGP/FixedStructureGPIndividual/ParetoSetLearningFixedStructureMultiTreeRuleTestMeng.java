package mengxu.ruleanalysis.MPSLGP.FixedStructureGPIndividual;
...
import mengxu.algorithm.multiobjective.MPSLGP.PSLInitializer;
...
import mengxu.ruleanalysis.MPSLGP.ParetoSetLearningMultipleTreeTestResult;
import mengxu.ruleanalysis.MPSLGP.ParetoSetLearningParetoFrontTestResult;
...
public class ParetoSetLearningFixedStructureMultiTreeRuleTestMeng {
	 ...
	 public static final long simSeed = 968356;
	    ...
	    public ParetoSetLearningFixedStructureMultiTreeRuleTestMeng(String trainPath, RuleTypeV2 ruleType, int numRuns,
...
//	                    System.out.println(((MultiObjectiveFitness)result.getGenerationalTestFitness(j)).objectives[1]);
							...
							for (int rule_i = 0; rule_i < generationalRules.length; rule_i++) {
								if (rule_i < generationalRules.length / 2) {//2 here represent the number of objectives considered
									ensembleSequencingRule.add(generationalRules[rule_i]);
								} else {
									ensembleRoutingRule.add(generationalRules[rule_i]);
								}
							}
							...
							testSet.getSimulations().get(0).useMeanRankEnsemble = false;
							testSet.getSimulations().get(0).useLinearEnsemble = true;
							testSet.getSimulations().get(0).useLS = false;
							ensembleRule.calcFitnessByLinearEnsemble(  //this is to get the final fitness by the voting of ensemble
									result.getGenerationalTestParetoFrontFitness(j).get(p), null,
									testSet, objectives);
						}
					}
	...
	public void writeEachRunParetoFrontToCSV(File targetPath, SchedulingSet testSet, int run){
		...
		//modified by mengxu 2022.08.11
		ParetoSetLearningParetoFrontTestResult result = ParetoSetLearningMultipleTreeTestResult.readParetoFrontFixedStructureTestResultFromFile(sourceFile, ruleType, numTrees, this.preferences.length);
...
//				TestResult result = NSGAIIMultipleTreeTestResult.readParetoFrontFromFile(sourceFile, ruleType, numTrees);
		...
		for (int j = 0; j < result.getGenerationalParetoFrontRules().size(); j++) {
			...
			for(int p = 0; p < generationalParetoFrontRules.size(); p++){
				...
				((GPRuleEvolutionStatePSL)state).evaluatePreferenceIndex = p; //modified by mengxu
...
//                              testFit.getObjective(k));
		...
		public static void main(String[] args) {
			...
			ParetoSetLearningFixedStructureMultiTreeRuleTestMeng multipletreeruleTest = new ParetoSetLearningFixedStructureMultiTreeRuleTestMeng(trainPath, ruleType, numRuns, testScenario, testSetName, numTrees, numPreferences);
