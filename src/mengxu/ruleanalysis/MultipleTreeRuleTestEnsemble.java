...
public class MultipleTreeRuleTestEnsemble {
	 ...
	 public static final long simSeed = 968356;
	    ...
	    public MultipleTreeRuleTestEnsemble(String trainPath, RuleTypeV2 ruleType, int numRuns,
...
//				testResults.add(result);
	             ...
	             //modified by mengxu add ensemble test 2021.05.08
				File ensembleSourceFile = new File(trainPath + "job." + i + ".ensemble.stat");  //this file keeps the rule
				...
				for (int j = 0; j < ensembleResult.getGenerationalEnsembleRules().size(); j++) {
					...
					if (numTrees == 2) {
						generationalEnsembleRules.calcFitness(  //in calcFitness(), it will check which one is routing/sequencing rule
								ensembleResult.getGenerationalEnsembleTestFitnesses().get(j), null,
								testSet, objectives);
					}
					//generationalRules[1] is routing rule

					System.out.println("Generation " + j + ": ensemble test fitness = " +
							ensembleResult.getGenerationalEnsembleTestFitnesses().get(j).fitness());
				}
				...
				result.setGenerationalEnsembleTestFitnesses(ensembleResult.getGenerationalEnsembleTestFitnesses());

				testResults.add(result);
//	            testResults.add(ensembleResult);
	        }
	        ...
	        try {
	            ...
	            for (int i = 0; i < numRuns; i++) {
	                ...
	                for (int j = 0; j < result.getGenerationalRules().size(); j++) { //use rules in each generation for testing
						...
						MultiObjectiveFitness ensembleTestFit =
								(MultiObjectiveFitness)result.getGenerationalEnsembleTestFitnesses().get(j);
...
//	                                numUniqueTerminals + ",");
		...
		public static void main(String[] args) {
			...
			MultipleTreeRuleTestEnsemble multipletreeruleTest = new MultipleTreeRuleTestEnsemble(trainPath, ruleType, numRuns, testScenario, testSetName, numTrees);
