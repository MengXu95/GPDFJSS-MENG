package mengxu.algorithm.diversepartnerselection.withNewStrategy.focusOnPoorCase;
...
public class GPRuleEvolutionStateDPSPC extends GPRuleEvolutionState {
	...
	public static final String P_FOCUS_RANDOM_CASE = "focus-random-case";
	private boolean focusRandomCase;
	...
	public List<Integer> poorCasesForGen = new ArrayList<>();
...
//	private boolean clusterWithFitness;
	...
	public void setup(EvolutionState state, Parameter base) {
		...
		focusRandomCase = state.parameters.getBoolean(new Parameter(P_FOCUS_RANDOM_CASE), null, false);
...
//		updateBinaryBringForPopulation(); //original
...
//		ensembleAll.add(ensemble);
		...
		if(focusRandomCase){
			poorCasesForGen = calculateTheFocusdRandomCases(individuals, 400);
		}
		else{
			poorCasesForGen = calculateTheFocusdPoorCases(individuals, 400);
		}
		...
		if(generation == 1 || generation == 25 || generation == 45){
			...
			writeMultiCaseFitnessOfTop400IndividualToFile(individuals, 400);
		}
...
//					writer.newLine();
	...
	public void multiCaseFitnessNormalisationForPopulation(){
		...
		double[] bests = new double[length0];
		double[] worsts = new double[length0];
		...
		for(int i=0; i<individuals.length; i++){
			...
			for(int j=0; j<length; j++){
				if(objectives[j][0] >= Double.MAX_VALUE || objectives[j][0] >= Double.POSITIVE_INFINITY){
					normalisation[j][0] = 1.1; //modified by mengxu 2022.03.07
				}
				else{
...
//		System.out.println("]");
	...
	public void writeMultiCaseFitnessOfTop400IndividualToFile(Individual[] individuals, int numTop){
	...
	//focus on random cases
	public List<Integer> calculateTheFocusdRandomCases(Individual[] individuals, int topNum){
		List<Integer> poorCases = new ArrayList<>();
		PopulationUtils.sort(individuals);
		OneInstanceMultiCaseMultiObjectiveFitness fitness = (OneInstanceMultiCaseMultiObjectiveFitness)individuals[0].fitness;
		int numCase = fitness.multiInstanceMultiObjectiveFitness.length;

//		int focusCaseNum = this.random[0].nextInt(numCase);
//		int[] perm = new int[numCase];
//		MOEADUtils.randomPermutation(this, 0, perm, numCase);
		for(int i=0; i<numCase; i++){
			if(this.random[0].nextBoolean()){
				poorCases.add(1);
			}
			else{
				poorCases.add(0);
