...
public class GPRuleEvolutionStateDPS extends GPRuleEvolutionState {
...
//		clusterWithFitness = state.parameters.getBoolean(new Parameter(P_CLUSTER_WITH_FITNESS), false);
	...
	public int evolve() {
		...
		System.out.println("Xover pro: " + sources[0].probability);
...
//		allTargetObjectiveList.add(targetObjectiveList);
		...
		//original modified by mengxu 2022.05.25
//		updateBinaryBringForPopulation();
//		calculateTheFocusdPoorCases(individuals, 400);
		updateBinaryBringForPopulationBasedEpsilon();
//		calculateTheFocusdPoorCases(individuals, 400);
		//=================================================================
...
//			bestFitnesses[i] = bestFit;
	...
	public void updateBinaryBringForPopulationBasedEpsilon(){
		Individual[] individuals = this.population.subpops[0].individuals;
...
//		}
		...
		List<Double> allEpsilon = calculateEpsilon(individuals,300);
		OneInstanceMultiCaseMultiObjectiveFitness fitness0 = (OneInstanceMultiCaseMultiObjectiveFitness)individuals[0].fitness;
		double[][] objectives0 = fitness0.multiInstanceMultiObjectiveFitness;
		int length0 = objectives0.length;
//		int[] bestIndexs = new int[length0];
//		double[] bestFitnesses = new double[length0];
		...
		for(int i=0; i<length0; i++){
			...
			double bestFit = ((OneInstanceMultiCaseMultiObjectiveFitness)individuals[indexBest].fitness).getFitness(i,0);
//			bestFitnesses[i] = bestFit;
			targets[i] = bestFit + allEpsilon.get(i);
//			System.out.println("Best vs Targets: [" + bestFit + " vs " + targets[i] + "]");
//			System.out.print(targets[i] + ", ");
		}
//		System.out.println("]");
	...
	public List<Double> calculateEpsilon(Individual[] allIndividuals, int topNum){
		PopulationUtils.sort(allIndividuals);
		List<Double> allEpsilon = new ArrayList<>();
		List<List<Double>> allCaseFitness = new ArrayList<>();
		OneInstanceMultiCaseMultiObjectiveFitness fitness = (OneInstanceMultiCaseMultiObjectiveFitness)allIndividuals[0].fitness;
		int numCase = fitness.multiInstanceMultiObjectiveFitness.length;
		...
		for(int ref=0; ref<topNum; ref++){
			fitness = (OneInstanceMultiCaseMultiObjectiveFitness)allIndividuals[ref].fitness;
			double[][] multiCaseFitness = fitness.multiInstanceMultiObjectiveFitness;
			for(int c=0; c<numCase; c++){
				double fit = multiCaseFitness[c][0];

				if(fit >= Double.MAX_VALUE || fit >= Double.POSITIVE_INFINITY){
//					System.out.println("case-fitness Inf.");
				}
				else{
