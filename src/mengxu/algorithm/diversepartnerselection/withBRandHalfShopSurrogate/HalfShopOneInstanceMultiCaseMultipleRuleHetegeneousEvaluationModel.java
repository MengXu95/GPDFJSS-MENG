...
import mengxu.algorithm.lexicaseselection.OneInstanceMultiCaseMultipleTreeMultipleRuleEvaluationModel;
...
public class HalfShopOneInstanceMultiCaseMultipleRuleHetegeneousEvaluationModel extends OneInstanceMultiCaseMultipleTreeMultipleRuleEvaluationModel implements Surrogate {
...
// 	}
	...
	public void evaluate(EvolutionState state, Individual[] individuals) {
    	for(Individual ind:individuals){
    		if(((GPIndividual)ind).trees.length == 2){
				AbstractRule sequencingRule = new GPRule(RuleType.SEQUENCING, ((GPIndividual) ind).trees[0]);
				AbstractRule routingRule = new GPRule(RuleType.ROUTING, ((GPIndividual) ind).trees[1]);
