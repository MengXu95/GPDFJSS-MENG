...
public class GPRuleEvolutionStateMultiCaseCluster extends GPRuleEvolutionState {
...
//		//modified by mengxu 2021.06.29 just use the top 300 to do parent=======
		double[][] indsMultiCaseFitness = getIndsMultiCaseFitness();
		double[][] indsCharListsMultiTree300 = Arrays.copyOfRange(indsMultiCaseFitness,  0 ,  clusterElementSize );
...
//			writeParentsPearsonScoreToFile();
	...
	public double[][] getIndsMultiCaseFitness(){
		int popSize = this.population.subpops[0].individuals.length;
		Individual ind = this.population.subpops[0].individuals[0];
		double[][] fit = ((OneInstanceMultiCaseMultiObjectiveFitness)ind.fitness).multiInstanceMultiObjectiveFitness;
		int numCase = fit.length;
		double[][] indsMultiCaseFitness = new double[popSize][numCase];
		for(int i=0; i<popSize; i++){
			ind = this.population.subpops[0].individuals[i];
			fit = ((OneInstanceMultiCaseMultiObjectiveFitness)ind.fitness).multiInstanceMultiObjectiveFitness;
			for(int j=0; j<numCase; j++){
				indsMultiCaseFitness[i][j] = fit[j][0];
			}
		}
		return indsMultiCaseFitness;
	}
