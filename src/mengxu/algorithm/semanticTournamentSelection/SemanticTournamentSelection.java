package mengxu.algorithm.semanticTournamentSelection;
...
public class SemanticTournamentSelection extends SelectionMethod implements SteadyStateBSourceForm {
...
//    public static final String P_CANDIDATE_SIZE = "candidate-size";
    ...
    public static final String P_PICKSMALLSIZE = "pick-small-size";
    public boolean pickSmallSize;
...
//            state.output.fatal("The number of cases must be an integer >= 1.", base.push(P_NUMCASE), def.push(P_NUMCASE));
        ...
        this.pickSmallSize = state.parameters.getBoolean(new Parameter(P_PICKSMALLSIZE), null, true);
...
//        candidateSize = state.parameters.getInt(new Parameter(P_CANDIDATE_SIZE),null,1);
    ...
    public int produce(int subpopulation, EvolutionState state, int thread) { //epsilon lexicase selection
        if(state.generation>((GPRuleEvolutionStateMCSTS)state).genForUseLS){
//            System.out.println("Tournament selection index: " + produceTournament(subpopulation, state, thread));
            int index = produceSemanticTournament(subpopulation, state, thread);
...
//                ((GPRuleEvolutionStateMV0)state).parentIndex.add(index);
                ((GPRuleEvolutionStateMCSTS)state).parentFitness.add(indexFitness);//add 2021.12.09
            }
            return index;
        }
        else{
...
//                ((GPRuleEvolutionStateMV0)state).parentIndex.add(index);
    ...
    //add by mengxu 2022.05.07
    public int produceSemanticTournament(final int subpopulation,
                                 final EvolutionState state,
                                 final int thread)
    {
        // pick size random individuals, then pick the best.
        ...
        {
            int j = getRandomIndividual(x, subpopulation, state, thread);
            if(betterThanWilcoxonTest(oldinds[j], oldinds[best], subpopulation, state, thread)) {  // j is better than best
                best = j;
            }
            else if(this.pickSmallSize){
                if(oldinds[j].size() < oldinds[best].size()){
                    best = j;
                }
            }
            else{
    ...
    public boolean betterThanWilcoxonTest(Individual first, Individual second, int subpopulation, EvolutionState state, int thread)
    {
        double[][] firstFitness = ((OneInstanceMultiCaseMultiObjectiveFitness) first.fitness).multiInstanceMultiObjectiveFitness;
        double[][] secondFitness = ((OneInstanceMultiCaseMultiObjectiveFitness) second.fitness).multiInstanceMultiObjectiveFitness;

        double[] firstFit = new double[firstFitness.length];
        double[] secondFit = new double[secondFitness.length];

//        boolean[] totalSame = new boolean[firstFitness.length];
        int trueNum = 0;
        for(int i=0; i< firstFitness.length; i++){
            firstFit[i] = firstFitness[i][0];
            secondFit[i] = secondFitness[i][0];
            if(firstFit[i] == secondFit[i]){
//                totalSame[i] = true;
                trueNum++;
            }
//            else{
...
//        mannWhitneyUTest.mannWhitneyUTest();
        ...
        double ref1 =  correlation.wilcoxonSignedRankTest(firstFit, secondFit, false);//the same with the original paper "semantic tournament selection"
//        double ref2 =  correlation.wilcoxonSignedRankTest(secondFit, firstFit, false);

        if(trueNum == firstFitness.length){
            return false;
        }
        else if(ref1 < this.alpha){
            return betterThan(first, second, subpopulation, state, thread);
        }
        return false;
    }
