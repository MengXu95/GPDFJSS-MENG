...
public class ParentSelectionSurrogateSVR extends TournamentSelection
    {
                           ...
                           final int thread)
        {
            ...
            if(state.generation>((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).switchGeneration){
...
//                                        individuals[index];
                                                   ...
                                                   final int thread)
        {
            ...
            MultiObjectiveFitnessMultiTaskLearningSVR fitness = (MultiObjectiveFitnessMultiTaskLearningSVR)state.population.subpops[0].individuals[0].fitness;
            ...
            if (pickWorst)
                for (int x=1;x<s;x++)
                {
                    ...
                    if (!fitness.betterThanBasedOnRelationshipModel(subpopIndex_j, ind_j, subpopIndex_best, ind_best, subpop_j, subpop_best, state, thread))  // j is at least as bad as best
                        best = j;
                }
            else
                for (int x=1;x<s;x++)
                                                   ...
                                                   final int thread)
        {
            ...
            for(int i=1; i<candidates.size(); i++){
                if(pickWorst){
                    ...
                    if (!fitness.betterThanBasedOnRelationshipModel(j, oldinds[j], best, oldinds[best], subpopulation, subpopulation, state, thread))  // j is at least as bad as best
                        best = j;
                }
                else{
                    int j = candidates.get(i);
                                                   ...
                                                   final int thread)
        {
            ...
            if (pickWorst)
                ...
                {
                    int j = getRandomIndividual(x, subpopulation, state, thread);
                    if (fitness.betterThanBasedOnRelationshipModel(j, oldinds[j], best, oldinds[best], subpopulation, subpopulation, state, thread))  // j is better than best
                        best = j;
                }
