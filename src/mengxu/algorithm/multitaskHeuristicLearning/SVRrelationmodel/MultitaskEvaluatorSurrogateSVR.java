 ...
 */
public class MultitaskEvaluatorSurrogateSVR extends SimpleEvaluator {
...
//        state.statistics.postEvaluationStatistics(state);
        ...
        ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).PCOnAllTasks = PCOnAllTasks;
...
//        fitnessAggregationAndUpdate(state, estimatedFitness);
        ...
        ArrayList<double[]> data = dataProcessing(state, PCOnAllTasks);
        try {
            svRModel.SVRmodelTrain(data);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).svRmodel = svRModel;
    ...
    public ArrayList<double[]> dataProcessing(EvolutionState state, double[][][][] PCOnAllTasks){
        ...
        double[] utilisationLevels = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).utilisationLevels;
...
//        double[] lowerBound = new double[this.multiproblems.length];
//        for(int i=0; i<this.multiproblems.length; i++) {
...
//    }
    ...
    // Strategy 2 2024.11.28: based on top 10 population rule
    public void lowerBoundCalculation(EvolutionState state){
        double[] lowerBound = new double[this.multiproblems.length];
        for(int i=0; i<this.multiproblems.length; i++) {
            ...
            for(int ind=0; ind<10; ind++){
                meanFit = meanFit + individuals[ind].fitness.fitness();
            }
            meanFit = meanFit/10;
            lowerBound[i] = meanFit;
        }
        this.lowerBound = lowerBound;
    }
    ...
    public double[][][][] calculatePCOnAllTasks(final EvolutionState state) {
        PhenoCharacterisation[][] phenoCharacterisations = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).phenoCharacterisations;
...
//            ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).currentSubpop = subpop;
//
...
//        }
        ...
        for(int subpop = 0; subpop < state.population.subpops.length; subpop++) {
            ...
            Individual[] inds = state.population.subpops[subpop].individuals; //only one subpop now
            //todo: seems wrong here!!! Double-check! Also double check for OnlySurrogate! 2024.11.30 by mengxu
            ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).currentSubpop = subpop;
            for (int treeID = 0; treeID < numTrees; treeID++) {//each population
                //each individual
...
//                    indsFitnessesMultiTreeTemp = new double[indsCharLists[0][ind].length][];
    ...
    public double[][][] fitnessEstimationOnAllTasksBasedOnSVRModel(final EvolutionState state, double[][][][] PCOnAllTasks){
        SVRmodel relationshipModel = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).svRmodel;
        double[] utilisationLevels = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).utilisationLevels;
        double[][][] indsFitnessesMultiTree = new double[state.population.subpops.length][state.population.subpops[0].individuals.length][state.population.subpops.length];
        for (int subpop = 0; subpop < state.population.subpops.length; subpop++) {
            Individual[] inds = state.population.subpops[subpop].individuals; //only one subpop now
            for(int i=0; i<inds.length; i++){
                ...
                for(int j=0; j<state.population.subpops.length; j++){
                    ...
                    if(j==subpop){
                        ...
                        double util_j = utilisationLevels[j];
                        double[] PC_j = PCOnAllTasks[subpop][i][j];
                        double[] util_PC_j = new double[PC_j.length+2];
                        util_PC_j[0] = util_j;
                        for(int k=0; k<PC_j.length; k++){
                            util_PC_j[k+1] = PC_j[k];
                        }
                        util_PC_j[PC_j.length+1] = -1;
                        double estimated_fit = Double.POSITIVE_INFINITY;
                        try {
                            estimated_fit = relationshipModel.predict(util_PC_j);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        indsFitnessesMultiTree[subpop][i][j] = estimated_fit;
                    }
                }
            }
        }
                                ...
                                int threadnum, SimpleProblemForm[] probs)
    {
        ...
        for(int pop=0;pop<len;pop++)
        {
            ...
            ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).currentSubpop = pop;
