...
import mengxu.algorithm.multitaskHeuristicLearning.onlysurrogate.MultitaskEvaluatorOnlySurrogate;
...
public class MultiObjectiveFitnessMultiTaskLearningSVR extends MultiObjectiveFitness
    {
     ...
     /*   if (newObjectives.length != objectives.length)
        ...
        for (int i = 0; i < newObjectives.length; i++)
            ...
            if (_f >= Double.POSITIVE_INFINITY || _f <= Double.NEGATIVE_INFINITY || Double.isNaN(_f))
                {
                    //todo: need to modify the problem: java.lang.NullPointerException 2021.06.03
...
//                }
  ...
  /*    0.798331424977548
    ...
    public Object clone()
        {
        MultiObjectiveFitnessMultiTaskLearningSVR f = (MultiObjectiveFitnessMultiTaskLearningSVR) (super.clone());
    ...
    public boolean equivalentTo(Fitness _fitness)
        {
        MultiObjectiveFitnessMultiTaskLearningSVR other = (MultiObjectiveFitnessMultiTaskLearningSVR) _fitness;
    ...
    public boolean betterThan(Fitness fitness)
        {
        return paretoDominates((MultiObjectiveFitnessMultiTaskLearningSVR)fitness);
        }

        public boolean betterThanBasedOnRelationshipModel(int index_first, Individual first,
                                                          int index_second, Individual second,
                                                          int subpop_first, int subpop_second, EvolutionState state, int thread) {
            boolean fitnessBasedOnRank = ((MultitaskEvaluatorSurrogateSVR)state.evaluator).fitnessBasedOnRank;
            if(fitnessBasedOnRank){
                return betterThanBasedOnEstimatedRank(index_first,first,index_second,second,subpop_first,subpop_second,state,thread);
            }
            else{
//                return betterThanBasedOnEstimatedFitness_Square_Normalise(index_first,first,index_second,second,subpop_first,subpop_second,state,thread);
//                return betterThanBasedOnEstimatedFitness(index_first,first,index_second,second,subpop_first,subpop_second,state,thread);
                return betterThanBasedOnEstimatedFitness_Normalise(index_first,first,index_second,second,subpop_first,subpop_second,state,thread);
            }
        }

        public boolean betterThanBasedOnEstimatedFitness_Normalise(int index_first, Individual first,
                                                                          int index_second, Individual second,
                                                                          int subpop_first, int subpop_second, EvolutionState state, int thread) {
            double[][][] estimatedFitness = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).estimatedFitness;
            double[] lowerbound = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).lowerBound;

            double weight_main;
            double weight_other;

            boolean average_weight = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).average_weight;
            double main_task_weight = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).main_task_weight;
            if(average_weight){
                weight_other = 1.0/(double)state.population.subpops.length;
                weight_main = weight_other;
            }
            else{
                weight_main = main_task_weight;
                weight_other = (1 - weight_main)/(state.population.subpops.length-1);
            }

            double fit_first = weight_main * first.fitness.fitness() / lowerbound[subpop_first];
            double count_first = 1;
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_first) {
                    double estimated_fit_first_i = estimatedFitness[subpop_first][index_first][i] / lowerbound[i];
                    if(estimatedFitness[subpop_first][index_first][i] >= Double.MAX_VALUE || estimatedFitness[subpop_first][index_first][i] >= Double.POSITIVE_INFINITY){

                    }
                    else{
                        fit_first = fit_first + weight_other * estimated_fit_first_i;
                        count_first = count_first + 1;
                    }
                }
            }

            double fit_second = weight_main * second.fitness.fitness() / lowerbound[subpop_second];
            double count_second = 1;
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_second) {
                    double estimated_fit_second_i = estimatedFitness[subpop_second][index_second][i] / lowerbound[i];
                    if(estimatedFitness[subpop_second][index_second][i] >= Double.MAX_VALUE || estimatedFitness[subpop_second][index_second][i] >= Double.POSITIVE_INFINITY){

                    }
                    else{
                        fit_second = fit_second + weight_other * estimated_fit_second_i;
                        count_second = count_second + 1;
                    }
                }
            }

            fit_first = fit_first/count_first * state.population.subpops.length;
            fit_second = fit_second/count_second * state.population.subpops.length;

            return fit_first < fit_second;
        }

        public boolean betterThanBasedOnEstimatedFitness(int index_first, Individual first,
                                                         int index_second, Individual second,
                                                         int subpop_first, int subpop_second, EvolutionState state, int thread) {
            double[][][] estimatedFitness = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).estimatedFitness;
            double[] lowerbound = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).lowerBound;
            ...
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_first) {
                    double estimated_fit_first_i = estimatedFitness[subpop_first][index_first][i];
                    fit_first = fit_first + estimated_fit_first_i;
                }
            }
            ...
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_second) {
                    double estimated_fit_second_i = estimatedFitness[subpop_second][index_second][i];
                    fit_second = fit_second + estimated_fit_second_i;
                }
            }

            fit_first = fit_first/state.population.subpops.length;
            fit_second = fit_second/state.population.subpops.length;

            return fit_first < fit_second;
        }

        public boolean betterThanBasedOnEstimatedRank(int index_first, Individual first,
                                                                       int index_second, Individual second,
                                                                       int subpop_first, int subpop_second, EvolutionState state, int thread) {
            double[][][] estimatedRank = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).estimatedRank;

            double rank_first = estimatedRank[subpop_first][index_first][subpop_first]*estimatedRank[subpop_first][index_first][subpop_first];
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_first) {
                    double estimated_rank_first_i = estimatedRank[subpop_first][index_first][i]*estimatedRank[subpop_first][index_first][i];
                    rank_first = rank_first + estimated_rank_first_i;
                }
            }

            double rank_second = estimatedRank[subpop_second][index_second][subpop_second]*estimatedRank[subpop_second][index_second][subpop_second];
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_second) {
                    double estimated_rank_second_i = estimatedRank[subpop_second][index_second][i]*estimatedRank[subpop_second][index_second][i];
                    rank_second = rank_second + estimated_rank_second_i;
                }
            }

            rank_first = Math.sqrt(rank_first)/state.population.subpops.length;
            rank_second = Math.sqrt(rank_second)/state.population.subpops.length;

            return rank_first < rank_second;
        }

        public boolean betterThanBasedOnEstimatedFitness_Square_Normalise(int index_first, Individual first,
                                                         int index_second, Individual second,
                                                         int subpop_first, int subpop_second, EvolutionState state, int thread) {
            double[][][] estimatedFitness = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).estimatedFitness;
            double[] lowerbound = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).lowerBound;

            double fit_first = (first.fitness.fitness() / lowerbound[subpop_first])*(first.fitness.fitness() / lowerbound[subpop_first]);
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_first) {
                    double estimated_fit_first_i = estimatedFitness[subpop_first][index_first][i] / lowerbound[i];
                    fit_first = fit_first + estimated_fit_first_i*estimated_fit_first_i;
                }
            }

            double fit_second = (second.fitness.fitness() / lowerbound[subpop_second])*(second.fitness.fitness() / lowerbound[subpop_second]);
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_second) {
                    double estimated_fit_second_i = estimatedFitness[subpop_second][index_second][i] / lowerbound[i];
                    fit_second = fit_second + estimated_fit_second_i*estimated_fit_second_i;
                }
            }

            fit_first = Math.sqrt(fit_first)/state.population.subpops.length;
            fit_second = Math.sqrt(fit_second)/state.population.subpops.length;

            return fit_first < fit_second;
        }

        public boolean betterThanBasedOnEstimatedRank_Square_Normalise(int index_first, Individual first,
                                                      int index_second, Individual second,
                                                      int subpop_first, int subpop_second, EvolutionState state, int thread) {
            double[][][] estimatedRank = ((MultitaskEvaluatorSurrogateSVR) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR) state).evaluator).estimatedRank;
            ...
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_first) {
                    double estimated_rank_first_i = estimatedRank[subpop_first][index_first][i];
                    rank_first = rank_first + estimated_rank_first_i*estimated_rank_first_i;
                }
            }
            ...
            for (int i = 0; i < state.population.subpops.length; i++) {
                if (i != subpop_second) {
                    double estimated_rank_second_i = estimatedRank[subpop_second][index_second][i];
                    rank_second = rank_second + estimated_rank_second_i*estimated_rank_second_i;
                }
            }
    ...
    public boolean paretoDominates(MultiObjectiveFitnessMultiTaskLearningSVR other)
        {
        boolean abeatsb = false;
    ...
    public static ArrayList partitionIntoParetoFront(Individual[] inds, ArrayList front, ArrayList nonFront)
        ...
        for (int i = 1; i < inds.length; i++)
            ...
            for (int j = 0; j < frontSize; j++)
                ...
                if (((MultiObjectiveFitnessMultiTaskLearningSVR) (frontmember.fitness)).paretoDominates((MultiObjectiveFitnessMultiTaskLearningSVR) (ind.fitness)))
                    {
                ...
                else if (((MultiObjectiveFitnessMultiTaskLearningSVR) (ind.fitness)).paretoDominates((MultiObjectiveFitnessMultiTaskLearningSVR) (frontmember.fitness)))
                    {
                    yank(j, front);
    ...
    public static ArrayList partitionIntoRanks(Individual[] inds)
        ...
        while(inds.length > 0) //the dominated individuals
            ...
            MultiObjectiveFitnessMultiTaskLearningSVR.partitionIntoParetoFront(inds, front, nonFront); //after this, the individuals are divided into two groups: front and nonfront
     ...
     */
    //todo: If all objectives' value are Infinite, how?
    public double sumSquaredObjectiveDistance(MultiObjectiveFitnessMultiTaskLearningSVR other)
        {
        double s = 0;
     ...
     */
    public double manhattanObjectiveDistance(MultiObjectiveFitnessMultiTaskLearningSVR other)
        {
        double s = 0;
        for (int i = 0; i < objectives.length; i++)
            {
    ...
    public void setToMeanOf(EvolutionState state, Fitness[] fitnesses)
        ...
        for(int i = 0; i < objectives.length; i++)
            ...
            for(int k = 0; k < fitnesses.length; k++)
                {
                MultiObjectiveFitnessMultiTaskLearningSVR f = (MultiObjectiveFitnessMultiTaskLearningSVR) fitnesses[k];
                sum += f.objectives[i];
                }
