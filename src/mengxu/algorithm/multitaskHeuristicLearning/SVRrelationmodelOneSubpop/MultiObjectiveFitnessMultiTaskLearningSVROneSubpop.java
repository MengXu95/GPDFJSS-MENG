...
public class MultiObjectiveFitnessMultiTaskLearningSVROneSubpop extends MultiObjectiveFitness
    {
...
//                }
  ...
  /*    0.798331424977548
    ...
    public Object clone()
        {
        MultiObjectiveFitnessMultiTaskLearningSVROneSubpop f = (MultiObjectiveFitnessMultiTaskLearningSVROneSubpop) (super.clone());
    ...
    public boolean equivalentTo(Fitness _fitness)
        {
        MultiObjectiveFitnessMultiTaskLearningSVROneSubpop other = (MultiObjectiveFitnessMultiTaskLearningSVROneSubpop) _fitness;
    ...
    public boolean betterThan(Fitness fitness)
        {
        return paretoDominates((MultiObjectiveFitnessMultiTaskLearningSVROneSubpop)fitness);
        }

        public boolean betterThanBasedOnRelationshipModel(int index_first, int index_second, EvolutionState state, int thread) {
            return betterThanBasedOnEstimatedFitness_Normalise(index_first,index_second,state,thread);
        }

        public boolean betterThanBasedOnEstimatedFitness_Normalise(int index_first, int index_second, EvolutionState state, int thread) {
            ArrayList<double[]> estimatedFitness = ((MultitaskEvaluatorSurrogateSVROneSubpop) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop) state).evaluator).estimatedFitness;
            double[] lowerbound = ((MultitaskEvaluatorSurrogateSVROneSubpop) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop) state).evaluator).lowerBound;
            ...
            boolean average_weight = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop) state).average_weight;
            double main_task_weight = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop) state).main_task_weight;
            if(average_weight){
            ...
            for (int i = 0; i < numTasks; i++) {
                double estimated_fit_first_i = estimatedFitness.get(i)[index_first] / lowerbound[i];
                if(estimatedFitness.get(i)[index_first] >= Double.MAX_VALUE || estimatedFitness.get(i)[index_first] >= Double.POSITIVE_INFINITY){

                }
                else{
                    if(i == currentTaskID){
                        fit_first = fit_first + weight_main * estimated_fit_first_i;
            ...
            for (int i = 0; i < numTasks; i++) {
                double estimated_fit_second_i = estimatedFitness.get(i)[index_second] / lowerbound[i];
                if(estimatedFitness.get(i)[index_second] >= Double.MAX_VALUE || estimatedFitness.get(i)[index_second] >= Double.POSITIVE_INFINITY){

                }
                else{
                    if(i == currentTaskID){
                        fit_second = fit_second + weight_main * estimated_fit_second_i;
            ...
            fit_first = fit_first/count_first * numTasks;
            fit_second = fit_second/count_second * numTasks;

            return fit_first < fit_second;
        }

        public boolean betterThanBasedOnEstimatedFitness(int index_first, int index_second, EvolutionState state, int thread) {
            ArrayList<double[]> estimatedFitness = ((MultitaskEvaluatorSurrogateSVROneSubpop) ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop) state).evaluator).estimatedFitness;
            int numTasks = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop) state).utilisationLevelNum;
            int currentTaskID = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop) state).currentUtilLevelIndex;
            ...
            double count_first = 0;
            for (int i = 0; i < numTasks; i++) {
                double estimated_fit_first_i = estimatedFitness.get(i)[index_first];
                if(estimatedFitness.get(i)[index_first] >= Double.MAX_VALUE || estimatedFitness.get(i)[index_first] >= Double.POSITIVE_INFINITY){
            ...
            double count_second = 0;
            for (int i = 0; i < numTasks; i++) {
                double estimated_fit_second_i = estimatedFitness.get(i)[index_second];
                if(estimatedFitness.get(i)[index_second] >= Double.MAX_VALUE || estimatedFitness.get(i)[index_second] >= Double.POSITIVE_INFINITY){
    ...
    public boolean paretoDominates(MultiObjectiveFitnessMultiTaskLearningSVROneSubpop other)
        {
        boolean abeatsb = false;
    ...
    public static ArrayList partitionIntoParetoFront(Individual[] inds, ArrayList front, ArrayList nonFront)
        ...
        for (int i = 1; i < inds.length; i++)
            ...
            for (int j = 0; j < frontSize; j++)
                ...
                if (((MultiObjectiveFitnessMultiTaskLearningSVROneSubpop) (frontmember.fitness)).paretoDominates((MultiObjectiveFitnessMultiTaskLearningSVROneSubpop) (ind.fitness)))
                    {
                ...
                else if (((MultiObjectiveFitnessMultiTaskLearningSVROneSubpop) (ind.fitness)).paretoDominates((MultiObjectiveFitnessMultiTaskLearningSVROneSubpop) (frontmember.fitness)))
                    {
                    yank(j, front);
    ...
    public static ArrayList partitionIntoRanks(Individual[] inds)
        ...
        while(inds.length > 0) //the dominated individuals
            ...
            MultiObjectiveFitnessMultiTaskLearningSVROneSubpop.partitionIntoParetoFront(inds, front, nonFront); //after this, the individuals are divided into two groups: front and nonfront
    ...
    public double sumSquaredObjectiveDistance(MultiObjectiveFitnessMultiTaskLearningSVROneSubpop other)
        {
        double s = 0;
     ...
     */
    public double manhattanObjectiveDistance(MultiObjectiveFitnessMultiTaskLearningSVROneSubpop other)
        {
        double s = 0;
        for (int i = 0; i < objectives.length; i++)
            {
    ...
    public void setToMeanOf(EvolutionState state, Fitness[] fitnesses)
        ...
        for(int i = 0; i < objectives.length; i++)
            ...
            for(int k = 0; k < fitnesses.length; k++)
                {
                MultiObjectiveFitnessMultiTaskLearningSVROneSubpop f = (MultiObjectiveFitnessMultiTaskLearningSVROneSubpop) fitnesses[k];
                sum += f.objectives[i];
                }
