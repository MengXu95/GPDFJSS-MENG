...
import mengxu.complexsimulation.ruleevaluation.MultipleTreeMultipleRuleHeterogeneousEvaluationModel;
...
public class GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop extends GPRuleEvolutionState {
	...
	public int currentUtilLevelIndex;

	public SVRmodel svRmodel = null;

	public ArrayList<double[][]> PCOnAllTasks = null;

	public int switchGeneration;

	public boolean average_weight;
	public double main_task_weight;
	public boolean util_level_rotate;
...
//		this.switchGeneration = state.parameters.getInt(p_switch_generation,null,1);
		...
		this.switchGeneration = this.utilisationLevelNum;

		this.phenoCharacterisations = new PhenoCharacterisation[this.utilisationLevelNum][2];
		this.utilisationLevels = new double[this.utilisationLevelNum];
		//calculate the pc of each individual in each subpop and do cluster--------------
		for(int i=0; i<this.utilisationLevelNum; i++){
			//initial utilisationLevel
			if(i==0){
				String P_PROBLEM_Util_LEVEL = "eval.problem.eval-model.sim-models.0.util-level";
				Parameter p = new Parameter(P_PROBLEM_Util_LEVEL);
				this.utilisationLevels[i] = state.parameters.getDouble(p, null, 0);
			}
			else{
				String P_PROBLEM_Util_LEVEL = "task-" + i;
				Parameter p = new Parameter(P_PROBLEM_Util_LEVEL);
				this.utilisationLevels[i] = state.parameters.getDouble(p, null, 0);
			}
		...
		this.currentUtilLevelIndex = 0;

		String P_UTIL_LEVEL_ROTATE = "util-level-rotate";
		Parameter p_util_level_rotate = new Parameter(P_UTIL_LEVEL_ROTATE);
		this.util_level_rotate = state.parameters.getBoolean(p_util_level_rotate,null,true);
	}
...
//		System.out.println("Edit 2 diversity: " + edit2Dvalue);
		...
		// Generate new task if needed by mengxu
		if(this.util_level_rotate){
			this.currentUtilLevelIndex = (this.currentUtilLevelIndex + 1)%this.utilisationLevelNum;
			...
			double currentUtilLevel = this.utilisationLevels[this.currentUtilLevelIndex];
			((DynamicSimulation)curSchedulingSet.getSimulations().get(0)).setUtilLevel(currentUtilLevel);
		}
