...
2. Send the exact shared prompt below to an external LLM and retain its JSON response.
...
The following files contain the **complete, directly usable system and user messages**, including all five reference pairs, their terminal facts, task settings, output schema and initial feedback lists. They are not summaries or templates with omitted sections. They are generated by [PopulationFactory](../OnlineEvoSpeak/PopulationFactory.java) and checked against the online generator in regression tests.
...
Both snapshots describe 100 individuals in total and the **first batch of 10**. The system prompt is exactly:
...
Use the complete **User Prompt** code block from the appropriate linked file as the user message. If the LLM interface has a separate system-instruction field, use that field for the system prompt. Do not include Markdown headings or code fences as part of the messages. The returned expressions are JSON strings; the exact online prompt deliberately does not ask the model to invent ECJ fitness encodings. The final product is still a TXT initial population, generated locally from those expressions.
...
Save the model's JSON response as a local file. For multiple batches, combine their `individuals` arrays into one object of the form `{"individuals":[...]}`; preserve each original response separately so its insights and explanations are not lost. Do not concatenate multiple JSON documents. Prepare at least the configured number of distinct feasible rule pairs, and use the same objective/scenario settings during validation and training.

The existing file-source validator and ECJ writer in OnlineEvoSpeak can be used **without making online calls**. With `evospeak.source=file`, it ignores credentials and consumes the manually prepared file. Example for weighted two-objective rules in `D:/experiments/OfflineEvoSpeak/candidates.json`:

```powershell
.\src\mengxu\algorithm\OnlineEvoSpeak\run.ps1 -Action train -ParamsFile .\src\mengxu\algorithm\OnlineEvoSpeak\multipletreegp-dynamicLLMWarmStartMO.params -Overrides @('evospeak.source=file','evospeak.population-file=D:/experiments/OfflineEvoSpeak/candidates.json','evospeak.run-gp=false','evospeak.output-directory=D:/experiments/OfflineEvoSpeak/validated')
```
