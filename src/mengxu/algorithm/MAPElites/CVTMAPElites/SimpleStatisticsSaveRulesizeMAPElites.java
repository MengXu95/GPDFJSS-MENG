...
import mengxu.algorithm.ensemble.ensembleGPJohnPark.EnsembleEvaluatorJohnPark;
...
public class SimpleStatisticsSaveRulesizeMAPElites extends SimpleStatisticsSaveRulesizeGen {
...
//			}
			...
			if (doGeneration && state.generation == state.numGenerations-1){//added by mengxu for MAP-Elites output
				if(state instanceof GPRuleEvolutionStateCVTMAPElites && ((GPRuleEvolutionStateCVTMAPElites) state).use_CVT_map){
					...
					state.output.println("All elites with Fitness: [-1]",statisticslog);
					state.output.println("Elites number: " + allIndividualsInCVTMap.size(),statisticslog);

					for(int e=0; e<allIndividualsInCVTMap.size(); e++){
						Individual individual = allIndividualsInCVTMap.get(e);
						individual.printIndividualForHumans(state,statisticslog);
					}
				}
				else if(state instanceof GPRuleEvolutionStateCVTMultiCaseMAPElites && ((GPRuleEvolutionStateCVTMultiCaseMAPElites) state).use_CVT_map){
				...
				else{ //added by mengxu for all kindsof GP to output all individuals in the final generation
					PopulationUtils.sort(state.population);
					Individual[] allIndividualsInFinalPopulation = state.population.subpops[0].individuals;
					state.output.println("All elites with Fitness: [-1]",statisticslog);
					state.output.println("Elites number: " + allIndividualsInFinalPopulation.length,statisticslog);

					for(int e=0; e<allIndividualsInFinalPopulation.length; e++){
						Individual individual = allIndividualsInFinalPopulation[e];
						individual.printIndividualForHumans(state,statisticslog);
					}
				}
			}
