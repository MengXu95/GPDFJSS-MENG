...
public class ParentSelectionCVTMultiCaseMAPElites extends TournamentSelection implements SteadyStateBSourceForm
    {
    /** default base */
    ...
    public int produce(final int subpopulation,
        ...
        if(((GPRuleEvolutionStateCVTMultiCaseMAPElites)state).use_CVT_map && ((GPRuleEvolutionStateCVTMultiCaseMAPElites)state).parentsFromCVTMapAndPop){
            ...
            int numberCandidate = oldinds.length + allIndividualsInCVTMap.size();
...
//        System.out.println("tournament size: " + s);
            ...
            if(((GPRuleEvolutionStateCVTMultiCaseMAPElites)state).tournamentSizeDynamic){
                s = (int)(s*(double)numberCandidate/(double)oldinds.length);
//                System.out.println("tournament size: " + s);
            }
            ...
            if (pickWorst)
                for (int x=1;x<s;x++)
                {
                    ...
                    if(best >= oldinds.length){//select from CVTMAP
                        indBest = allIndividualsInCVTMap.get(best - oldinds.length);
                    }
                    else{
                        indBest = oldinds[best];
                    }

                    int j = getRandomIndividual(x, subpopulation, state, thread, numberCandidate);
                    if(j >= oldinds.length){//select from CVTMAP
                        indCandidate = allIndividualsInCVTMap.get(j - oldinds.length);
                    }
                    else{
                        indCandidate = oldinds[j];
                    }

                    if (!betterThan(indCandidate, indBest, subpopulation, state, thread))  // j is at least as bad as best
                        best = j;
                }
            else
                for (int x=1;x<s;x++)
                {
                    ...
                    if (betterThan(indCandidate, indBest, subpopulation, state, thread))  // j is better than best
                        best = j;
                }
            return best;
        }
        else if(((GPRuleEvolutionStateCVTMultiCaseMAPElites)state).use_CVT_map && ((GPRuleEvolutionStateCVTMultiCaseMAPElites)state).parentsOnlyFromCVTMapRandom){
            // pick size random individuals, then pick the best.
            Individual[] oldinds = state.population.subpops[subpopulation].individuals;
            ArrayList<Individual> allIndividualsInCVTMap = ((GPRuleEvolutionStateCVTMultiCaseMAPElites)state).CVTMultiCasemap.getAllIndividualsInCVTMap(state);
            int numberCandidate = allIndividualsInCVTMap.size();
            int best = oldinds.length+getRandomIndividual(0, subpopulation, state, thread, numberCandidate);
            return best;
        }
        else{
            // pick size random individuals, then pick the best.
