...
//				best_i[x] = ((GPRuleEvolutionStateGridMAPElites)state).gridPCmap.getBestIndividualFromCurrentGen(state);
				best_i[x] = ((GPRuleEvolutionStateGridMAPElites)state).gridPCmap.getBestIndividual(state);
			}
			else{
				...
				for(int y=1;y<state.population.subpops[x].individuals.length;y++)
				{
					if (state.population.subpops[x].individuals[y] == null)
					{
						...
						best_i[x] = (Individual) state.population.subpops[x].individuals[y].clone();
					if (best_i[x] == null)
					{
						if (!warned)
						{
...
//			//add by mengxu 2025.1.3 to make it easy to compare
//			double originalFit = best_i[x].fitness.fitness();
//			double[] updatedFit = new double[1];
//			updatedFit[0] = originalFit;
//			((MultiObjectiveFitness)best_i[x].fitness).setObjectives(state,updatedFit);
		...
		//add by mengxu to output all the individuals in MAP
		if(state.generation == state.numGenerations-1){
			this.outputAllIndividualsInMAP(state);
		}

	}

	public void outputAllIndividualsInMAP(EvolutionState state){
		ArrayList<Individual> all_inds = ((GPRuleEvolutionStateGridMAPElites)state).gridPCmap.getAllIndividualsInGridMap();
		state.output.println("\n\nAll individuals in GridMAP:",statisticslog);
		state.output.println("GridMAP number: " + all_inds.size(),statisticslog);

		for(int e=0; e<all_inds.size(); e++){
			Individual individual = all_inds.get(e);
