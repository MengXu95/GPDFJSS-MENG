package mengxu.algorithm.mutation;
...
import mengxu.algorithm.GPRuleEvolutionStateV12;

import java.util.ArrayList;

/**
 * Created by MengXu on 06.02.2021.
 */
public class MutationBasedArchivePipeline  extends MutationPipeline{

    public double mutationRateBasedOnArchive = 0.5;
                       ...
                       final int thread)
    {
        ...
        for(int q=start; q < n+start; q++) //q=start = 0  q<1
        {
            ...
            for(int x=0;x<numTries;x++)
            {
                ...
                //Todo: add a probably to decide if mutation based on archive
                if(state.random[thread].nextDouble()>mutationRateBasedOnArchive){
                    ...
                    //Todo: how to select a suitable subtree from archive
                    ArrayList<ArrayList<Individual>> archive = ((GPRuleEvolutionStateV12)state).getArchiveInds();
                    int archiveSize = archive.get(subpopulation).size();
                    int selectIndIndex = state.random[thread].nextInt(archiveSize);
                    Individual selectInd = archive.get(subpopulation).get(selectIndIndex);


//                    p2 = ((GPRuleEvolutionStateV12)state).
                }
