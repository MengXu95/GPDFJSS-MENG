...
public class OneInstanceMultiCaseEvaluatorNSGAII extends SimpleEvaluator {
	/**
	...
	public Individual[] buildArchive(EvolutionState state, int subpop) {
			...
			if (rank.length + newSubpopulation.size() >= originalPopSize[subpop]) {
				...
				QuickSort.qsort(rank, new SortComparator() {
					public boolean lt(Object a, Object b) { //Returns true if a < b, else false 
						...
						return (((OneInstanceMultiCaseFitnessNSGAII) i1.fitness).sparsity > ((OneInstanceMultiCaseFitnessNSGAII) i2.fitness).sparsity);
					}
		...
		SimpleBreeder breeder = (SimpleBreeder) (state.breeder);
	...
	public ArrayList assignFrontRanks(Subpopulation subpop) {
		...
		ArrayList frontsByRank = OneInstanceMultiCaseFitnessNSGAII.partitionIntoRanks(inds);
		...
		for (int rank = 0; rank < numRanks; rank++) {
			...
			for (int ind = 0; ind < numInds; ind++)
				((OneInstanceMultiCaseFitnessNSGAII) (((Individual) (front.get(ind))).fitness)).rank = rank;
		}
		return frontsByRank;
	}
	...
	public void assignSparsity(Individual[] front) {
		int numObjectives = ((OneInstanceMultiCaseFitnessNSGAII) front[0].fitness).getObjectives().length;
		...
		for (int i = 0; i < front.length; i++) //front.length means how many individuals in this ranking(front)
			((OneInstanceMultiCaseFitnessNSGAII) front[i].fitness).sparsity = 0; //the first individual in this front, the sparsity
		//is assigned to 0
		...
		for (int i = 0; i < numObjectives; i++) {
			...
			QuickSort.qsort(front, new SortComparator() {
				public boolean lt(Object a, Object b) { //less than
					Individual i1 = (Individual) a;
					Individual i2 = (Individual) b;
					return (((OneInstanceMultiCaseFitnessNSGAII) i1.fitness)
							.getObjective(o) < ((OneInstanceMultiCaseFitnessNSGAII) i2.fitness).getObjective(o));
				}
			...
			((OneInstanceMultiCaseFitnessNSGAII) front[0].fitness).sparsity = Double.POSITIVE_INFINITY;
			((OneInstanceMultiCaseFitnessNSGAII) front[front.length - 1].fitness).sparsity = Double.POSITIVE_INFINITY;
			for (int j = 1; j < front.length - 1; j++) {
				OneInstanceMultiCaseFitnessNSGAII f_j = (OneInstanceMultiCaseFitnessNSGAII) (front[j].fitness);
				OneInstanceMultiCaseFitnessNSGAII f_jplus1 = (OneInstanceMultiCaseFitnessNSGAII) (front[j + 1].fitness);
				OneInstanceMultiCaseFitnessNSGAII f_jminus1 = (OneInstanceMultiCaseFitnessNSGAII) (front[j - 1].fitness);
