...
public class GPRuleEvolutionStateUpdateImmediate extends GPRuleEvolutionState {
	...
	ArrayList<ArrayList<Double>> storeBestPC = new ArrayList<>();
...
//		System.out.println("PC diversity: " + pcDvalue);
		...
		ArrayList<Double> bestPC = new ArrayList<>();
		bestPC.add((double) generation);
		for(double PC: indsCharListsMultiTree[0][bestIndex]){
			bestPC.add(PC);
		}
		storeBestPC.add(bestPC);
...
//		proHybird = pcDvalue * pcDvalue * (generation+1);
		proHybird =pcDvalue;
...
//		}
	    ...
	    if (generation == numGenerations-1)
			...
			writeBestPCToFile(); //modified by mengxu 2021.07.06
	        return R_FAILURE;
	        }
...
//		}
	...
	//2021.2.15 modified by mengxu
	public void writeBestPCToFile(){
		File diversities = new File("job." + jobSeed + ".bestPC.csv"); //successedTransfer[i][j]: task j makes a successful transfer for task i.
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(diversities));
			writer.write("Gen");
			for(int i=0;i<40;i++){
				writer.write(",decision" + i);
			}
			writer.newLine();
			for (int ind = 0; ind < storeBestPC.size(); ind++) {
				ArrayList<Double> ref = storeBestPC.get(ind);
				writer.write(String.valueOf(ref.get(0)));
				for(int i=1;i<40;i++){
