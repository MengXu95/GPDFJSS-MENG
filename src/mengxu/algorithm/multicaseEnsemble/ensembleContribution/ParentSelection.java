...
public class ParentSelection extends TournamentSelection {
    ...
    public int produce(int subpopulation, EvolutionState state, int thread) { //epsilon lexicase selection
        int index = produceFitnessAndLocalEnsembleContribution(subpopulation, state, thread);
...
//        int totalIndsAndEnsemble = oldinds.length; //todo: test by mengxu 2023.03.07
        ...
        if(best >= oldinds.length && ensembleRouletteSelection){
        ...
        else if(best >= oldinds.length && !ensembleRouletteSelection){
            EnsembleRule ensemble = ensembles.get(best - oldinds.length);
            //select individual with the best ensemble contribution
//            double randomPro = state.random[thread].nextDouble(true,true);
            double[] allElements = ensemble.getEnsembleContribution();
            List<Integer> allElementsIndex = ensemble.getElementOriginalIndex();//todo: need double check
            return allElementsIndex.get(state.random[thread].nextInt(allElements.length));
        }
        return best;
    }
