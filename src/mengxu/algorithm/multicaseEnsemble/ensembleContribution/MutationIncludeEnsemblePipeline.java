...
public class MutationIncludeEnsemblePipeline extends MutationPipeline
    {
    ...
    public int produce(final int min,
        ...
        for(int q=start; q < n+start; q++) //q=start = 0  q<1
            ...
            boolean parentIndividual = true;
            if(i == null){
                parentIndividual = false;
            }
            if(parentIndividual){
                q = produceNormalMutationOfIndividual(min, max, start, subpopulation, i, inds, state, thread, initializer, q, n);
            }
            else if(!parentIndividual){
                q = produceNormalMutationOfIndividualInEnsemble(min, max, start, subpopulation, inds, state, thread, initializer, q, n);
                return 0;
            }
            }
        return n;
        ...
        public int produceNormalMutationOfIndividualInEnsemble(final int min,
                                                          final int max,
                                                          final int start,
                                                          final int subpopulation,
                                                          final Individual[] inds,
...
//            EnsembleRule parentEnsemblesForMutation = ((EnsembleEvaluator)state.evaluator).parentsForMutation;
            ...
            EnsembleRule ensembleParent = ((EnsembleEvaluator)state.evaluator).parentsForMutation;
...
//            int indexPointOfParent = state.random[thread].nextInt(lengthParent);
            ...
            int indexPointOfParent =  -1;
            if(((GPRuleEvolutionStateEnsembleContribution)state).mutationUseEnsembleContribution &&
                ...
                for(int i=0; i< allElements.length; i++){
                    ...
                    if(randomPro < pro){
                        indexPointOfParent = i;
                        break; //modified by mengxu 2023.04.12
                    }
                }
            }
            else if(((GPRuleEvolutionStateEnsembleContribution)state).mutationUseEnsembleContribution &&
                    !((GPRuleEvolutionStateEnsembleContribution)state).ensembleRouletteSelection){
                //step 1: give selection probability based on the local ensemble contribution
                //step 2: select individual based on the probability
                //for parent 1
                ...
                indexPointOfParent = state.random[thread].nextInt(lengthParent);
            }

            GPIndividual individualOfParent = (GPIndividual)ensembleParent.getEnsemble().get(indexPointOfParent).clone();

            inds[q] = produceNormalMutation(min, max, start, subpopulation, individualOfParent, inds, state, thread, initializer, q, n);
            return q;
        }
