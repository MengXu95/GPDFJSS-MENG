...
import mengxu.algorithm.multicaseEnsemble.OneInstanceMultiCaseFitnessEnsemble;
...
public class GPRuleEvolutionStateMSMCE extends GPRuleEvolutionState {
...
//		dynamicSatisfyRate = state.parameters.getBoolean(new Parameter(P_DYNAMIC_SATISFY_RATE), null, false);
...
//		MultiInstanceMultiObjectiveFitness fitnessExample = (MultiInstanceMultiObjectiveFitness)individuals[0].fitness;
//		int numCase = fitnessExample.multiInstanceMultiObjectiveFitness.length;
		//----------------------------------------------------------

		//add 2021.12.23 mengxu to estimate the target objective list
//		Simulation simulation = ((MSMCEMultipleTreeMultipleRuleEvaluationModel)((MSMCEMultipleTreeRuleOptimizationProblem)evaluator.p_problem).getEvaluationModel()).getSchedulingSet().getSimulations().get(0);
//		double [] targetObjectiveList = estimateTargetObjectiveList(simulation,numCase);
//		allTargetObjectiveList.add(targetObjectiveList);
//		updateBinaryBringForPopulation();
//		calculateTheFocusdPoorCases(individuals, 400);
		//=================================================================
...
//		//Kmeans cluster--------------------------------
//		KMeans kRun =new KMeans(clusterSize, customerPoints);
...
//		double[] caseFit = new double[individuals.length];
		OneInstanceMultiCaseFitnessEnsemble fitness0 = (OneInstanceMultiCaseFitnessEnsemble)individuals[0].fitness;
		double[][] objectives0 = fitness0.multiInstanceMultiObjectiveFitness;
		int caseNum = objectives0.length;
		for(int i=0; i<caseNum; i++){
			for(int j=0; j<individuals.length; j++){
				OneInstanceMultiCaseFitnessEnsemble casefitness = (OneInstanceMultiCaseFitnessEnsemble)individuals[j].fitness;
...
//		double[] caseFit = new double[individuals.length];
		...
		for(int c=0; c<allCaseFitRank.size(); c++){
			...
			for(int j=0; j<caseFitRank.length; j++){
				...
				OneInstanceMultiCaseFitnessEnsemble casefitness = (OneInstanceMultiCaseFitnessEnsemble)individuals[indId].fitness;
...
//		double[] bestFitnesses = new double[length0];
		...
		for(int i=0; i<length0; i++){
			...
			double bestFit = ((OneInstanceMultiCaseFitnessEnsemble)individuals[indexBest].fitness).getFitness(i,0);
//			bestFitnesses[i] = bestFit;
			targets[i] = bestFit * (1 + satisfyRate);
		...
		for(int i=0; i<individuals.length; i++){
			OneInstanceMultiCaseFitnessEnsemble fitness = (OneInstanceMultiCaseFitnessEnsemble)individuals[i].fitness;
	...
	//focus on poor cases
	public List<Integer> calculateTheFocusdPoorCases(Individual[] individuals, int topNum){
		List<Integer> poorCases = new ArrayList<>();
		PopulationUtils.sort(individuals);
		OneInstanceMultiCaseFitnessEnsemble fitness = (OneInstanceMultiCaseFitnessEnsemble)individuals[0].fitness;
		int numCase = fitness.multiInstanceMultiObjectiveFitness.length;

		double[][] allFit = new double[topNum][numCase+1];
		double[] sumFit = new double[numCase+1];

		for(int ind=0; ind<topNum && ind<individuals.length; ind++){
			fitness = (OneInstanceMultiCaseFitnessEnsemble)individuals[ind].fitness;
			double[][] multiCaseFitness = fitness.multiInstanceMultiObjectiveFitness;
			allFit[ind][0] = fitness.fitness();
			sumFit[0] += fitness.fitness();
			for(int i=0; i<numCase; i++){
				allFit[ind][i+1] = multiCaseFitness[i][0];
				sumFit[i+1] += multiCaseFitness[i][0];
			}
		}

		System.out.print("Means: [");
		double meanStandardFit = sumFit[0];
		for(int i=0; i<numCase; i++){
			System.out.print(sumFit[i+1]/400 + ", ");
			if(sumFit[i+1] > meanStandardFit){
				poorCases.add(1);
			}
			else{
				poorCases.add(0);
	...
	public List<Double> calculateEpsilon(Individual[] allIndividuals){
		List<Double> allEpsilon = new ArrayList<>();
		List<List<Double>> allCaseFitness = new ArrayList<>();
		OneInstanceMultiCaseFitnessEnsemble fitness = (OneInstanceMultiCaseFitnessEnsemble)allIndividuals[0].fitness;
		int numCase = fitness.multiInstanceMultiObjectiveFitness.length;
		for(int c=0; c<numCase; c++){
			List<Double> allFitness = new ArrayList<>();
			allCaseFitness.add(allFitness);
		}


		for(int ref=0; ref<allIndividuals.length; ref++){
			fitness = (OneInstanceMultiCaseFitnessEnsemble)allIndividuals[ref].fitness;
			double[][] multiCaseFitness = fitness.multiInstanceMultiObjectiveFitness;
			for(int c=0; c<numCase; c++){
				double fit = multiCaseFitness[c][0];
				allCaseFitness.get(c).add(fit);
			}
		...
		for(int c=0; c<numCase; c++) {
			List<Double> allFitness = allCaseFitness.get(c);
