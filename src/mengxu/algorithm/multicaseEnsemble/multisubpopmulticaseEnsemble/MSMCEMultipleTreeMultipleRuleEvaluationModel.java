...
public class MSMCEMultipleTreeMultipleRuleEvaluationModel extends MultipleTreeMultipleRuleEvaluationModel {
...
//        System.out.println();
    ...
    //add by mengxu 2023.01.14 for multi-subpop multi-case ensemble
    public void evaluate(List<Fitness> currentFitnesses,
                         List<AbstractRule> rules,
                         ec.EvolutionState state,
                         List<List<AbstractRule>> bestRulesForPrecedingCases,
                         int numJobPerCase) {
        //expecting 2 rules here - one routing rule and one sequencing rule
        if (rules.size() != 2) {
            System.out.println("Rule evaluation failed!");
        ...
        countInd++;
        bestRulesForPrecedingCases.add(rules); //hide by mengxu 2023.02.07
...
//        double[][] fitnesses = new double[numCase][objectives.size()];//modified by mengxu
        ...
        for (int j = 0; j < simulations.size(); j++) {
            ...
            //System.out.println(simulation);
//            simulation.run();
            simulation.runBasedOnMultiCaseEnsembleRules(bestRulesForPrecedingCases,numJobPerCase);

            for (int i = 0; i < objectives.size(); i++) {
                //2018.10.23  cancel normalized process
//                double normObjValue = simulation.objectiveValue(objectives.get(i))  // this line: the value of makespan
//                        / schedulingSet.getObjectiveLowerBound(i, col);
//                int startJobID = simulation.getWarmupJobs() + (bestRulesForPrecedingCases.size()-1)*numJobPerCase;
//                int endJobID = simulation.getWarmupJobs() + bestRulesForPrecedingCases.size()*numJobPerCase;
                int startJobID = (bestRulesForPrecedingCases.size()-1)*numJobPerCase;
                int endJobID = bestRulesForPrecedingCases.size()*numJobPerCase;
                double caseFitness = simulation.objectiveValueCurCase(objectives.get(i),startJobID,endJobID);
                System.out.println("CaseFitness: " + caseFitness);
...
//                double[] multiCaseObjValue = simulation.objectiveValueMultiCase(objectives.get(i),numCase); // this line: the value of makespan
                ...
                for (WorkCenter w: simulation.getSystemState().getWorkCenters()) {
                    if (w.numOpsInQueue() > 100) {
                        caseFitness = Double.MAX_VALUE;
                        countBadrun++;
                        break;
                    }
                }
                //2018.10.23  cancel normalized process
                fitnesses[i] += caseFitness;  //the value of fitness is the normalization of the objective value
