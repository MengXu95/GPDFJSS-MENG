...
public class MutationPipelineSemanticV1 extends GPBreedingPipeline
    {
    ...
    //modified by fzhang 26.5.2018 in order to use it for multi-tree mutation  three variables have been changed
    //int numTries;
    public int numTries;
    ...
    public Object clone()
        {
        MutationPipelineSemanticV1 c = (MutationPipelineSemanticV1)(super.clone());
    ...
    public int produce(final int min,
        ...
        boolean nonduplicate = false;//add by mengxu 2023.06.06
        ...
        for(int q=start; q < n+start; q++) //q=start = 0  q<1
            ...
            if (tree!=TREE_UNFIXED && (tree<0 || tree >= i.trees.length))
                ...
                if(state instanceof GPRuleEvolutionStateOneInstanceMultiCase){
                    ParentForMutation parentForMutation = new ParentForMutation(q, i.fitness.fitness(), q, -1);
                    ((GPRuleEvolutionStateOneInstanceMultiCase) state).parentIndexForMutation.add(parentForMutation);
                }
                if(state instanceof GPRuleEvolutionStateMultiInstance){
                    ParentForMutation parentForMutation = new ParentForMutation(q, i.fitness.fitness(), q, -1);
                    ((GPRuleEvolutionStateMultiInstance) state).parentIndexForMutation.add(parentForMutation);
                }
                if(state instanceof GPRuleEvolutionStateMultiReplication){
                    ParentForMutation parentForMutation = new ParentForMutation(q, i.fitness.fitness(), q, -1);
                    ((GPRuleEvolutionStateMultiReplication) state).parentIndexForMutation.add(parentForMutation);
                }
                if(state instanceof GPRuleEvolutionStateMCSTS){
                    ParentForMutation parentForMutation = new ParentForMutation(q, i.fitness.fitness(), q, -1);
                    ((GPRuleEvolutionStateMCSTS) state).parentIndexForMutation.add(parentForMutation);
                }
...
//                    ParentForMutationMO parentForMutation = new ParentForMutationMO(((MultiObjectiveFitness)i.fitness).objectives, q, null);
//                    ((GPRuleEvolutionStateMOLS) state).parentIndexForMutation.add(parentForMutation);
//                }
...
//                if(state instanceof GPRuleEvolutionStateMV0){//this is for lexicase selection modified by mengxu 2022.02.28
//                    int index = ((GPRuleEvolutionStateMV0) state).parentIndexForMutation.size()-1;
//                    ((GPRuleEvolutionStateMV0) state).parentIndexForMutation.get(index).setChildID(q);
//                }

                //add by mengxu 2023.06.22
                double randPro = state.random[0].nextDouble();
                if(((GPRuleEvolutionStateSemanticV1)state).generation > ((GPRuleEvolutionStateSemanticV1)state).generationForDoPCMap) {
                    if(randPro < ((GPRuleEvolutionStateSemanticV1)state).limitPortsDoLocalSearch){
//                    if (randPro_j < ((GPRuleEvolutionStateSemantic) state).proForLocalSearchAndExplore) {
                        boolean acceptLocalSearchSeq_j = false;
                        boolean acceptLocalSearchRout_j = false;

                        List<double[]> PC_j = ((GPRuleEvolutionStateSemanticV1) state).calculatePC(j);
                        double[] sequencingPC_j = PC_j.get(0);
                        double[] routingPC_j = PC_j.get(1);

                        if(((GPRuleEvolutionStateSemanticV1)state).timesDoLocalSearch > ((GPRuleEvolutionStateSemanticV1)state).limitTimesDoLocalSearch){
                            ((GPRuleEvolutionStateSemanticV1) state).doSequencingLocalSearch = false;
                            ((GPRuleEvolutionStateSemanticV1) state).doRoutingLocalSearch = false;
                        }

                        if (((GPRuleEvolutionStateSemanticV1) state).doSequencingLocalSearch) {
                            acceptLocalSearchSeq_j = ((GPRuleEvolutionStateSemanticV1) state).PCMap.localSearchAcceptSequencing(sequencingPC_j, ((GPRuleEvolutionStateSemanticV1) state).localSearchAcceptDis);
                        }
                        if (((GPRuleEvolutionStateSemanticV1) state).doRoutingLocalSearch) {
                            acceptLocalSearchRout_j = ((GPRuleEvolutionStateSemanticV1) state).PCMap.localSearchAcceptRouting(routingPC_j, ((GPRuleEvolutionStateSemanticV1) state).localSearchAcceptDis);
                        }

                        if ((((GPRuleEvolutionStateSemanticV1) state).doSequencingLocalSearch && acceptLocalSearchSeq_j) || (((GPRuleEvolutionStateSemanticV1) state).doRoutingLocalSearch && acceptLocalSearchRout_j))
                        {
                            inds[q] = j;
                            ((GPRuleEvolutionStateSemanticV1)state).offspringIndexByLocalSearch.add(q); //add by mengxu 2023.07.06
                            ((GPRuleEvolutionStateSemanticV1)state).timesDoLocalSearch++;
                        }
                        else{
                            n--;
                        }
                    }
                    else if(randPro >= ((GPRuleEvolutionStateSemanticV1)state).limitPortsDoLocalSearch && randPro < ((GPRuleEvolutionStateSemanticV1)state).limitPortsDoLocalSearch + ((GPRuleEvolutionStateSemanticV1)state).limitPortsDoExploration){
                        boolean acceptExploreSeq_j = false;
                        boolean acceptExploreRout_j = false;
                        List<double[]> PC_j = ((GPRuleEvolutionStateSemanticV1) state).calculatePC(j);
                        double[] sequencingPC_j = PC_j.get(0);
                        double[] routingPC_j = PC_j.get(1);

                        if(((GPRuleEvolutionStateSemanticV1)state).timesDoExploration > ((GPRuleEvolutionStateSemanticV1)state).limitTimesDoExploration){
                            ((GPRuleEvolutionStateSemanticV1) state).doSequencingExplore = false;
                            ((GPRuleEvolutionStateSemanticV1) state).doRoutingExplore = false;
                        }

                        if (((GPRuleEvolutionStateSemanticV1) state).doSequencingExplore) {
                            if(((GPRuleEvolutionStateSemanticV1) state).focusOnWholeMap) {
                                acceptExploreSeq_j = ((GPRuleEvolutionStateSemanticV1) state).PCMap.exploreAcceptSequencingNonDuplicatedOffspringInMap(sequencingPC_j, ((GPRuleEvolutionStateSemanticV1) state).exploreAcceptDis);
                            }
                            else {
                                acceptExploreSeq_j = ((GPRuleEvolutionStateSemanticV1) state).PCMap.exploreAcceptSequencingNonDuplicatedOffspringInCurrentGeneration(sequencingPC_j, ((GPRuleEvolutionStateSemanticV1) state).exploreAcceptDis);
                            }
                        }
                        if (((GPRuleEvolutionStateSemanticV1) state).doRoutingExplore) {
                            if(((GPRuleEvolutionStateSemanticV1) state).focusOnWholeMap) {
                                acceptExploreRout_j = ((GPRuleEvolutionStateSemanticV1) state).PCMap.exploreAcceptRoutingNonDuplicatedOffspringInMap(routingPC_j, ((GPRuleEvolutionStateSemanticV1) state).exploreAcceptDis);
                            }
                            else {
                                acceptExploreRout_j = ((GPRuleEvolutionStateSemanticV1) state).PCMap.exploreAcceptRoutingNonDuplicatedOffspringInCurrentGeneration(routingPC_j, ((GPRuleEvolutionStateSemanticV1) state).exploreAcceptDis);
                            }
                        }

                        if (acceptExploreSeq_j || acceptExploreRout_j) {
                            inds[q] = j;
                            if(acceptExploreSeq_j){
                                ((GPRuleEvolutionStateSemanticV1) state).PCMap.updateSequencingPCInCurrentGeneration(sequencingPC_j);
//                                System.out.println("Add one more sequencing PC!");
                            }
                            if(acceptExploreRout_j){
                                ((GPRuleEvolutionStateSemanticV1) state).PCMap.updateRoutingPCInCurrentGeneration(routingPC_j);
//                                System.out.println("Add one more routing PC!");
                            }
                            ((GPRuleEvolutionStateSemanticV1)state).offspringIndexByExploration.add(q); //add by mengxu 2023.07.06
                            ((GPRuleEvolutionStateSemanticV1)state).timesDoExploration++;
                        }
                        else{
                            n--;
                        }
                        ...
                        inds[q] = j;
                        ((GPRuleEvolutionStateSemanticV1)state).offspringIndexByNormalEvolution.add(q); //add by mengxu 2023.07.06
                    }
                }
                else{
                    inds[q] = j;
...
//                double randPro_j = state.random[0].nextDouble();
//                if(((GPRuleEvolutionStateSemantic)state).doSequencingLocalSearch && randPro_j < ((GPRuleEvolutionStateSemantic)state).proForLocalSearchAndExplore){
//                    List<double[]> PC_j = ((GPRuleEvolutionStateSemantic)state).calculatePC(j);
//                    double[] sequencingPC_j = PC_j.get(0);
//                    double[] routingPC_j = PC_j.get(1);
//                    boolean acceptLocalSearchSeq_j = ((GPRuleEvolutionStateSemantic)state).PCMap.localSearchAcceptSequencing(sequencingPC_j, ((GPRuleEvolutionStateSemantic)state).localSearchAcceptDis);
//                    boolean acceptLocalSearchRout_j = ((GPRuleEvolutionStateSemantic)state).PCMap.localSearchAcceptRouting(routingPC_j, ((GPRuleEvolutionStateSemantic)state).localSearchAcceptDis);
//                    boolean acceptExploreSeq_j = ((GPRuleEvolutionStateSemantic)state).PCMap.exploreAcceptSequencing(sequencingPC_j, ((GPRuleEvolutionStateSemantic)state).exploreAcceptDis);
//                    boolean acceptExploreRout_j = ((GPRuleEvolutionStateSemantic)state).PCMap.exploreAcceptRouting(routingPC_j, ((GPRuleEvolutionStateSemantic)state).exploreAcceptDis);
//                    if(acceptLocalSearchSeq_j || acceptLocalSearchRout_j || acceptExploreSeq_j || acceptExploreRout_j){
//                        inds[q] = j;
//                    }
//                    else{
//                        n--;
