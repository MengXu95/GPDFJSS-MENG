...
public class GPRuleEvolutionStateMOEADarchive extends GPRuleEvolutionState {
...
//		System.out.println("Generation " + generation + " bad run: " + badRun);
		...
		boolean useBiasBalance = ((MOEADarchiveInitializer)this.initializer).useBias;
		boolean normaliseBeforeBiasBalance = ((MOEADarchiveInitializer)this.initializer).normaliseBeforeBiasBalance;
		boolean useArchive = ((MOEADarchiveInitializer)this.initializer).useArchive;
...
//		RuleOptimizationProblem problem = (RuleOptimizationProblem)evaluator.p_problem;
	...
	public void printFrontforGen(boolean updateBiasBalance, boolean normalisation){
		boolean useArchive = ((MOEADarchiveInitializer)this.initializer).useArchive;
		boolean archiveNoPCDuplicated = ((MOEADarchiveInitializer)this.initializer).archiveNoPCDuplicated;
		boolean multiDiff = ((MOEADarchiveInitializer)this.initializer).multiDiff;
		...
		if(normalisation){
			...
			double[] idealPoint = ((MOEADarchiveInitializer)this.initializer).idealPoint;
			double[] maxObjectives = ((MOEADarchiveInitializer)this.initializer).maxObjectives;
...
//							this.externalArchive.addIndividualAndPCToArchive((Individual) (sortedFront[i]), pc, true);
			...
			if(updateBiasBalance) {
				((MOEADarchiveInitializer) this.initializer).setBiasBalance(diffs);
			}
		}
		else{
