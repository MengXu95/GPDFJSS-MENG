...
public class NSGA2EvaluatorNoEnvironmentalSelectionNormalisation extends NSGA2Evaluator {
...
//            System.out.println("normalised max: " + max );
			...
			for (int j = 1; j < front.length - 1; j++) {
				...
				//Based on one time run, it might be better to have the following code
				int index_plus = j+1;
...
//			System.out.println("Print HERE!!!!!!");
		...
		NSGA2BreederNoEnvironmentalSelectionNormalisation breeder = (NSGA2BreederNoEnvironmentalSelectionNormalisation) (state.breeder);
		...
		for(int i=0; i<individuals.length; i++){
			...
			for(int j=0; j<numObjectives; j++){
				if(objectiveValues[j] >= Double.MAX_VALUE || objectiveValues[j] >= Double.POSITIVE_INFINITY){
					...
					objectives.get(j).add(objectiveValues[j]);
