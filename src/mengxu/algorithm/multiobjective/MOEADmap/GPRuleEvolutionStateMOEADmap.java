...
public class GPRuleEvolutionStateMOEADmap extends GPRuleEvolutionState {
...
//		System.out.println("Generation " + generation + " bad run: " + badRun);
		...
		boolean useBiasBalance = ((MOEADmapInitializer)this.initializer).useBias;
		boolean normaliseBeforeBiasBalance = ((MOEADmapInitializer)this.initializer).normaliseBeforeBiasBalance;
		boolean useArchive = ((MOEADmapInitializer)this.initializer).useArchive;
...
//		}
	...
	public void printFrontforGen(boolean updateBiasBalance, boolean normalisation){
		boolean useArchive = ((MOEADmapInitializer)this.initializer).useArchive;
		boolean archiveNoPCDuplicated = ((MOEADmapInitializer)this.initializer).archiveNoPCDuplicated;
		boolean multiDiff = ((MOEADmapInitializer)this.initializer).multiDiff;
		...
		if(normalisation){
			...
			double[] idealPoint = ((MOEADmapInitializer)this.initializer).idealPoint;
			double[] maxObjectives = ((MOEADmapInitializer)this.initializer).maxObjectives;
...
//							this.externalArchive.addIndividualAndPCToArchive((Individual) (sortedFront[i]), pc, true);
			...
			if(updateBiasBalance) {
				((MOEADmapInitializer) this.initializer).setBiasBalance(diffs);
			}
		}
		else{
