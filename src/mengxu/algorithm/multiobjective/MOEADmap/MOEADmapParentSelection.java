...
public class MOEADmapParentSelection extends TournamentSelection {
...
//				best = getRandomIndividual(start, subpopulation, state, thread);
		...
		if(state instanceof GPRuleEvolutionStateMOEADmap){
			((GPRuleEvolutionStateMOEADmap)state).parentIndex.add(best);
		}
		return best;
	}

	public int getRandomIndividual(int subproblem, int subpopulation, EvolutionState state, int thread) {
		MOEADmapBreeder breeder = (MOEADmapBreeder) state.breeder;
		MOEADmapInitializer init = (MOEADmapInitializer) state.initializer;
		if(breeder.getNeighborType() == MOEADmapInitializer.NeighborType.NEIGHBOR){
