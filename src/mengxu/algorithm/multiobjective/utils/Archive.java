...
public class Archive {
    List<Individual> archive; //based on generation
    ...
    public void addIndividualToArchive(Individual individual){
        archive.add(individual);
    }

    public void addIndividualToArchiveNoGenotypeDuplicated(Individual individual){
        boolean duplicated = false;

        GPTree[] gpTrees = ((GPIndividual)individual).trees;
        GPTree gpTree0 = gpTrees[0];
        GPTree gpTree1 = gpTrees[1];
        for(int i=0; i< this.archive.size(); i++){
            Individual ind = this.archive.get(i);
    ...
    public void addIndividualToArchiveNoGenotypeDuplicated(EvolutionState state, Individual individual){
        boolean duplicated = false;
...
//            }
    ...
    public void addIndividualAndPCToArchive(Individual individual, double[] pc, boolean checkPhenotypeDuplicated){
        if(checkPhenotypeDuplicated){
            ...
            if(isPhenotypeDuplicated == -1){
                ...
                archive.set(isPhenotypeDuplicated, individual);
                PC.set(isPhenotypeDuplicated, pc);
            }
        }
        else{
            archive.add(individual);
