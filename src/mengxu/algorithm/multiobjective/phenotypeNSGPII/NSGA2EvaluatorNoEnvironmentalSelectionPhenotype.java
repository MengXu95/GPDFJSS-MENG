...
package mengxu.algorithm.multiobjective.phenotypeNSGPII;
...
public class NSGA2EvaluatorNoEnvironmentalSelectionPhenotype extends NSGA2Evaluator {
...
//		}
	...
	public void evaluatePopulation(final EvolutionState state) {
		if(state.generation > 0){
			PhenoCharacterisation[] phenoCharacterisation = ((GPRuleEvolutionStateMO)state).phenoCharacterisation;
	...
	public void fillTheClearedIndividualsBasedOnParetoFront(final EvolutionState state){
		//generate offspring to fill the cleared one by using crossover and mutation only select parents from the Pareto front by meng xu 2023.05.30

		//Step 0: need to calculate the probability to be selected as parent using roulette selection based on the crowding distance
		PhenoCharacterisation[] phenoCharacterisation = ((GPRuleEvolutionStateMO)state).phenoCharacterisation;
		int ParetoFrontSize = this.ParetoFrontFromLastGeneration.length;
		//todo: need to consider the situation that only 1 individual at the Pareto front, this might happen!!!
		double[] probability = new double[ParetoFrontSize];
		double[] culmulativeCrowdingDistance = new double[ParetoFrontSize];
		//todo: need to give the individuals at the most left and the most right side a crowding distance of true value otherwise it is Inf, will be wrong;
		if(ParetoFrontSize==1){
			probability[0] = 1;
		}
		else if(ParetoFrontSize==2){
			probability[0] = 0.5;
			probability[1] = 1;
		}
		else{
			double maxCrowdingDistance = 0;
			for(int i=1; i<ParetoFrontSize-1; i++){
				double crowdingDistance = ((NSGA2MultiObjectiveFitness) this.ParetoFrontFromLastGeneration[i].fitness).sparsity;
				if(crowdingDistance >= Double.POSITIVE_INFINITY || crowdingDistance >= Double.MAX_VALUE){
					continue;
				}
				if(maxCrowdingDistance < crowdingDistance){
					maxCrowdingDistance = crowdingDistance;
				}
			}
			if(maxCrowdingDistance == 0){
				maxCrowdingDistance = 1;
			}
			double sumCrowdingDistance = 0;
			for(int i=0; i<ParetoFrontSize; i++){
				double crowdingDistance = ((NSGA2MultiObjectiveFitness) this.ParetoFrontFromLastGeneration[i].fitness).sparsity;
				if(i==0 || i==ParetoFrontSize-1 || crowdingDistance >= Double.POSITIVE_INFINITY || crowdingDistance >= Double.MAX_VALUE){
					crowdingDistance = maxCrowdingDistance;
				}
				sumCrowdingDistance += crowdingDistance;
				culmulativeCrowdingDistance[i] = sumCrowdingDistance;
			}
			for(int i=0; i<ParetoFrontSize; i++){
				double pro = culmulativeCrowdingDistance[i] / sumCrowdingDistance;
				if(culmulativeCrowdingDistance[i] >= Double.MAX_VALUE ||
						culmulativeCrowdingDistance[i] >= Double.POSITIVE_INFINITY ||
						sumCrowdingDistance == 0 ){
					System.out.println("culmulativeCrowdingDistance[i]: " + culmulativeCrowdingDistance[i]);
					System.out.println("sumCrowdingDistance: " + sumCrowdingDistance);
				}
				probability[i] = pro;
			}
		}
		...
		for (int subpop = 0; subpop < state.population.subpops.length; subpop++) {
			Subpopulation subpopulation = state.population.subpops[subpop];
			Individual[] sortedPop = subpopulation.individuals;
			// do regular breeding of this subpopulation
			int times = 15;
			int time = 0;
			while(this.clearedIndsIndex.size()>0){
				//Step 1: decide to do crossover or do mutation
				double randomRate = state.random[0].nextDouble();
				if(randomRate <= this.crossoverRate){
					//do crossover
					//Step 1-0: select the first parent based on roulette selection
					GPIndividual parent1 = null;
					int parent1Index = -1;
					GPIndividual parent2 = null;
					int parent2Index = -1;
					double randomRateP1 = state.random[0].nextDouble();
//					if(ParetoFrontSize == 2){
//						System.out.println("Check here!");
//					}
					for(int i=0; i<ParetoFrontSize-1; i++){
						if(randomRateP1 <= probability[i] && i==0){
							parent1 = (GPIndividual)this.ParetoFrontFromLastGeneration[i].clone();
							parent1Index = i;
							i = ParetoFrontSize; //break
						}
						else if(randomRateP1 > probability[i] && randomRateP1 <= probability[i+1]){
							parent1 = (GPIndividual)this.ParetoFrontFromLastGeneration[i+1].clone();
							parent1Index = i+1;
							i = ParetoFrontSize; //break
						}
					}
					//Step 1-1: select the second parent based on roulette selection
					//get the crowding distance of the left neighbor of parent1 if it has the left neighbor
					if(parent1Index == 0){
						//can only select the right neighbor as parent2
						parent2 = (GPIndividual)this.ParetoFrontFromLastGeneration[parent1Index+1].clone();
					}
					else if(parent1Index == ParetoFrontSize-1){
						//can only select the left neighbor as parent2
						parent2 = (GPIndividual)this.ParetoFrontFromLastGeneration[parent1Index-1].clone();
					}
					else if(ParetoFrontSize == 1 && parent1Index == -1){
						//todo: need to modify this
						parent1 = (GPIndividual)this.ParetoFrontFromLastGeneration[0].clone();
						parent2 = (GPIndividual)this.ParetoFrontFromLastGeneration[0].clone();
					}
					else{
						if(parent1Index == -1){
							System.out.println("Check here!");
							System.out.println("randomRateP1: " + randomRateP1);
							System.out.print("[");
							for(int i=0; i<ParetoFrontSize; i++){
						...
						double crowdingDistanceLeft = ((NSGA2MultiObjectiveFitness) this.ParetoFrontFromLastGeneration[parent1Index-1].fitness).sparsity;
						double crowdingDistanceRight = ((NSGA2MultiObjectiveFitness) this.ParetoFrontFromLastGeneration[parent1Index+1].fitness).sparsity;
						double proLeft = crowdingDistanceLeft/(crowdingDistanceLeft+crowdingDistanceRight);
						double randomRateP2 = state.random[0].nextDouble();
						if(randomRateP2 <= proLeft){
							parent2 = (GPIndividual)this.ParetoFrontFromLastGeneration[parent1Index-1].clone();
						}
						else{
							parent2 = (GPIndividual)this.ParetoFrontFromLastGeneration[parent1Index+1].clone();
					...
					//Step 1-2: do crossover based on the selected two parents
					if(this.clearedIndsIndex.size()==0){
						return;
					}
					Individual[] offspring = produceForCrossover(state, subpop, parent1, parent2);
					//todo: need to check the phenotype before replace the cleared one
					int replaceIndex = this.clearedIndsIndex.get(0);
					if(phenotypeSimilarityChecking(state,0,phenoCharacterisation,offspring[0],replaceIndex)){
						sortedPop[replaceIndex] = offspring[0];
						this.clearedIndsIndex.remove(0);
						if(this.clearedIndsIndex.size()==0){
							return;
						}
					}
					...
					if(phenotypeSimilarityChecking(state,0,phenoCharacterisation,offspring[1],replaceIndex)) {
						replaceIndex = this.clearedIndsIndex.get(0);
						sortedPop[replaceIndex] = offspring[1];
						this.clearedIndsIndex.remove(0);
						if(this.clearedIndsIndex.size()==0){
							return;
						}
					...
					//do mutation
					//Step 2-0: select a parent based on roulette selection
					GPIndividual parent = null;
					double randomRateP = state.random[0].nextDouble();
					for(int i=0; i<ParetoFrontSize-1; i++){
						if(randomRateP <= probability[i] && i==0){
							parent = (GPIndividual)this.ParetoFrontFromLastGeneration[i].clone();
							i = ParetoFrontSize; //break
						}
						else if(randomRateP > probability[i] && randomRateP <= probability[i+1]){
							parent = (GPIndividual)this.ParetoFrontFromLastGeneration[i+1].clone();
							i = ParetoFrontSize; //break
						}
					}
					if(ParetoFrontSize == 1 && parent == null){
						//todo: need to modify this
						parent = (GPIndividual)this.ParetoFrontFromLastGeneration[0].clone();
					}
					//Step 2-1: do mutation based on the selected parent
					if(this.clearedIndsIndex.size()==0){
						return;
					}
					Individual offspring = produceForMutation(state, subpop, parent);
					//todo: need to check the phenotype before replace the cleared one
					int replaceIndex = this.clearedIndsIndex.get(0);
					if(phenotypeSimilarityChecking(state,0,phenoCharacterisation,offspring,replaceIndex)) {
						sortedPop[replaceIndex] = offspring;
						this.clearedIndsIndex.remove(0);
						if (this.clearedIndsIndex.size() == 0) {
							return;
						}
	...
	public boolean phenotypeSimilarityChecking(final EvolutionState state,
											   double radius, PhenoCharacterisation[] pc,
		...
		for(int ind=0; ind<this.sortedPopCharLists.size(); ind++){
			if(this.sortedPopCharLists.get(ind) == null){
				continue;
			}
			double[] pcInd = this.sortedPopCharLists.get(ind);
			double distance = PhenoCharacterisation.distance(pcInd, combinePheChar);
			if (distance > radius) {
...
//		System.out.println("No same individuals!");
		this.sortedPopCharLists.set(replaceIndex,combinePheChar);
		return true;
	}
...
//			System.out.println("Print HERE!!!!!!");
		...
		NSGA2BreederNoEnvironmentalSelectionPhenotype breeder = (NSGA2BreederNoEnvironmentalSelectionPhenotype) (state.breeder);
