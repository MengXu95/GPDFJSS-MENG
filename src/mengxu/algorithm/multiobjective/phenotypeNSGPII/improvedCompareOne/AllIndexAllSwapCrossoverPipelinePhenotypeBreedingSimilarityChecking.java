 ...
 */
public class AllIndexAllSwapCrossoverPipelinePhenotypeBreedingSimilarityChecking extends CrossoverPipeline {
    /**
...
//            }
          ...
          //  System.out.println(parents[1].trees.length);
            ...
            if(((GPRuleEvolutionStateMO)state).useParentOffspringDistanceLimitation){
                nonduplicate_j1 = ((NSGA2EvaluatorNoEnvironmentalSelectionPhenotypeBreedingSimilarityChecking)state.evaluator).phenotypeSimilarityChecking(state,j1);
                similarWithParent_j1 = ((NSGA2EvaluatorNoEnvironmentalSelectionPhenotypeBreedingSimilarityChecking)state.evaluator).phenotypeSimilarityCheckingWithParents(state,parents,j1);
                if(nonduplicate_j1 && similarWithParent_j1){
                    inds[q] = j1;
                    q++;
                }
                else{
...
//            q++;
            ...
            if (q<n+start && !tossSecondParent)
                ...
                if(j2 == null){
                    ...
                    if(((GPRuleEvolutionStateMO)state).useParentOffspringDistanceLimitation){
                        nonduplicate_j2 = ((NSGA2EvaluatorNoEnvironmentalSelectionPhenotypeBreedingSimilarityChecking)state.evaluator).phenotypeSimilarityChecking(state,j2);
                        similarWithParent_j2 = ((NSGA2EvaluatorNoEnvironmentalSelectionPhenotypeBreedingSimilarityChecking)state.evaluator).phenotypeSimilarityCheckingWithParents(state,parents,j2);
                        if(nonduplicate_j2 && similarWithParent_j2){
                            inds[q] = j2;
                            q++;
                        }
                        else{
