...
public class NSGA2EvaluatorNoEnvironmentalSelectionPhenotypeBreedingSimilarityChecking extends NSGA2Evaluator {
...
//				System.out.println("Phenotype same!");
	...
	public boolean phenotypeSimilarityCheckingWithParents(final EvolutionState state,
														  Individual[] parents,
														  Individual individual){
		PhenoCharacterisation[] pc = ((GPRuleEvolutionStateMO)state).phenoCharacterisation;
		RuleType[] ruleTypes = {RuleType.SEQUENCING, RuleType.ROUTING}; //ruleType is an array
...
//		double[] combinePheChar = ArrayUtils.addAll(charListSeq, charListRout);
		...
		//calculate the PC of parents
		for(int ind=0; ind<parents.length; ind++){
			Individual parent_ind = parents[ind];
			int[] parent_ind_charListSeq;
			int[] parent_ind_charListRout;
			int parent_ind_treeID = 0;
			PhenoCharacterisation parent_ind_phenoCharacterisation = pc[parent_ind_treeID];//fzhang 2018.10.02  define two phenotype characteristic---phenoCharacterisation
			RuleType parent_ind_ruleType = ruleTypes[parent_ind_treeID];  //ruleType is a rule type---ruleType[0] = SEQUENCING  ruleType[1] = ROUTING
			parent_ind_charListSeq = parent_ind_phenoCharacterisation.characterise(  //characterise: calculate the distance
					new GPRule(parent_ind_ruleType, ((GPIndividual) parent_ind).trees[parent_ind_treeID]));
			parent_ind_treeID = 1;
			parent_ind_phenoCharacterisation = pc[parent_ind_treeID];
			parent_ind_ruleType = ruleTypes[parent_ind_treeID];
			parent_ind_charListRout = parent_ind_phenoCharacterisation.characterise(  //characterise: calculate the distance
					new GPRule(parent_ind_ruleType, ((GPIndividual) parent_ind).trees[parent_ind_treeID]));

			double[] parent_ind_combinePheChar = new double[parent_ind_charListSeq.length+parent_ind_charListRout.length];
//		double[] combinePheChar = ArrayUtils.addAll(charListSeq, charListRout);
			int parent_ind_i=0;
			for(parent_ind_i=0;parent_ind_i<charListSeq.length;parent_ind_i++){
				parent_ind_combinePheChar[parent_ind_i] = (double)parent_ind_charListSeq[parent_ind_i];
			}
			for(;parent_ind_i<parent_ind_charListSeq.length+parent_ind_charListRout.length;parent_ind_i++){
				parent_ind_combinePheChar[parent_ind_i] = (double)parent_ind_charListRout[parent_ind_i-parent_ind_charListSeq.length];
			}

			double distance = 0;
			if(((GPRuleEvolutionStateMO) state).useEuclideanDistance){
				distance = PhenoCharacterisation.distance(parent_ind_combinePheChar, combinePheChar);
			}
			else{
				distance = PhenoCharacterisation.distanceBinaryBring(parent_ind_combinePheChar, combinePheChar);
			}
			//todo: currently, the strategy is relaxted, that is if the offspring performs similar with only one parent then it is accepted, might
			//      change to must be similar with both parents for Crossover
//			System.out.println("distance: " + distance);
			if (distance > 0 && distance < ((GPRuleEvolutionStateMO)state).distanceForParentOffspringLimitation) {
				// Individual j is not in the niche
				return true;
			}
			else{
...
//        }
	...
	public void assignSparsity(Individual[] front) {
		int numObjectives = ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[0].fitness).getObjectives().length;

		for (int i = 0; i < front.length; i++)
			((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[i].fitness).sparsity = 0;
		...
		for (int i = 0; i < numObjectives; i++) {
			...
			ec.util.QuickSort.qsort(front, new SortComparator() {
				public boolean lt(Object a, Object b) {
					...
					return (((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) i1.fitness)
							.getObjective(o) < ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) i2.fitness).getObjective(o));
				}
			...
			final double min = ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[0].fitness).getObjective(o);
			final double max = ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[front.length - 1].fitness).getObjective(o);
...
//            System.out.println("normalised max: " + max );
			...
			((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[0].fitness).sparsity = Double.POSITIVE_INFINITY;
			((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[front.length - 1].fitness).sparsity = Double.POSITIVE_INFINITY;
			for (int j = 1; j < front.length - 1; j++) {
				NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking f_j = (NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) (front[j].fitness);
				NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking f_jplus1 = (NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) (front[j + 1].fitness);
				NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking f_jminus1 = (NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) (front[j - 1].fitness);
		...
		for (int i = 0; i < numObjectives; i++) {
			...
			ec.util.QuickSort.qsort(front, new SortComparator() {
				public boolean lt(Object a, Object b) {
					...
					Individual i2 = (Individual) b;
					return (((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) i1.fitness)
							.getNormalisedObjectives(o) < ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) i2.fitness).getNormalisedObjectives(o));
				}
			...
			//fzhang 2018.11.22 normize the objective to calculate the sparsity
			final double min = ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[0].fitness).getNormalisedObjectives(o);
			final double max = ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) front[front.length - 1].fitness).getNormalisedObjectives(o);
			//todo: this means the NSGPII still use the fitness normalisation 2023.02.14 by mengxu
			//todo: might modify this part
...
//            System.out.println("normalised max: " + max );
			...
			for (int j = 1; j < front.length - 1; j++) {
				...
				while(index_plus < front.length-1 && f_jplus1.getNormalisedObjectives(o) == f_j.getNormalisedObjectives(o)){
					...
					f_jplus1 = (NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) (front[index_plus].fitness);
				}
				...
				while(index_minus > 0 && f_jminus1.getNormalisedObjectives(o) == f_j.getNormalisedObjectives(o)){
					...
					f_jminus1 = (NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) (front[index_minus].fitness);
				}
...
//				}
//					f_j.sparsity += (f_jplus1.getNormalisedObjectives(o) - f_jminus1.getNormalisedObjectives(o)) * ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking)f_j).biasWeight[o];
	...
	public ArrayList assignFrontRanks(Subpopulation subpop) {
		...
		for (int rank = 0; rank < numRanks; rank++) {
			...
			for (int ind = 0; ind < numInds; ind++)
				((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) (((Individual) (front.get(ind))).fitness)).rank = rank;
		}
		return frontsByRank;
	}
...
//			System.out.println("Print HERE!!!!!!");
			...
			if (rank.length + newSubpopulation.size() >= originalPopSize[subpop]) {
				...
				ec.util.QuickSort.qsort(rank, new SortComparator() {
					public boolean lt(Object a, Object b) { //Returns true if a < b, else false
						Individual i1 = (Individual) a;
						Individual i2 = (Individual) b;
						return (((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) i1.fitness).sparsity > ((NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking) i2.fitness).sparsity);
					}
		...
		NSGA2BreederNoEnvironmentalSelectionPhenotypeBreedingSimilarityChecking breeder = (NSGA2BreederNoEnvironmentalSelectionPhenotypeBreedingSimilarityChecking) (state.breeder);
	...
	public void zScoreSigmoidNormalisation(EvolutionState state){
		...
		for(int i=0; i<individuals.length; i++){
			NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking fitnessNormalisation = (NSGA2MultiObjectiveFitnessNormalisationSimilarityChecking)state.population.subpops[0].individuals[i].fitness;
