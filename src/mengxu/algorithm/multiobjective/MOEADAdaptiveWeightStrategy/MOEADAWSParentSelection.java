...
public class MOEADAWSParentSelection extends TournamentSelection {
...
//				best = getRandomIndividual(start, subpopulation, state, thread);
		...
		if(state instanceof GPRuleEvolutionStateMOEADAWS){
			((GPRuleEvolutionStateMOEADAWS)state).parentIndex.add(best);
		}
		return best;
	}

	public int getRandomIndividual(int subproblem, int subpopulation, EvolutionState state, int thread) {
		MOEADAWSBreeder breeder = (MOEADAWSBreeder) state.breeder;
		MOEADAWSInitializer init = (MOEADAWSInitializer) state.initializer;
		if(breeder.getNeighborType() == MOEADAWSInitializer.NeighborType.NEIGHBOR){
