...
public class GPRuleEvolutionStateMOEADm2mcoverage extends GPRuleEvolutionState {
...
//		System.out.println("Generation " + generation + " bad run: " + badRun);
		...
		boolean useBiasBalance = ((MOEADm2mInitializercoverage)this.initializer).useBias;
		boolean normaliseBeforeBiasBalance = ((MOEADm2mInitializercoverage)this.initializer).normaliseBeforeBiasBalance;
		boolean useArchive = ((MOEADm2mInitializercoverage)this.initializer).useArchive;
...
//		}
	...
	public void printFrontforGen(boolean updateBiasBalance, boolean normalisation){
		boolean useArchive = ((MOEADm2mInitializercoverage)this.initializer).useArchive;
		boolean archiveNoPCDuplicated = ((MOEADm2mInitializercoverage)this.initializer).archiveNoPCDuplicated;
		boolean multiDiff = ((MOEADm2mInitializercoverage)this.initializer).multiDiff;
		...
		for(int s=0; s<this.population.subpops.length; s++){
			...
			if(normalisation){
				...
				double[] idealPoint = ((MOEADm2mInitializercoverage)this.initializer).idealPoint;
				double[] maxObjectives = ((MOEADm2mInitializercoverage)this.initializer).maxObjectives;
...
//							this.externalArchive.addIndividualAndPCToArchive((Individual) (sortedFront[i]), pc, true);
				...
				if(updateBiasBalance) {
					((MOEADm2mInitializercoverage) this.initializer).setBiasBalance(diffs);
				}
			}
			else{
