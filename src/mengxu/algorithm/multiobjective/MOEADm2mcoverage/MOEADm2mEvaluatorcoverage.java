...
public class MOEADm2mEvaluatorcoverage extends SimpleEvaluator{
    /**
    ...
    public void evaluatePopulation(final EvolutionState state)
    {
        ...
        if(((MOEADm2mInitializercoverage)state.initializer).normalisation && ((MOEADm2mInitializercoverage)state.initializer).manualRuleNormalisation){//add by mengxu 2022.10.02
            ...
            ((MOEADm2mInitializercoverage)state.initializer).curSchedulingSetObjectiveLowerBoundMtx = curSchedulingSet.getObjectiveLowerBoundMtx();
        }
        else if(((MOEADm2mInitializercoverage)state.initializer).normalisation && ((MOEADm2mInitializercoverage)state.initializer).bestObjectiveNormalisation){//add by mengxu 2022.10.06
            RealMatrix adaptLowerBoundMtx = new Array2DRowRealMatrix(((MOEADm2mInitializercoverage)state.initializer).numObjectives, 1);

            Individual[] inds = state.population.subpops[0].individuals;
            for(int j=0; j<((MOEADm2mInitializercoverage)state.initializer).numObjectives; j++){
            ...
            ((MOEADm2mInitializercoverage)state.initializer).curSchedulingSetObjectiveLowerBoundMtx = adaptLowerBoundMtx;
        }
        ...
        ((MOEADm2mInitializercoverage)state.initializer).updateNadirPoint(sortedParetoFront);

        if(((MOEADm2mInitializercoverage)state.initializer).useMapping){
//            ((MOEADmapInitializer)state.initializer).solutionToSubProblemMatching(state);//add by mengxu 2022.08.08
            ((MOEADm2mInitializercoverage)state.initializer).solutionToSubProblemMatchingBasedOnAngle(state);//add by mengxu 2022.08.08
        }


        //print ideal points and nadir points
        System.out.println("ideal points: [" + ((MOEADm2mInitializercoverage)state.initializer).idealPoint[0] + ", " + ((MOEADm2mInitializercoverage)state.initializer).idealPoint[1] + "]");

        System.out.println("nadir points: [" + ((MOEADm2mInitializercoverage)state.initializer).nadirPoint[0] + ", " + ((MOEADm2mInitializercoverage)state.initializer).nadirPoint[1] + "]");

        System.out.println("max objectives: [" + ((MOEADm2mInitializercoverage)state.initializer).maxObjectives[0] + ", " + ((MOEADm2mInitializercoverage)state.initializer).maxObjectives[1] + "]");
...
//    }
    ...
    public void allocationOfIndividualsToSubpopulations(EvolutionState state){
        ...
        for(int i=0; i<allIndividuals.size(); i++){
            Individual ind = allIndividuals.get(i);
            double minDis = Double.POSITIVE_INFINITY;
            int bestFitSubpop = -1;
            for(int s=0; s<numSubpop; s++) {//map the individuals with subpop
                double dis = disToSubpop(state, s, ind);
                if(dis < minDis){
                    minDis = dis;
                    bestFitSubpop = s;
                }
            ...
            if(bestFitSubpop != -1){
                allIndividualsSubpop.get(bestFitSubpop).add(ind);
            }
            else{
                remainingIndividuals.add(ind);
            }
        }
        ...
        for(int i=0; i<numSubpop; i++){
            ...
            if(individualsSubpop.size() >= originalPopSize[indexSubpop]){
                ...
                ArrayList<Individual> newSubpopulation = new ArrayList(); //a new one, size = 0
...
//                    newInds[p] = remainingIndividuals.get(randomIndex);
    ...
    public double disToSubpop(EvolutionState state, int subpopID, Individual individual){
        double[] problemWeights = ((MOEADm2mInitializercoverage)state.initializer).weights[subpopID];

        int numObjectives = ((MOEADm2mInitializercoverage)state.initializer).numObjectives;
        double[] idealPoint = ((MOEADm2mInitializercoverage)state.initializer).idealPoint;

        boolean normalisation = ((MOEADm2mInitializercoverage)state.initializer).normalisation;
        boolean manualRuleNormalisation = ((MOEADm2mInitializercoverage)state.initializer).manualRuleNormalisation;
        boolean bestObjectiveNormalisation = ((MOEADm2mInitializercoverage)state.initializer).bestObjectiveNormalisation;
        RealMatrix curSchedulingSetObjectiveLowerBoundMtx = ((MOEADm2mInitializercoverage)state.initializer).curSchedulingSetObjectiveLowerBoundMtx;
            ...
            for(int m=j+1; m<numObjectives; m++) {
                if (j != m) {
                    if(normalisation && manualRuleNormalisation){//add by mengxu 2022.10.06
                        ...
                        double weight_m = problemWeights[m];
                        double weight_j = problemWeights[j];

                        double value = (objective_j*objective_m + weight_j*weight_m)/(Math.sqrt(objective_j*objective_j + weight_j*weight_j) * Math.sqrt(objective_m*objective_m + weight_m*weight_m));
...
//                        if(angle >= 90){ //todo: need to check why the angle could be greater than 90??? Should not be I think! This is might because the normalisation method is not right!
//                            System.out.println("angle: " + angle);
//                        }
                        disToSubprom += angle; //test git push
                    }
                    else{
                        double objective_m = fit[m] - idealPoint[m];
                        double objective_j = fit[j] - idealPoint[j];
...
//            if(normalisation && manualRuleNormalisation){//add by mengxu 2022.10.06
//                double norm_objective_j = (fit[j] - curSchedulingSetObjectiveLowerBoundMtx.getEntry(j,0))/curSchedulingSetObjectiveLowerBoundMtx.getEntry(j,0);
//                double angle = Math.abs(Math.toDegrees(Math.atan(norm_objective_j/problemWeights[j])));
////                double angle = Math.abs(Math.toDegrees(Math.atan(norm_objective_j)) - Math.toDegrees(Math.atan(problemWeights[j])));
//                disToSubprom += angle;
//            }
//            else if(normalisation && bestObjectiveNormalisation){//add by mengxu 2022.10.06
//                // find from the CSDN 2022.10.28
//
//                // the following is defined by mengxu original
//                double norm_objective_j = (fit[j] - curSchedulingSetObjectiveLowerBoundMtx.getEntry(j,0))/curSchedulingSetObjectiveLowerBoundMtx.getEntry(j,0);
//                double angle = Math.abs(Math.toDegrees(Math.atan(norm_objective_j/problemWeights[j])));
////                double angle = Math.abs(Math.toDegrees(Math.atan(norm_objective_j)) - Math.toDegrees(Math.atan(problemWeights[j])));
//                disToSubprom += angle;
...
//    }
    ...
    public ArrayList assignFrontRanks(Subpopulation subpop) {
        ...
        for (int rank = 0; rank < numRanks; rank++) {
            ...
            for (int ind = 0; ind < numInds; ind++)
                ((MOEADm2mMultiObjectiveFitnesscoverage) (((Individual) (front.get(ind))).fitness)).rank = rank;
        }
        return frontsByRank;
    }
    ...
    public void assignSparsity(Individual[] front) {
        int numObjectives = ((MOEADm2mMultiObjectiveFitnesscoverage) front[0].fitness).getObjectives().length;
        ...
        for (int i = 0; i < front.length; i++) //front.length means how many individuals in this ranking(front)
            ((MOEADm2mMultiObjectiveFitnesscoverage) front[i].fitness).sparsity = 0; //the first individual in this front, the sparsity
        //is assigned to 0
        ...
        for (int i = 0; i < numObjectives; i++) {
            ...
            QuickSort.qsort(front, new SortComparator() {
                public boolean lt(Object a, Object b) { //less than
                    Individual i1 = (Individual) a;
                    Individual i2 = (Individual) b;
                    return (((MOEADm2mMultiObjectiveFitnesscoverage) i1.fitness)
                            .getObjective(o) < ((MOEADm2mMultiObjectiveFitnesscoverage) i2.fitness).getObjective(o));
                }
            ...
            ((MOEADm2mMultiObjectiveFitnesscoverage) front[0].fitness).sparsity = Double.POSITIVE_INFINITY;
            ((MOEADm2mMultiObjectiveFitnesscoverage) front[front.length - 1].fitness).sparsity = Double.POSITIVE_INFINITY;
            for (int j = 1; j < front.length - 1; j++) {
                MOEADm2mMultiObjectiveFitnesscoverage f_j = (MOEADm2mMultiObjectiveFitnesscoverage) (front[j].fitness);
                MOEADm2mMultiObjectiveFitnesscoverage f_jplus1 = (MOEADm2mMultiObjectiveFitnesscoverage) (front[j + 1].fitness);
                MOEADm2mMultiObjectiveFitnesscoverage f_jminus1 = (MOEADm2mMultiObjectiveFitnesscoverage) (front[j - 1].fitness);
