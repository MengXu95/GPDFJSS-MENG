...
public class MOEADm2mInitializercoverage extends GPInitializer {
...
//	public int[][][] neighbourhood; //modified by mengxu 2022.10.11
	public boolean tchebycheff;
	public int numObjectives;
	public int popSize;
	public int numSubpop; //added by mengxu 2022.10.11
...
//		weights = new double[popSize][numObjectives]; //original
		Parameter numSubPop = new Parameter("pop.subpops");
		numSubpop = popSize = state.parameters.getInt(numSubPop, null);; // modified by mengxu 2022.10.11
		weights = new double[numSubpop][numObjectives]; // modified by mengxu 2022.10.11
...
//		neighbourhood = new int[numSubpop][popSize][neighborSize];//hided by mengxu 2021.08.11
//		initializeNeighborhood(); //hided by mengxu 2021.08.11
//		identifyNeighbourWeights(); //original by fangfang

		if(normalisation && (manualRuleNormalisation || bestObjectiveNormalisation)){ //add by mengxu 2022.10.02
	 ...
	 * @param state
	 */
	public void solutionToSubProblemMatching(EvolutionState state){
		Population oldPop = (Population) state.population;
		Individual[] oldInds = oldPop.subpops[0].individuals;
		Individual[] oldIndsCopy = oldPop.subpops[0].individuals.clone();
...
//	2022.10.07 by mengxu to mapping individuals with subproblems based on angle
	public void solutionToSubProblemMatchingBasedOnAngle(EvolutionState state){
		Population oldPop = (Population) state.population;
		Individual[] oldInds = oldPop.subpops[0].individuals;
		Individual[] oldIndsCopy = oldPop.subpops[0].individuals.clone();
		List<Individual> allBadIndividuals =  new ArrayList<>();
...
//						dis = calculateWeightSumScore(oldIndsCopy[j], probId);
	...
	public int subProblemToSolutionMatching(EvolutionState state, Individual offspring){
		...
		double[] offspringObjective = ((MOEADm2mMultiObjectiveFitnesscoverage)offspring.fitness).objectives;
...
//		double[] x = new double[popSize]; //modified by mengxu
//		int[] idx = new int[popSize];
//
//		for (int s = 0; s < numSubpop; s++) {
//			for (int i = 0; i < popSize; i++) {
...
//
//				System.arraycopy(idx, 0, neighbourhood[s][i], 0, neighborSize);
//			}
//		}
//	}
...
//			idealPoint[i] = Double.POSITIVE_INFINITY; //modified by mengxu based on Jmetal
	...
	public void updateIdealPoint(EvolutionState state) {
		...
		for(Individual individual:individuals){
			...
			double[] objectives = ((MOEADm2mMultiObjectiveFitnesscoverage)individual.fitness).objectives;
...
//				System.out.println("update one idealPoint!");
	...
	private void initWeights() {
		for (int i = 0; i < popSize; i++) {
			//todo: it is weired! After the modification, it seems that the performance get worse !!!! 2022.10.14
			//todo: I think this might because the objective[0] is more hard to optimise and the original version
			//      focus more on optimising the objective[0]??????
			if (numObjectives == 2) {
				double[] weightVector = new double[2];
