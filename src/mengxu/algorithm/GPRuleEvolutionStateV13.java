 ...
 *
 */
public class GPRuleEvolutionStateV13 extends GPRuleEvolutionState {
...
//    double[][][] tempindsCharListsMultiTree;
    ...
    ArrayList<double[][]> tempfitnessesForModelAll = new ArrayList<>(); //modified by MengXu, which is used to store all the surrogate model for each generation
    ArrayList<double[][][]> tempindsCharListsMultiTreeAll = new ArrayList<>(); //modified by MengXu
    ArrayList<ArrayList<Double>> storeGenIndEachGenMinD = new ArrayList<>(); //modified by MengXu
...
//            double[][][] tempindsCharListsMultiTree;
            ...
            for(int subpop = 0; subpop < this.population.subpops.length; subpop++){
                ...
                fitnessesForModel[subpop] = fitnessesForModelSubpop;//so double[][][]可以是变长吗？？？
            }
            tempfitnessesForModelAll.add(fitnessesForModel);//modified by mengxu
            tempindsCharListsMultiTreeAll.add(indsCharListsMultiTree);//modified by mengxu
...
//            tempindsCharListsMultiTree = indsCharListsMultiTree;
        ...
        if(generation != 0){
            ...
            //2021.02.10 modified by mengxu, estimate the fitness of all the surrogate model and get the mean fitness as the fitness
            evaluatePoolIndividualAll(this,tempindsCharListsMultiTreeAll,tempfitnessesForModelAll);
//            evaluatePoolIndividual(this,tempindsCharListsMultiTree,tempfitnessesForModel);
            ...
            HashMap indsPopPoolMap = new HashMap();
            ...
            for(int ind = 0; ind < this.population.subpops[0].individuals.length; ind++){
                ...
                indsPopPoolMap.put(this.population.subpops[0].individuals[ind],0);
            }
            ...
            for(int ind = this.population.subpops[0].individuals.length; ind < indsPopPool.length; ind++){
                ...
                indsPopPoolMap.put(archiveInds.get(0).get(ind-this.population.subpops[0].individuals.length),1);
            }
            ...
            int numFromArchive = 0;
            for(int ind = 0; ind < this.population.subpops[0].individuals.length; ind++){
                this.population.subpops[0].individuals[ind] = indsPopPool[ind];
                numFromArchive += (int)indsPopPoolMap.get(indsPopPool[ind]);
            }
            output.message("The number of individuals from the archive: " + numFromArchive + "/" + archiveInds.get(0).size());
        }
        ...
        if (generation != numGenerations - 1) {
            for (int sub = 0; sub < this.population.subpops.length; sub++) {
                ...
                //2020.12.8 must get the archiveUpdateRate * this.population.subpops[sub].individuals.length number inds
/*				int numAddInd = 0;
...
//								OneDimArchiveIndsDeletedBase.add((selectedInsRank.get(i) + 1) / (OneDimArchiveIndsGens.get(i) + 1));//rank/generation
								...
								double normalisedRank = (selectedInsRank.get(i) - 0)/(49-0);
								double normalisedGeneraiton = (OneDimArchiveIndsGens.get(i) - 0) / (1022-0);
								OneDimArchiveIndsDeletedBase.add(normalisedRank + 1/(normalisedGeneraiton + 1));//rank/generation
							}
*//*							double[] deleteIndIndex = OneDimArchiveIndsDeletedBase.stream().mapToDouble(Double::doubleValue).toArray();
                ...
                while (numUpdatedInds < this.population.subpops[sub].individuals.length && updateNums < archiveUpdateRate * this.population.subpops[sub].individuals.length) {//must add 10 inds in each generation
                    if (OneDimArchiveInds.size() < archiveSize) {
                        ...
                        ArrayList<Double> OneDimArchiveIndsDeletedBase = new ArrayList<>();//calculate the data to decide which inds to delete
                        //delete one individual and insert a new individual
                        double minRank = Collections.min(selectedInsRank);
                        double maxRank = Collections.max(selectedInsRank);
...
//								OneDimArchiveIndsDeletedBase.add((selectedInsRank.get(i) + 1) / (OneDimArchiveIndsGens.get(i) + 1));//rank/generation
                            //2020.12.3 need to nornamise the rank and genration
                            double normalisedRank = 0.0;
                            if(maxRank - minRank == 0){//this means all the values are the same.
                                normalisedRank = selectedInsRank.get(i);
                            }
                            else{
                                normalisedRank = (selectedInsRank.get(i) - minRank)/(maxRank - minRank);
                            }
...
//							OneDimArchiveIndsDeletedBase.add(normalisedRank + 1/(normalisedGeneration + 1));//rank/generation
                            OneDimArchiveIndsDeletedBase.add(normalisedRank + 1/(normalisedGeneration + 1));//rank/generation //modified by meng, add a "()"
                        }
...
/*		OneDimArchiveIndsAddBase = new ArrayList<>();
    ...
    //2021.02.10 modified by mengxu, write the minD of KNN to file
    public void writeMinDToFile(){
        File indPoolMinDFile = new File("job." + jobSeed + ".indsInPoolMinD.csv"); //successedTransfer[i][j]: task j makes a successful transfer for task i.
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(indPoolMinDFile));
            writer.write("Generation,indIndex,eachGen,minDistance");
            writer.newLine();
            for (int ind = 0; ind < storeGenIndEachGenMinD.size(); ind++) {
//                for(int i = 0; i<storeGenIndEachGenMinD.get(ind).size(); i++){
//                    writer.write();
//                }

                writer.write(storeGenIndEachGenMinD.get(ind).get(0) + "," + storeGenIndEachGenMinD.get(ind).get(1) + "," + storeGenIndEachGenMinD.get(ind).get(2)+ "," + storeGenIndEachGenMinD.get(ind).get(3));
                writer.newLine();
            }
    ...
    //==========================================KNN========================================
    //2021.02.10 evaluate archive based on all the surrogate models saved. modified by mengxu,
    ...
    public void evaluatePoolIndividualAll(final EvolutionState state, ArrayList<double[][][]> indsCharListsMultiTreeAll, ArrayList<double[][]> fitnessesForModelAll) {
        ...
        for (int sub = 0; sub < state.population.subpops.length; sub++) {
            for (int i = 0; i < archiveInds.get(0).size(); i++) //
            {
                ...
                //KNN
                //===============================start==============================================
                for(int j=0;j<indsCharListsMultiTreeAll.size();j++){
                    ArrayList<Double> storeIndEachGenMinD = new ArrayList<>();
                    storeIndEachGenMinD.add((double)generation);
                    storeIndEachGenMinD.add((double)i);
                    storeIndEachGenMinD.add(OneDimArchiveIndsGens.get(i));

                    double[][][] indsCharListsMultiTree = indsCharListsMultiTreeAll.get(j);
                    double[][] fitnessesForModel = fitnessesForModelAll.get(j);
                    ...
                    meanFitness += fitnessesForModel[sub][index];
                    storeIndEachGenMinD.add(dMin);
                    storeGenIndEachGenMinD.add(storeIndEachGenMinD);
                }
                double finalFitness = meanFitness / indsCharListsMultiTreeAll.size();
                ((Clearable)individual.fitness).surrogateFitness(finalFitness);
