...
public class MultiReplicationMultiObjectiveFitness extends MultiObjectiveFitness {
    /** parameter for size of objectives */
    public static final String P_REPLICATION = "eval.problem.eval-model.sim-models.0.replications";
    ...
    public void setup(EvolutionState state, Parameter base)
    {
        ...
        int numIns;
        numInstance = state.parameters.getInt(new Parameter(P_REPLICATION), def.push(P_REPLICATION), 0);
        if (numInstance <= 0)
            state.output.fatal("The number of instances must be an integer >= 1.", base.push(P_REPLICATION), def.push(P_REPLICATION));
...
//        return s + FITNESS_POSTAMBLE;
    ...
    {
        MultiReplicationMultiObjectiveFitness other = (MultiReplicationMultiObjectiveFitness) _fitness;
...
//                    return true;
    ...
    public boolean betterThan(Fitness fitness)
    {
        ...
        switch (objective) {
            ...
            case MEAN_TARDINESS:
            case MEAN_WEIGHTED_TARDINESS:
                return compareInstanceV2mean((MultiReplicationMultiObjectiveFitness)fitness);
            case MAX_FLOWTIME:
            case MAX_WEIGHTED_FLOWTIME:
            case MAX_TARDINESS:
            case MAX_WEIGHTED_TARDINESS:
                return compareInstanceV2max((MultiReplicationMultiObjectiveFitness)fitness);
        }
        System.out.println("Error, MultiInstanceMultiObjectiveFitness.betterThan()");
    ...
    public boolean betterThanBasedOnBinaryBring(Fitness fitness){
        double[][] otherBinaryBring = ((MultiReplicationMultiObjectiveFitness)fitness).getMultiInstanceMultiObjectiveBinaryBring();
        double advantage0 = 0;
        double advantage1 = 0;
    ...
    public boolean betterThanBasedOnNormalisation(Fitness fitness){
        double[][] otherNormalisation = ((MultiReplicationMultiObjectiveFitness)fitness).getMultiInstanceMultiObjectiveNormalisation();
        double advantage0 = 0;
        double advantage1 = 0;
    ...
    public boolean compareInstanceV1(MultiReplicationMultiObjectiveFitness other)
    {
    ...
    public boolean compareInstanceV2mean(MultiReplicationMultiObjectiveFitness other)
    {
        boolean abeatsb = false;
    ...
    public boolean compareInstanceV2max(MultiReplicationMultiObjectiveFitness other)
    {
        boolean abeatsb = false;
