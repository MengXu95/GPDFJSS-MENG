...
public class EpsilonTTGenNoRepeatCandidateLexicaseSelectionMultiReplication extends SelectionMethod implements SteadyStateBSourceForm {

    /** default base */
    public static final String P_LEXICASE = "lexicase";

    public static final String P_NUMINSTANCE = "eval.problem.eval-model.sim-models.0.replications";
...
//            state.output.fatal("The number of cases must be an integer >= 1.", base.push(P_NUMCASE), def.push(P_NUMCASE));
        ...
        this.numInstance = state.parameters.getInt(new Parameter(P_NUMINSTANCE), null, 1);
...
//        System.out.println();
    ...
    public int produce(int subpopulation, EvolutionState state, int thread) { //epsilon lexicase selection
        if(state.generation>((GPRuleEvolutionStateMultiReplication)state).genForUseLS){
...
//                ((GPRuleEvolutionStateMV0)state).parentIndex.add(index);
                ((GPRuleEvolutionStateMultiReplication)state).parentFitness.add(indexFitness);//add 2021.12.09
            }
            return index;
        }
        else{
...
//                System.out.println("epsilon selection index: " + allIndividualsIndex.get(0));
            ...
            //add 2021.11.16-------------------------------
            if(state instanceof GPRuleEvolutionStateMultiReplication){
                double[] caseNumber = ((GPRuleEvolutionStateMultiReplication)state).allGenerationCaseNumber.get(state.generation-((GPRuleEvolutionStateMultiReplication)state).genForUseLS-1);
                caseNumber[instanceIndex]++;
                ((GPRuleEvolutionStateMultiReplication)state).allGenerationCaseNumber.set(state.generation-((GPRuleEvolutionStateMultiReplication)state).genForUseLS-1, caseNumber);
            }
            //---------------------------------------------

            int indexBest = PopulationUtils.getIndexOfbestIndsInstance(allIndividuals,instanceIndex);
            double bestFitness = ((MultiReplicationMultiObjectiveFitness)allIndividuals.get(indexBest).fitness).getFitness(instanceIndex,0);
...
//                    System.out.println("epsilon selection index: " + allIndividualsIndex.get(0));
                ...
                if(((MultiReplicationMultiObjectiveFitness)allIndividuals.get(j).fitness).getFitness(instanceIndex,0)>bestFitness + epsilon){
...
//        System.out.println("epsilon selection index by random: " + allIndividualsIndex.get(index));
    ...
    public double calculateEpsilon(List<Individual> allIndividuals, int i){
        ...
        for(int ref=0; ref<allIndividuals.size(); ref++){
            double fit = ((MultiReplicationMultiObjectiveFitness)allIndividuals.get(ref).fitness).getFitness(i,0);
...
//        }
//        System.out.println();
        //todo:error, some times allFitness.size could be 0,which means no elements in allFitness, then get will be wrong
...
//        System.out.println("tournament size for ELSMO:" + s);
        ...
        for (int x=0;x<permSize;x++)
        {
            ...
            double[][] multiCaseFitness = ((MultiReplicationMultiObjectiveFitness)ind.fitness).multiInstanceMultiObjectiveFitness;
