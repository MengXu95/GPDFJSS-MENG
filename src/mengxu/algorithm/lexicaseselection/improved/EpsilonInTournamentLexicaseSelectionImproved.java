...
public class EpsilonInTournamentLexicaseSelectionImproved extends SelectionMethod implements SteadyStateBSourceForm {
...
//    }
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        numIns = state.parameters.getInt(new Parameter(P_NUMCASE), def.push(P_NUMCASE), 0)+1;
        if (numIns <= 0)
...
//                System.out.println("epsilon selection index: " + allIndividualsIndex.get(0));
                if(state instanceof GPRuleEvolutionStateImproved){
                    ((GPRuleEvolutionStateImproved)state).parentIndex.add(allIndividualsIndex.get(0));
                }
            ...
            double bestFitness = ((MultiInstanceMultiObjectiveFitnessImproved)allIndividuals.get(indexBest).fitness).getFitness(instanceIndex,0);
...
//                    System.out.println("epsilon selection index: " + allIndividualsIndex.get(0));
                ...
                if(((MultiInstanceMultiObjectiveFitnessImproved)allIndividuals.get(j).fitness).getFitness(instanceIndex,0)>bestFitness + epsilon){
...
//        System.out.println("epsilon selection index by random: " + allIndividualsIndex.get(index));
        if(state instanceof GPRuleEvolutionStateImproved){
            ((GPRuleEvolutionStateImproved)state).parentIndex.add(allIndividualsIndex.get(index));
        }
...
//    @Override
//    public int produce(int subpopulation, EvolutionState state, int thread) { //epsilon lexicase selection
//        List<double[][]> oldindsNormalisation = ((GPRuleEvolutionStateMV0)state).getNormalisationFitness();
...
//            int instanceIndex = randomInstanceIndex.get(i);
//
//            int indexBest = PopulationUtils.getIndexOfbestIndsInstanceNormalisation(allFitnessNormalisation,instanceIndex);
//            double bestFitness = allFitnessNormalisation.get(indexBest)[instanceIndex][0];
//
//            //calculate the epsilon------------------------
//            double epsilon = calculateEpsilonNormalisation(allFitnessNormalisation,instanceIndex);
//
//
//            int j=0;
...
//                    return allIndividualsIndex.get(0);
//                }
//                if(allFitnessNormalisation.get(j)[instanceIndex][0]>bestFitness + epsilon){
//                    allIndividualsIndex.remove(j);
...
//        System.out.println("epsilon selection index: " + allIndividualsIndex.get(index));
    ...
    public double calculateEpsilon(List<Individual> allIndividuals, int i){
        ...
        for(int ref=0; ref<allIndividuals.size(); ref++){
            double fit = ((MultiInstanceMultiObjectiveFitnessImproved)allIndividuals.get(ref).fitness).getFitness(i,0);
            allFitness.add(fit);
        }
...
//                    best = j;
            ...
            for (int x=0;x<s;x++)
            {
                ...
                double[][] multiCaseFitness = ((MultiInstanceMultiObjectiveFitnessImproved)ind.fitness).multiInstanceMultiObjectiveFitness;
