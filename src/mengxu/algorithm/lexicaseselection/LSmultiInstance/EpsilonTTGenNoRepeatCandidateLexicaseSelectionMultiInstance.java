...
public class EpsilonTTGenNoRepeatCandidateLexicaseSelectionMultiInstance extends SelectionMethod implements SteadyStateBSourceForm {
...
//        System.out.println();
    ...
    public int produce(int subpopulation, EvolutionState state, int thread) { //epsilon lexicase selection
        if(state.generation>((GPRuleEvolutionStateMultiInstance)state).genForUseLS){
...
//                ((GPRuleEvolutionStateMV0)state).parentIndex.add(index);
                ((GPRuleEvolutionStateMultiInstance)state).parentFitness.add(indexFitness);//add 2021.12.09
            }
            return index;
        }
        else{
...
//                System.out.println("epsilon selection index: " + allIndividualsIndex.get(0));
            ...
            //add 2021.11.16-------------------------------
            if(state instanceof GPRuleEvolutionStateMultiInstance){
                double[] caseNumber = ((GPRuleEvolutionStateMultiInstance)state).allGenerationCaseNumber.get(state.generation-((GPRuleEvolutionStateMultiInstance)state).genForUseLS-1);
                caseNumber[instanceIndex]++;
                ((GPRuleEvolutionStateMultiInstance)state).allGenerationCaseNumber.set(state.generation-((GPRuleEvolutionStateMultiInstance)state).genForUseLS-1, caseNumber);
            }
            //---------------------------------------------

            int indexBest = PopulationUtils.getIndexOfbestIndsInstance(allIndividuals,instanceIndex);
            double bestFitness = ((MultiInstanceMultiObjectiveFitness)allIndividuals.get(indexBest).fitness).getFitness(instanceIndex,0);
...
//                    System.out.println("epsilon selection index: " + allIndividualsIndex.get(0));
                ...
                if(((MultiInstanceMultiObjectiveFitness)allIndividuals.get(j).fitness).getFitness(instanceIndex,0)>bestFitness + epsilon){
...
//        System.out.println("epsilon selection index by random: " + allIndividualsIndex.get(index));
    ...
    public double calculateEpsilon(List<Individual> allIndividuals, int i){
        ...
        for(int ref=0; ref<allIndividuals.size(); ref++){
            double fit = ((MultiInstanceMultiObjectiveFitness)allIndividuals.get(ref).fitness).getFitness(i,0);
