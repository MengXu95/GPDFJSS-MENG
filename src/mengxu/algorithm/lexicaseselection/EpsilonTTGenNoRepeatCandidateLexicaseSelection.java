...
public class EpsilonTTGenNoRepeatCandidateLexicaseSelection extends SelectionMethod implements SteadyStateBSourceForm {
...
//        System.out.println();
    ...
    public int produce(int subpopulation, EvolutionState state, int thread) { //epsilon lexicase selection
        if(state.generation>((GPRuleEvolutionStateOneInstanceMultiCase)state).genForUseLS){
...
//                ((GPRuleEvolutionStateMV0)state).parentIndex.add(index);
                ((GPRuleEvolutionStateOneInstanceMultiCase)state).parentFitness.add(indexFitness);//add 2021.12.09
            }
            return index;
        }
        else{
...
//                System.out.println("epsilon selection index: " + allIndividualsIndex.get(0));
            ...
            //add 2021.11.16-------------------------------
            if(state instanceof GPRuleEvolutionStateOneInstanceMultiCase){
                double[] caseNumber = ((GPRuleEvolutionStateOneInstanceMultiCase)state).allGenerationCaseNumber.get(state.generation-((GPRuleEvolutionStateOneInstanceMultiCase)state).genForUseLS-1);
                caseNumber[instanceIndex]++;
                ((GPRuleEvolutionStateOneInstanceMultiCase)state).allGenerationCaseNumber.set(state.generation-((GPRuleEvolutionStateOneInstanceMultiCase)state).genForUseLS-1, caseNumber);
            }
            //---------------------------------------------
