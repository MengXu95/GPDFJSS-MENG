...
public class GPRuleEvolutionStateM3GP extends GPRuleEvolutionState {
...
//		storeGenDiversities.add(diversities);
		...
		int badRun = 0;
		List<EnsembleRule> ensembles = ((EnsembleEvaluatorM3GP)this.evaluator).ensemblePool;
...
//			writeEnsembleToFile();//modified by mengxu 2021.05.08
		...
		((EnsembleEvaluatorM3GP)this.evaluator).ensembleOffspringPool.clear();
		//save the top 10 elites, add by mengxu 2023.07.31
		List<EnsembleRule> oldEnsemblePopulation = ((EnsembleEvaluatorM3GP)this.evaluator).ensemblePool;
		EnsembleRule[] oldEnsemblePopulationSorted = ((EnsembleEvaluatorM3GP)this.evaluator).sortEnsembles(oldEnsemblePopulation);
		for(int i=0; i<10; i++){
			((EnsembleEvaluatorM3GP)this.evaluator).ensembleOffspringPool.add(oldEnsemblePopulationSorted[i]);
		}
		...
		((EnsembleEvaluatorM3GP)this.evaluator).ensemblePool.clear();
		for(int i=0; i<((EnsembleEvaluatorM3GP)this.evaluator).ensembleOffspringPool.size(); i++){
			((EnsembleEvaluatorM3GP)this.evaluator).ensemblePool.add(((EnsembleEvaluatorM3GP)this.evaluator).ensembleOffspringPool.get(i).clone());
		}
