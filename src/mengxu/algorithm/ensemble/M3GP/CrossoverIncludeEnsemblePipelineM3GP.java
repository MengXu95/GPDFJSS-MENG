 ...
 */
public class CrossoverIncludeEnsemblePipelineM3GP extends AllIndexAllSwapCrossoverPipeline {
    /**
...
//                }
...

    {
        ...
        EnsembleRule[] parentEnsemblesForCrossover = ((EnsembleEvaluatorM3GP)state.evaluator).parentsForCrossover;
        ...
        ((EnsembleEvaluatorM3GP)state.evaluator).ensembleOffspringPool.add(offspringEnsemble1);

        //todo: the following need to be modified

        // add the individuals to the population

        if(state instanceof GPRuleEvolutionStateM3GP){//this is for lexicase selection modified by mengxu 2022.02.28
            int index = ((GPRuleEvolutionStateM3GP) state).parentsIndexPairForCrossover.size()-1;
            ((GPRuleEvolutionStateM3GP) state).parentsIndexPairForCrossover.get(index).setChild1ID(q);
        }
...
//        q++;
        ...
        {
            ((EnsembleEvaluatorM3GP)state.evaluator).ensembleOffspringPool.add(offspringEnsemble2);
            if(state instanceof GPRuleEvolutionStateM3GP){//this is for lexicase selection modified by mengxu 2022.02.28
                int index = ((GPRuleEvolutionStateM3GP) state).parentsIndexPairForCrossover.size()-1;
                ((GPRuleEvolutionStateM3GP) state).parentsIndexPairForCrossover.get(index).setChild2ID(q);
            }
