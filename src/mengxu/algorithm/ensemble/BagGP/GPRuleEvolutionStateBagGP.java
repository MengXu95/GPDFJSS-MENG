package mengxu.algorithm.ensemble.BagGP;
...
public class GPRuleEvolutionStateBagGP extends GPRuleEvolutionState {
	...
	public Individual finalIndividual;

	public int cycles;
	public int cycleLength;

	public List<Individual> bagEnsemble = new ArrayList<>();
	...
	public void setup(EvolutionState state, Parameter base) {
		...
		cycles = state.parameters.getInt(new Parameter("cycles"), null, 1);
		cycleLength = state.parameters.getInt(new Parameter("cycle-length"), null, 1);
...
//		evaluator.evaluatePopulation(this);
		//todo: need to double check whether this is right
		if(this.generation != 0 && this.generation%this.cycleLength == 0){
			evaluator.evaluatePopulation(this);  //// here, after this we evaluate the population
			//after each cycle length, save the best individual into the ensemble
			Individual[] individuals = this.population.subpops[0].individuals;
			PopulationUtils.sort(individuals);
			bagEnsemble.add((Individual)individuals[0].clone());
			//need to re-initialise the whole population
			//todo: need to double check whether the re-initialised population is totally the same with the original initialised population!!! 2023.03.11
//			individuals[0].printIndividualForHumans(this,0);
			this.population = this.initializer.initialPopulation(this,0);
			Individual[] newIndividuals = this.population.subpops[0].individuals;
//			newIndividuals[0].printIndividualForHumans(this,0);
...
//		diversities.add(pcDvalue);
	    ...
	    if (generation == numGenerations-1)
	        {
				//added by mengxu 2023.03.10
				individuals = this.population.subpops[0].individuals;
				PopulationUtils.sort(individuals);
				this.finalIndividual = individuals[0];
