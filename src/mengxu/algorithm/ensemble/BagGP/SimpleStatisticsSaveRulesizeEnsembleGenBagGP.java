...
public class SimpleStatisticsSaveRulesizeEnsembleGenBagGP extends SimpleStatisticsSaveRulesizeGen {
...
//			}
			...
			if (doGeneration && state.generation == state.numGenerations-1){//added by mengxu for bagGP output
				...
				List<Individual> bagEnsemble = ((GPRuleEvolutionStateBagGP)state).bagEnsemble;

				for(int e=0; e<bagEnsemble.size(); e++){
					bagEnsemble.get(e).printIndividualForHumans(state,statisticslog);
				}
			}
