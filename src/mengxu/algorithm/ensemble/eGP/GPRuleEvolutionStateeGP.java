...
import mengxu.algorithm.diversitymeasure.ParentIndexDiversity;
import mengxu.algorithm.diversitymeasure.ParentsCombinationForXcoverDiversity;
...
public class GPRuleEvolutionStateeGP extends GPRuleEvolutionState {
	...
	public List<Double> averageEnsembleSize = new ArrayList<>();

	public boolean breedIndividualPopulation;
	...
	public void setup(EvolutionState state, Parameter base) {
		...
		this.breedIndividualPopulation = true;
...
//		storeGenDiversities.add(diversities);
		...
		List<EnsembleRule> ensembles = ((EnsembleEvaluatoreGP)this.evaluator).ensemblePool;
		double averageEnsembleSizeGen = 0;
		for(int i=0; i < ensembles.size(); i++){
			EnsembleRule ensembleRule = ensembles.get(i);
			averageEnsembleSizeGen += ensembleRule.getEnsemble().size();
			if(ensembleRule.getFitness().fitness() >= Double.POSITIVE_INFINITY ||
					ensembleRule.getFitness().fitness() >= Double.MAX_VALUE){
				badRun ++;
			}
		}
		System.out.println("Generation " + generation + " bad run: " + badRun);
		averageEnsembleSizeGen = averageEnsembleSizeGen/individuals.length;
		this.averageEnsembleSize.add(averageEnsembleSizeGen);
		System.out.println("Average ensemble size:  "  + averageEnsembleSizeGen);
...
//			writeSelectParentIndexToFile();
			...
			writeAverageEnsembleSizeToFile();
...
//			writeEnsembleToFile();//modified by mengxu 2021.05.08
		...
		((EnsembleEvaluatoreGP)this.evaluator).ensembleOffspringPool.clear();
		//save the top 2 elites, add by mengxu 2023.07.31
		List<EnsembleRule> oldEnsemblePopulation = ((EnsembleEvaluatoreGP)this.evaluator).ensemblePool;
		EnsembleRule[] oldEnsemblePopulationSorted = ((EnsembleEvaluatoreGP)this.evaluator).sortEnsembles(oldEnsemblePopulation);
		for(int i=0; i<2; i++){
			((EnsembleEvaluatoreGP)this.evaluator).ensembleOffspringPool.add(oldEnsemblePopulationSorted[i]);
		}
		...
		//add by mengxu 2023.07.31 to clone
		((EnsembleEvaluatoreGP)this.evaluator).ensemblePool.clear();
		for(int i=0; i<((EnsembleEvaluatoreGP)this.evaluator).ensembleOffspringPool.size(); i++){
			((EnsembleEvaluatoreGP)this.evaluator).ensemblePool.add(((EnsembleEvaluatoreGP)this.evaluator).ensembleOffspringPool.get(i).clone());
		}
...
//					writer.newLine();
	...
	//2023.07.31 modified by mengxu
	public void writeAverageEnsembleSizeToFile(){
		File diversities = new File("job." + jobSeed + ".averageEnsembleSize.csv"); //successedTransfer[i][j]: task j makes a successful transfer for task i.
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(diversities));
			writer.write("Gen, Ensemble Size");
			writer.newLine();
			for (int ind = 0; ind < this.averageEnsembleSize.size(); ind++) {
				writer.write(ind + "," + this.averageEnsembleSize.get(ind));
				writer.newLine();
			}
