...
public class MutationIncludeEnsemblePipelineeGP extends MutationPipeline
    {
...
//                ((GPRuleEvolutionStateeGP) state).parentIndexForMutation.add(parentForMutation);
//            }
...
//            EnsembleRule parentEnsemblesForMutation = ((EnsembleEvaluator)state.evaluator).parentsForMutation;
            ...
            EnsembleRule ensembleParent = ((EnsembleEvaluatoreGP)state.evaluator).parentsForMutation;
...
//            int indexPointOfParent = state.random[thread].nextInt(lengthParent);
            ...
            ((EnsembleEvaluatoreGP)state.evaluator).ensembleOffspringPool.add(offspringEnsemble);
...
//            EnsembleRule parentEnsemblesForMutation = ((EnsembleEvaluator)state.evaluator).parentsForMutation;
            ...
            //original same as M3GP
//            Species species = ensembleParentList.get(0).species;
//            Individual newIndividual = species.newIndividual(state, 0);

            //select from individual population
            int indexInd = state.random[0].nextInt(state.population.subpops[0].individuals.length);
            Individual newIndividual = (Individual)state.population.subpops[0].individuals[indexInd].clone();
