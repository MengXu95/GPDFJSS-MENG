...
public class SimpleBreedereGP extends SimpleBreeder
    {

    public Population breedPopulation(EvolutionState state)
    {//implement by mengxu 2023.07.31
        //Step 1: breed the individual population
        ((GPRuleEvolutionStateeGP)state).breedIndividualPopulation = true;
        Population newpop = breedIndividualPopulation(state);
        //Step 2: breed the ensemble population
        ((GPRuleEvolutionStateeGP)state).breedIndividualPopulation = false;
        breedEnsemblePopulation(state);
        return newpop;
    }
    ...
    public Population breedIndividualPopulation(EvolutionState state)
    {
        ...
        a subpopulation. */
            ...
            else
            {
                ...
                for(int y=0;y<numThreads;y++)
                {
                    SimpleBreedereGPThread r = new SimpleBreedereGPThread();
                    r.threadnum = y;
                    r.newpop = newpop;
                    r.numinds = numinds[y];
                    r.from = from[y];
    ...
    /** A simple breeder that doesn't attempt to do any cross-
        population breeding.  Basically it applies pipelines,
        one per thread, to various subchunks of a new population. */
    public void breedEnsemblePopulation(EvolutionState state)
        {
...
class SimpleBreedereGPThread implements Runnable
    {
    Population newpop;
    public int[] numinds;
    public int[] from;
    public SimpleBreedereGP me;
