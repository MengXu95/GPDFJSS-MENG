...
public class AllIndexAllSwapCrossoverPipelineWithBR extends CrossoverPipeline {
    ...
    public void setup(final EvolutionState state, final Parameter base)
    {
        ...
        distanceBB = state.parameters.getBoolean(new Parameter(P_DISTANCE_BB), null, true);
    }
...
//                        if(selectedGPNodePairs.size() == numNodes){//modified by mengxu 2022.03.07
//                            break;
//                        }
                    }
                }
...
//                offsprings[i*2+1] = j2;
            ...
            evaluateOffspring(state, ((GPRuleEvolutionStateBaseline)state).indsCharListsMultiTreeGen, ((GPRuleEvolutionStateBaseline)state).fitnessesForModelGen, offsprings);
...
//                    double d  = PhenoCharacterisation.distanceBinaryBring(pcIntermediate, pcModel);
                ...
                fit[0] = fitnessesForModel[sub][index];
                ((MultiObjectiveFitness)individual.fitness).setObjectives(state,fit);
...
/*			Individual[] inds = PopulationUtils.sortInds(individuals);
    ...
    public static void sortOffspring(Individual[] ind)
    {
        ...
        {
            if(((MultiObjectiveFitness)o1.fitness).objectives[0] < ((MultiObjectiveFitness)o2.fitness).objectives[0])
                return -1;
            if(((MultiObjectiveFitness)o1.fitness).objectives[0] == ((MultiObjectiveFitness)o2.fitness).objectives[0])
                return 0;
