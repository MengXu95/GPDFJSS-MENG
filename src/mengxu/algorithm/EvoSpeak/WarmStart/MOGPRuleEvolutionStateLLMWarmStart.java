...
public class MOGPRuleEvolutionStateLLMWarmStart extends GPRuleEvolutionState {
	...
	public boolean manualNormalization;
	public int numObjective;
	public double[] weights;
	public double[] baselineObjectives;
	...
	public void setup(EvolutionState state, Parameter base) {
		...
		this.numObjective = state.parameters.getInt(new Parameter("eval.problem.eval-model.objectives"), null, 1);
		this.manualNormalization = state.parameters.getBoolean(new Parameter("manual-normalization"), null, true);
		this.baselineObjectives = new double[numObjective];
		this.weights = new double[numObjective];
		double weight0 = state.parameters.getDouble(new Parameter("weight-objective0"), null, 0);
		this.weights[0] = weight0;
		this.weights[1] = 1-weight0; //only suitable for 2 objectives
	}
	...
	public int evolve() {
		...
		if(manualNormalization){
			...
			for(int i=0; i<this.baselineObjectives.length; i++){
				this.baselineObjectives[i] = curSchedulingSet.getObjectiveLowerBoundMtx().getEntry(i,0);
			}
			Individual[] individuals = this.population.subpops[0].individuals;
			for(int i=0; i<individuals.length; i++){
				MultiObjectiveFitnessLLM fitnessLLM = (MultiObjectiveFitnessLLM)individuals[i].fitness;
				fitnessLLM.setWeightsAndBaseObjectives(weights, baselineObjectives, manualNormalization);
			}
		}
		if(generation == 0){
...
//					writer.newLine();
	...
	public void writeAllIndividualsToFile(){
		File popFitness = new File("job." + jobSeed + ".gen_" + generation + ".AllIndividuals_n.csv"); //successedTransfer[i][j]: task j makes a successful transfer for task i.
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(popFitness));
			writer.write("Ind, Fitness0, Fitness1, WeightedFitness");
			writer.newLine();
			writer.write("-1 ," + this.baselineObjectives[0] + "," + this.baselineObjectives[1]+ ", 1");
			writer.newLine();
			Individual[] individuals = this.population.subpops[0].individuals;
			for (int ind = 0; ind < individuals.length; ind++) {
				double[] objectives = ((MultiObjectiveFitness)individuals[ind].fitness).objectives;
				writer.write(ind + "," + objectives[0] + "," + objectives[1] + "," + individuals[ind].fitness.fitness());
				writer.newLine();
			}
