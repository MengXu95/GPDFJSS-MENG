...
public class MutationPipelineMcts extends MutationPipeline
    {
    ...
    public Object clone()
        {
        MutationPipelineMcts c = (MutationPipelineMcts)(super.clone());
    ...
    public int produce(final int min,
        ...
        for(int q=start; q < n+start; q++) //q=start = 0  q<1
        {
            ...
            //modified by mengxu 2021.07.15 for CCGP
            int indexMCTS;
            if(i.trees.length > 1){
                //MTGP
                indexMCTS = t;
            ...
            for (int x = 0; x < numTries; x++) {
                ...
                //todo: need to modify this to select a "poor" node
                // to replace the original one (random select). 2021.11.11


                GPNode p1Copy = (GPNode) p1.clone();// do not know if this will make a mistake mengxu

//                    GPNodeParent cparent = this;
//                    while(cparent!=null && cparent instanceof GPNode)
//                        cparent = ((GPNode)(cparent)).parent;
//                    return cparent;
                //modified by mengxu. 2021.04.16 to use based on the MCTS
//                //todo: need to find the mctsTreeNode

                List<GPNode> linkedGpNode = new ArrayList<>();
                while (p1Copy.parent != null && p1Copy.parent instanceof GPNode) {
                    linkedGpNode.add((GPNode) (p1Copy).parent);
                    p1Copy = (GPNode) (p1Copy).parent;
                }
                MctsTreeNode mctsTreeNode;
                if (linkedGpNode.size() != 0) {
                    int ref = linkedGpNode.size() - 1;
                    mctsTreeNode = state.mctses[indexMCTS].findCorrespondingMctsTreeNode(linkedGpNode.get(ref));
                    ref--;
                    while (ref >= 0) {
                        mctsTreeNode = mctsTreeNode.findCorrespondingMctsTreeNode(linkedGpNode.get(ref));
                        ref--;
                    }
                } else {
                    mctsTreeNode = state.mctses[indexMCTS].getMctsRootNode();
                }
                ...
                //modified by mengxu. 2021.04.08 related to Mcts
                p2 = builder.newRootedTreeMctsMutation(state,
...
//                    p2 = builder.newRootedTreeMcts(state,
//                            p1.parentType(initializer),
//                            thread,
//                            p1.parent,
//                            i.trees[t].constraints(initializer).functionset,
