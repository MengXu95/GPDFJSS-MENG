...
import mengxu.algorithm.multicaseEnsemble.benchmark.DivNichGP.GPRuleEvolutionStateDivNich;
...
public class GPRuleEvolutionState extends SimpleEvolutionState {
	...
	//2019.12.6 check the total time
	public static double totalTime = 0;

	public final static String P_EXPECT_TIME = "expect-training-time";
	public static double expectTrainingTime;
	public List<Double> sumGenTimes = new ArrayList<>();
...
/*    public void setWeights(double[] weights, int subPopNum) {
	...
	public void initTerminalSet() {
        ...
        for (int subPopNum = 0; subPopNum < numSubPops; subPopNum++) {
         //for (int subPopNum = 0; subPopNum < numSubPops; ++subPopNum) {
			...
			else if (terminalFrom.equals("relative_with_preference")) {// add by mengxu 2024.3.11 for Pareto set learning
				//that is to say, put the terminals we defined into terminals[]
				initRelativeWithPreferenceTerminalSet(subPopNum);
			}
			else if (terminalFrom.equals("relative_for_multitask")) {// add by mengxu 2024.3.11 for Pareto set learning
				//that is to say, put the terminals we defined into terminals[]
				initRelativeForMultitaskTerminalSet(subPopNum);
			}
	...
	public void initRelativeWithPreferenceTerminalSet(int subPopNum) {
		LinkedList<GPNode> terminals = new LinkedList<GPNode>();
		for (JobShopAttribute a : JobShopAttribute.relativeWithPreferenceAttributes()) {
			GPNode attribute = new AttributeGPNode(a);
			attribute.Using_PSL = true;
			terminals.add(attribute); //read all the terminals we defined to keep them into terminals[]
		}
		this.terminals[subPopNum] = terminals.toArray(new GPNode[0]);
	}

	public void initRelativeForMultitaskTerminalSet(int subPopNum) {
		LinkedList<GPNode> terminals = new LinkedList<GPNode>();
...
//			attribute.Using_PSL = true;
	...
	//mengxu 2024.6.18  random pick terminals but give the preference terminal high probability to be selected
	public GPNode pickTerminalRandomWithPreferenceTerminalHighPro(int subPopNum) {
		int index = random[0].nextInt(terminals[subPopNum].length+2);
		if(index>=terminals[subPopNum].length){
			index=index-2;
		}
		return terminals[subPopNum][index];
	}
	...
	public void setup(EvolutionState state, Parameter base) {
		...
		expectTrainingTime = state.parameters.getDoubleWithDefault(new Parameter(P_EXPECT_TIME), null, 7200);
...
//		double totalTime = 0;
		...
		while ( result == R_NOTDONE )
        {
			...
			if(this instanceof GPRuleEvolutionStateDivNich && ((GPRuleEvolutionStateDivNich)this).printEnsemble){
			}
			else{
...
//			sumGenTimes.add(totalTime);
		...
		//2019.12.6 record the summed up so far
		File timeSumGenFile = new File("job." + jobSeed + ".timeSumGen.csv"); //jobSeed = 0
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(timeSumGenFile));
			writer.write("Gen,timeSumGen");
			writer.newLine();
			for (int gen = 0; gen < sumGenTimes.size(); gen++) {
				writer.write(gen + "," + sumGenTimes.get(gen));
				writer.newLine();
			}
			writer.close();
		} catch (IOException e) {
	...
	private void setupTerminals() {
        ...
        if (numSubPops == 1) {
        ...
        }else {//added by mengxu to make it suitable for more than two subpop 2022.10.11
			terminalsFrom = new String[numSubPops];
			includeErc = new boolean[numSubPops];
			int subPopNum = 0;
			...
			if (subPop1TerminalSet == null) {
				...
				subPop1TerminalSet = parameters.getStringWithDefault(p,
						null, "relative");
				output.warning("No terminal set for subpopulation " + subPopNum + " specified - using "+subPop1TerminalSet+".");

			}
			terminalsFrom[subPopNum] = subPop1TerminalSet;

			subPopNum++;
			while(subPopNum < numSubPops){
				p = new Parameter(P_TERMINALS_FROM + "." + subPopNum);
				String subPop2TerminalSet = parameters.getStringWithDefault(p,
						null, null);
				if (subPop2TerminalSet == null) {
					//use whatever we settled on for first population
					subPop2TerminalSet = subPop1TerminalSet;
					output.warning("No terminal set for subpopulation " + subPopNum + " specified - using terminal set for subpopulation 1.");
				}
				terminalsFrom[subPopNum] = subPop2TerminalSet;
				subPopNum++;
			}
