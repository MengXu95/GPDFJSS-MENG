...
public enum JobShopAttribute {
   ...
   /* LEAST_MACHINE_WORKLOAD_RATIO("LWR"),  //for one machine, the workload over the workInSystem,ratio
    ...
    //modified by mengxu 2021.08.31 transfer time related
//    UPLOAD_TIME("UPT"),
//    DOWNLOAD_TIME("DOWNT"),
    TRANSFER_TIME("TRANT"),

    Preference_One("Pone"), // added by mengxu 2024.3.11 for Pareto set learning

    Preference_Two("Ptwo"), // added by mengxu 2024.3.11 for Pareto set learning

    UTILI_LEVEL("UTIL"); // added by mengxu 2024.8.14 for multitask learning
...
//                break;
    ...
    //the following is added by mengxu 2024.3.11 for including new terminals for Pareto set learning
    public double value_Pareto_set_learning(OperationOption op, WorkCenter workCenter, SystemState systemState, EvolutionState state
    ) {
        double value = -1;
...
//                break;
    ...
    //the following is added by mengxu 2024.8.14 for including new terminals for multi-task learning
    public double value_multi_task_learning(OperationOption op, WorkCenter workCenter, SystemState systemState, EvolutionState state
    ) {
        double value = -1;
...
//                break;
            ...
            //meng xu 2024.8.14 for multi-task learning
            case UTILI_LEVEL:
                if(state instanceof GPRuleEvolutionStateMultiTaskLearningSurrogate){
                    value = ((GPRuleEvolutionStateMultiTaskLearningSurrogate)state).utilisationLevels[((GPRuleEvolutionStateMultiTaskLearningSurrogate)state).currentSubpop];
                }
                else if(state instanceof GPRuleEvolutionStateMultiTaskLearningOnlySurrogate){
                    value = ((GPRuleEvolutionStateMultiTaskLearningOnlySurrogate)state).utilisationLevels[((GPRuleEvolutionStateMultiTaskLearningOnlySurrogate)state).currentSubpop];
                }
                else if(state instanceof GPRuleEvolutionStateMultiTaskLearningSurrogateSVR){
                    value = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).utilisationLevels[((GPRuleEvolutionStateMultiTaskLearningSurrogateSVR)state).currentSubpop];
                }
                else if(state instanceof GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop){
                    value = ((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop)state).utilisationLevels[((GPRuleEvolutionStateMultiTaskLearningSurrogateSVROneSubpop)state).currentUtilLevelIndex];
                }
                break;
    ...
    //the following is added by mengxu 2024.4.24 for including new terminals for Pareto set learning
    public double value_Pareto_set_learning_normalisation(OperationOption op, WorkCenter workCenter, SystemState systemState, EvolutionState state
    ) {
        double value = -1;
        ...
        if(maxMachineWaitingTime == 0){
            maxMachineWaitingTime = 1;
        }
        if(maxProcTime == 0){
            maxProcTime = 1;
        }
        if(maxNextProcTime == 0){
            maxNextProcTime = 1;
        }
        if(maxOpWaitingTime == 0){
            maxOpWaitingTime = 1;
        }
        if(maxWorkRemaining == 0){
            maxWorkRemaining = 1;
        }
        if(maxNumOpsRemaining == 0){
            maxNumOpsRemaining = 1;
        }
        if(maxWeight == 0){
            maxWeight = 1;
        }
        if(maxTimeInSystem == 0){
            maxTimeInSystem = 1;
        }
        if(maxRelativeDueDate == 0){
            maxRelativeDueDate = 1;
        }
        if(maxSlack == 0){
            maxSlack = 1;
        }
        ...
        switch (this) {
            case NUM_OPS_IN_QUEUE:
                value = workCenter.getQueue().size()/maxNumOpsInQueue;
                break;
            case WORK_IN_QUEUE:
                value = workCenter.getWorkInQueue()/maxWorkInQueue;
                break;
            case MACHINE_WAITING_TIME:
                value = (systemState.getClockTime() - workCenter.getReadyTime())/maxMachineWaitingTime;
                break;
            case PROC_TIME:
                value = op.getProcTime()/maxProcTime;
                break;
            case NEXT_PROC_TIME:
                value = op.getNextProcTime()/maxNextProcTime;
                break;
            case OP_WAITING_TIME:
                value = (systemState.getClockTime() - op.getReadyTime())/maxOpWaitingTime;
                break;
            case WORK_REMAINING:
                value = op.getWorkRemaining()/maxWorkRemaining;
                break;
            case NUM_OPS_REMAINING:
                value = op.getNumOpsRemaining()/maxNumOpsRemaining;
                break;
            case RELATIVE_DUE_DATE:
                value = (op.getJob().getDueDate() - systemState.getClockTime())/maxRelativeDueDate;
                break;
            case WEIGHT:
                value = op.getJob().getWeight()/maxWeight;
                break;
            case TIME_IN_SYSTEM:
                value = (systemState.getClockTime() - op.getJob().getReleaseTime())/maxTimeInSystem;
                break;
            case SLACK:
                value = (op.getJob().getDueDate() - systemState.getClockTime() - op.getWorkRemaining())/maxSlack;
                break;
            case TRANSFER_TIME:
                int opID = op.getOperation().getId();
                if(opID==0){
                    value = ((HeterogeneousSimulation)systemState.getSimulation()).getEntryWorkCenterTranserTime(workCenter.getId())/maxTransferTime;
                    break;
                }
                else if(opID==op.getJob().getOperations().size()-1){
                    int fromWorkCenterID = op.getJob().getOperation(opID-1).getSelectedWorkCenter().getId();
                    double transferTime = ((HeterogeneousSimulation)systemState.getSimulation()).getWorkCenterTranserTime(fromWorkCenterID, workCenter.getId());
                    double downloadTime = ((HeterogeneousSimulation)systemState.getSimulation()).getWorkCenterExitTranserTime(workCenter.getId());
                    value = (transferTime + downloadTime)/maxTransferTime;
                    break;
                }
                else{
                    int fromWorkCenterID = op.getJob().getOperation(opID-1).getSelectedWorkCenter().getId();
                    value = ((HeterogeneousSimulation)systemState.getSimulation()).getWorkCenterTranserTime(fromWorkCenterID, workCenter.getId())/maxTransferTime;
                    break;
                }
            case Preference_One:
    ...
    //the following is added by mengxu 2024.4.24 for including new terminals for Pareto set learning
    public double value_Pareto_set_learning_sigmoid_normalisation(OperationOption op, WorkCenter workCenter, SystemState systemState, EvolutionState state
    ) {
        double value = -1;
...
//                break;
        ...
        //sigmoid function
        value = 1/(1+Math.exp(-value)); //add by mengxu 2024.5.15
    ...
    public static JobShopAttribute[] relativeAttributes() {
        return new JobShopAttribute[]{
                ...
                //modified by menxu 2023.01.12 for tardiness
                JobShopAttribute.RELATIVE_DUE_DATE,
                JobShopAttribute.SLACK,

                //modified by mengxu 2021.08.31 for heterogeneous
                JobShopAttribute.TRANSFER_TIME,//todo: only for Heterogeneous similation
        };
    }

    //add by mengxu for Pareto set learning 2024.3.11
    public static JobShopAttribute[] relativeWithPreferenceAttributes() {
        return new JobShopAttribute[]{
                JobShopAttribute.NUM_OPS_IN_QUEUE,
                JobShopAttribute.WORK_IN_QUEUE,
                JobShopAttribute.MACHINE_WAITING_TIME,
                ...
                //modified by mengxu 2023.01.12 for tardiness
                JobShopAttribute.RELATIVE_DUE_DATE,
                JobShopAttribute.SLACK,

                //modified by mengxu 2021.08.31 for heterogeneous
                JobShopAttribute.TRANSFER_TIME,
                //by mengxu 2024.3.11 for Pareto set learning
                JobShopAttribute.Preference_One, //todo: only for Pareto set learning
                JobShopAttribute.Preference_Two,
//                JobShopAttribute.Preference_One, //todo: only for Pareto set learning
//                JobShopAttribute.Preference_Two,
//                JobShopAttribute.Preference_One, //todo: only for Pareto set learning
//                JobShopAttribute.Preference_Two,
    ...
    //add by mengxu for Pareto set learning 2024.3.11
    public static JobShopAttribute[] relativeForMultiTaskAttributes() {
        return new JobShopAttribute[]{
                JobShopAttribute.NUM_OPS_IN_QUEUE,
                JobShopAttribute.WORK_IN_QUEUE,
                JobShopAttribute.MACHINE_WAITING_TIME,
                ...
                //by mengxu 2024.8.14 for Multi-task
                JobShopAttribute.UTILI_LEVEL, //todo: only for multitask learning
//                JobShopAttribute.UTILI_LEVEL, //give it more probability to be select
//                JobShopAttribute.UTILI_LEVEL, //give it more probability to be select
        };
    }
