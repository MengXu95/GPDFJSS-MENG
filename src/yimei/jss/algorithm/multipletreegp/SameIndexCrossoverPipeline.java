...
/*            if (sources[0] == sources[1])  // grab from the same source
            ...
            //fzhang 2019.11.17 grab different individuals from different subpopulations
            //get the probability that the two subpops will communicate with each other.
            rnd = state.random[thread].nextDouble(); //probability  (0,1)
            transferprob = state.parameters.getDoubleWithDefault(new Parameter(P_TRANSFER_PROB), null, 1);
            if (rnd <= transferprob) {
                //================choose parents from different subpopulations.
                if(state.population.subpops.length == 2){
                    if (subpopulation == 0) {
                        sources[0].produce(1, 1, 0, subpopulation, parents, state, thread);
                        sources[1].produce(1, 1, 1, subpopulation + 1, parents, state, thread);
                    } else {
                        sources[0].produce(1, 1, 0, subpopulation, parents, state, thread);
                        sources[1].produce(1, 1, 1, subpopulation - 1, parents, state, thread);
                    }
                    ...
                    sources[0].produce(1, 1, 0, subpopulation, parents, state, thread);
                    List<Integer> subpopList = new ArrayList<>();
                    for(int sub = 0; sub < state.population.subpops.length; sub++){
                        if(sub != subpopulation){
                            subpopList.add(sub);
                        }
                    }
                    int chooseFromSubPop = subpopList.get(state.random[thread].nextInt(subpopList.size()));
                    sources[1].produce(1, 1, 1, chooseFromSubPop, parents, state, thread);
                }

            } else {
                //original way, crossover independent for different subpopulations
                if (sources[0] == sources[1])  // grab from the same source
                    sources[0].produce(2, 2, 0, subpopulation, parents, state, thread);
                else // grab from different sources
                {
