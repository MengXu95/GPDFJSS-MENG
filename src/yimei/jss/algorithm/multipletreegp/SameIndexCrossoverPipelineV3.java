 ...
 * 2020.11.29 choose the individuals hat has high
 */
public class SameIndexCrossoverPipelineV3 extends CrossoverPipeline {
    /**
    ...
    public final static String P_TRANSFER_PROB = "transfer-prob";
    public static double rnd;
    public static double transferprob;
    @Override
...

    {
        ...
        for (int q = start; q < n + start; /* no increment */)  // keep on going until we're filled up
        {
            ...
            //fzhang 2020.11.23 grab different individuals from different groups if need to transfer knowledge
            //get the probability that the two subpops will communicate with each other.
            //======================start=================================
/*            rnd = state.random[0].nextDouble(); //probability  (0,1)
            transferprob = state.parameters.getDoubleWithDefault(new Parameter(P_TRANSFER_PROB), null, 1);
//            transferprob = state.parameters.getDoubleWithDefault(new Parameter(P_TRANSFER_PROB), null, 1)*GPRuleEvolutionStateV5.archiveInds.get(0).size()
//            /state.population.subpops[0].individuals.length;
            if (rnd <= transferprob) { //do not do this at generation 0, because the pool at generation 0 is the same as the original population
                //transfer
                sources[0].produce(1, 1, 0, subpopulation, parents, state, thread); //choose parent by tournament selection
                //2020.11.29 select the individual has (OneDimArchiveIndsGens.get(i).get(i) + 1) / (selectedInsRank.get(i) + 1)

                //delete one individual and insert a new individual
                //ArrayList<Double> OneDimArchiveIndsAdddBase = new ArrayList<>();//calculate the data to decide which inds to delete
                //for (int i = 0; i < GPRuleEvolutionStateV6.archiveInds.get(0).size(); i++) {
                ArrayList<Double> OneDimArchiveIndsAdddBase = GPRuleEvolutionStateV6.OneDimArchiveIndsAddBase;//rank/generation
                //}
                double[] addIndIndex = OneDimArchiveIndsAdddBase.stream().mapToDouble(Double::doubleValue).toArray();
                double[] usedAddIndIndex = addIndIndex.clone();
                int selectInd = 0;

                int numZero = 0;
                for (int c = 0; c < usedAddIndIndex.length; c++) {
                    if (usedAddIndIndex[c] == 0) {
                        numZero++;
                    }
                }
                //when all zeros, or all the elements are the same. Just use the original way.
                if (numZero == usedAddIndIndex.length || Arrays.stream(usedAddIndIndex).distinct().count() == 1) {
                    //random choose one
                    selectInd = state.random[0].nextInt(usedAddIndIndex.length);// random choose one
                } else {
                    RandomChoice.organizeDistribution(usedAddIndIndex);
                    selectInd = RandomChoice.pickFromDistribution(usedAddIndIndex, state.random[0].nextDouble());
                }

                //int random  = state.random[0].nextInt(GPRuleEvolutionStateV6.archiveInds.get(0).size());
                parents[1] = (GPIndividual) GPRuleEvolutionStateV6.archiveInds.get(0).get(selectInd);
            }
            else{
