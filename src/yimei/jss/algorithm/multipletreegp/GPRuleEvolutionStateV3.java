 ...
 *
 */
public class GPRuleEvolutionStateV3 extends GPRuleEvolutionState {
	...
	double[] indexInds;
	ArrayList<Double> selectedInsRank = new ArrayList<>();
	@Override
...
/*				for (int ind = 0; ind < this.population.subpops[sub].individuals.length; ind++) {
				...
				if (generation == 0) {
					for (int numUpdatedInds = 0; numUpdatedInds < archiveUpdateRate * this.population.subpops[sub].individuals.length; numUpdatedInds++) {
						...
						int chosenIndIndex = 0;
						//2020.11.24 when the elements in the array are all 0, then there are problems.
						int countZero= 0;
						for(int c = 0; c < usedIndexInds.length; c++){
							if(usedIndexInds[c] == 0){
								countZero++;
							}
						}
						//when all zeros, or all the elements are the same. Just use the original way.
						if(countZero == usedIndexInds.length || Arrays.stream(usedIndexInds).distinct().count() == 1){
							//random choose one
							chosenIndIndex = state.random[0].nextInt(usedIndexInds.length);// random choose one
						}
						else{
							RandomChoice.organizeDistribution(usedIndexInds);
							chosenIndIndex = RandomChoice.pickFromDistribution(usedIndexInds, this.random[0].nextDouble());
						}

						selectedInsRank.add(Double.valueOf(chosenIndIndex));
					...
					for (int numUpdatedInds = 0; numUpdatedInds < archiveUpdateRate * this.population.subpops[sub].individuals.length; numUpdatedInds++) {
						...
						if (OneDimArchiveInds.size() < archiveSize) {
							...
							//2020.11.24 remove the low ranked inds
							double[] deleteIndIndex = selectedInsRank.stream().mapToDouble(Double::doubleValue).toArray();
