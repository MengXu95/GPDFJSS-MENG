 ...
 *
 */
public class GPRuleEvolutionStateV9 extends GPRuleEvolutionState {
    ...
    public static final String P_DECIMATION_RATE = "decimation-rate"; //modified by mengxu. 2021.03.31
    private double decimationRate; //modified by mengxu. 2021.03.31
    ...
    ArrayList<ArrayList<Integer>> storeGenArcSizeRepeatFromArcRankAll = new ArrayList<>(); //modified by MengXu
    ...
    public void setup(EvolutionState state, Parameter base) {
        ...
        decimationRate = state.parameters.getDouble(new Parameter(P_DECIMATION_RATE), null);//modified by mengxu 2021.03.31
...
/*		indexInds = new double[archiveSize];
    ...
    public int evolve() {
        ArrayList<Integer> storeGenArcSizeRepeatFromArcRank = new ArrayList<>();
        if (generation != 0){
            storeGenArcSizeRepeatFromArcRank.add(generation);
            storeGenArcSizeRepeatFromArcRank.add(archiveInds.get(0).size());
        }
        ...
        //todo:
        //modified by mengxu 2021.03.26 decimation: random delet 0.1% individuals before evaluation and recreate them.
        //not finish.
        if(generation != 0){
            //double decimation_rate = 0.1;
            int deciNum = 0;
            while (deciNum<this.population.subpops[0].individuals.length * decimationRate){
                int index = this.random[this.breedthreads-1].nextInt(this.population.subpops[0].individuals.length);
                this.population.subpops[0].individuals[index] = this.population.subpops[0].individuals[index].species.newIndividual(this,this.breedthreads-1);
                deciNum++;
            }
        }
        ...
        for (int sub = 0; sub < this.population.subpops.length; sub++) {
            ...
            if (generation != 0) {
                storeGenArcSizeRepeatFromArcRank.add(repeatNum);
        ...
        if(generation != 0){
            ...
            int numTopFiveRank = 0;
            for(int ind = 0; ind < this.population.subpops[0].individuals.length; ind++){
                this.population.subpops[0].individuals[ind] = indsPopPool[ind];
                numFromArchive += (int)indsPopPoolMap.get(indsPopPool[ind]);
                if((int)indsPopPoolMap.get(indsPopPool[ind])==1 && ind<5){
                    numTopFiveRank ++;
                }
            }
            storeGenArcSizeRepeatFromArcRank.add(numFromArchive);
            storeGenArcSizeRepeatFromArcRank.add(numTopFiveRank);
//            output.message("The number of individuals from the archive: " + numFromArchive + "/" + archiveInds.get(0).size());
        }
        if(generation != 0){
            storeGenArcSizeRepeatFromArcRankAll.add(storeGenArcSizeRepeatFromArcRank);
        }
...
/*		OneDimArchiveIndsAddBase = new ArrayList<>();
        ...
        if (generation == numGenerations-1)
        {
            ...
            writeGenRankToFile();
            writeArcNumToFile();//modified by mengxu
            return R_FAILURE;
        }
    ...
    public void writeArcNumToFile(){
        File indFromArcFile = new File("job." + jobSeed + ".indsFromArc.csv"); //successedTransfer[i][j]: task j makes a successful transfer for task i.
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(indFromArcFile));
            writer.write("Gen,ArcSize,NumRepeat,FromArc,Rank");
            writer.newLine();
            for (int ind = 0; ind < storeGenArcSizeRepeatFromArcRankAll.size(); ind++) {
                ArrayList<Integer> ref = storeGenArcSizeRepeatFromArcRankAll.get(ind);
                writer.write(ref.get(0) + "," + ref.get(1) + "," + ref.get(2) + "," + ref.get(3) + "," + ref.get(4));
                writer.newLine();
            }
...
//        phenoCharacterisation = new PhenoCharacterisation[2];
        double[][] poolIndPC = phenotypicForSurrogateV1.phenotypicIndividual(this, phenoCharacterisation, archiveInds.get(0));
