...
public class OperationOption implements Comparable<OperationOption> {
    ...
    public OperationOption getNextByEnsembleVotingRecordSpread(SystemState systemState, EnsembleRule ensembleRule,
                                                               double[] ensembleContribution, double[] ensembleSpread) { //2023.02.24
        Operation nextOperation = operation.getNext();
        if (nextOperation != null) {
            return nextOperation.chooseOperationOptionByEnsembleVotingRecordSpread(systemState, ensembleRule, ensembleContribution, ensembleSpread);
        } return null;
    }

    public OperationOption getNextByEnsembleVoting(SystemState systemState, EnsembleRule ensembleRule, double[] ensembleContribution) { //2023.02.24
        Operation nextOperation = operation.getNext();
        if (nextOperation != null) {
            return nextOperation.chooseOperationOptionByEnsembleVoting(systemState, ensembleRule, ensembleContribution);
        } return null;
    }

    public OperationOption getNextByEnsembleMeanRank(SystemState systemState, EnsembleRule ensembleRule, double[] ensembleContribution) { //2023.02.24
        Operation nextOperation = operation.getNext();
        if (nextOperation != null) {
            return nextOperation.chooseOperationOptionByEnsembleMeanRank(systemState, ensembleRule, ensembleContribution);
        } return null;
    }

    public OperationOption getNextByEnsembleLinear(SystemState systemState, EnsembleRule ensembleRule, double[] ensembleContribution) { //2023.02.24
        Operation nextOperation = operation.getNext();
        if (nextOperation != null) {
            return nextOperation.chooseOperationOptionByEnsembleLinear(systemState, ensembleRule, ensembleContribution);
        } return null;
    }
    ...
    //modified by mengxu for HEFT 2021.07.27
    //For HEFT algorithm
    public double getEarliestExecutionStartTime(){
        if(operation.getId() == 0){
            return Math.max(readyTime, workCenter.getReadyTime());
...
/*    public boolean priorTo(OperationOption other) {
    ...
    public OperationOption clone() {
        ...
        OperationOption clonedOp = new OperationOption(operation, optionId, procTime, clonedWC);
        clonedOp.setReadyTime(readyTime);
        clonedOp.setWorkRemaining(workRemaining);
        clonedOp.setNumOpsRemaining(numOpsRemaining);
        clonedOp.setFlowDueDate(flowDueDate);
        clonedOp.setNextProcTime(nextProcTime);
        clonedOp.setPriority(priority);
