...
public class SimpleRuleTest{
	 ...
	 public static final long simSeed = 968356;
	    ...
	    public SimpleRuleTest(String trainPath, RuleTypeV2 ruleType, int numRuns,
		...
		public void writeToCSV() {
	        ...
	        //for test: which machines are choosen by routing rule, CCGP. Scenario: run1, rule in generaiton 51
	        for (int i = 0; i < numRuns; i++) {
	        	System.out.println("Run "+ i);
	        //for (int i = 0; i < 1; i++) {
	            File sourceFile = new File(trainPath + "job." + i + ".out.stat");  //this file keeps the rule
	            TestResult result = SimpleTestResult.readFromFile(sourceFile, ruleType);
...
//	                                numUniqueTerminals + ",");
		...
		public static void main(String[] args) {
			...
			SimpleRuleTest simpleruleTest = new SimpleRuleTest(trainPath, ruleType, numRuns, testScenario, testSetName);

			for (int i = 0; i < numObjectives; i++) {
				simpleruleTest.addObjective(args[idx]);
				idx ++;
			}

			simpleruleTest.writeToCSV();
		}
}
