 ...
 */
public class FirstOperationUploadStartEvent extends AbstractEvent{
    private FirstOperationUploadProcess process;

    public FirstOperationUploadStartEvent(double time, FirstOperationUploadProcess process) {
        super(time);
        this.process = process;
    }

    public FirstOperationUploadStartEvent(FirstOperationUploadProcess process) {
        this(process.getStartTime(), process);
    }

    @Override
    public void trigger(Simulation simulation) {
//        process.getOperationOption().setReadyTime(time);
        simulation.addEvent(new FirstOperationUploadFinishEvent(process));
    }
    ...
    public String toString() {
        return String.format("%.1f: job %d op 0 upload to workCenter %d transfer time %f.\n",
                time, process.getOperationOption().getJob().getId(), process.getToWorkCenter().getId(), process.getDuration());
    }
