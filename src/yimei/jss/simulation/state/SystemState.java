...
public class SystemState {
    ...
    public List<Job> jobsCompleted; //modified by mengxu 2021.05.27
    ...
    //modified by mengxu 2021.09.01 for heterogeneous
    public SystemState(double clockTime, List<WorkCenter> workCenters,
                       List<Job> jobsInSystem, List<Job> jobsCompleted,
...
//    //modified by mengxu for LS
//    public SystemState(double clockTime, int numJobs) {
//        this(clockTime, new ArrayList<>(), new LinkedList<>(), new ArrayList<>(numJobs));
//    }
//    public SystemState(boolean useLS, int numJobs) {
//        this(0.0, numJobs);
//    }
...
//        }
    ...
    //mengxu 2020.06.22  calculate total remaining processing time
    public double getTotalRemainProcessTime() {
        double totalRemainProcessTime = 0;
        for(int i = 0; i< workCenters.size(); i++) {
            totalRemainProcessTime += workCenters.get(i).getRemainProcessTime();
        }
        return totalRemainProcessTime;
    }
    ...
    //modified by mengxu 2021.06.22
    public double getNumOfOperationInWholeSystem() {
        //modified by mengxu
        double totalNumOfOperationInSystem =0;
        for(int i = 0; i< jobsInSystem.size(); i++) {
            totalNumOfOperationInSystem += jobsInSystem.get(i).getOperations().size()-jobsInSystem.get(i).getProcessFinishEvents().size();
        }
        return totalNumOfOperationInSystem;
    }


    public double getNumOfOperationWaitingToBeDispatch() {
        return getNumOfOperationInWholeSystem() - getNumOfOperationInSystem();
    }
...
//        jobsCompleted.clear();//original
        if(simulation.useLS){
            jobsCompleted = Arrays.asList(new Job[simulation.getNumJobsRecorded()]);//add by mengxu for LS 2021.05.27
        }
        else{
            jobsCompleted.clear();
        }
...
//    }
    ...
    public SystemState clone() {
        ...
        //modified by mengxu 2021.09.01 for heterogeneous
        //rules do not maintain state //todo: 2022.03.22
        return new SystemState(clockTime, clonedWCs,
                new LinkedList<>(), new ArrayList<>(), simulation);
