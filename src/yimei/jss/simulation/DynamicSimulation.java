...
public class DynamicSimulation extends Simulation {
    ...
    //modified by mengxu 2021.07.27 make them protected for extends
    protected long seed;
    protected RandomDataGenerator randomDataGenerator;

    protected final int minNumOperations;
    protected final int maxNumOperations;
//    protected final double utilLevel; //original
    protected double utilLevel; //modified by mengxu 2024.12.1
    protected final double dueDateFactor;
    protected final boolean revisit;

    protected AbstractIntegerSampler numOperationsSampler;
...
//    private AbstractRealSampler jobWeightSampler;
    ...
    public double getInterArrivalTimeSamplerMean(){
        return this.interArrivalTimeSampler.getMean();
    }
    ...
    //modified by mengxu for heterogeneous 2021.07.27
    public DynamicSimulation(long seed,
                              AbstractRule sequencingRule,
                              AbstractRule routingRule,
                              int numWorkCenters,
...
//        setInterArrivalTimeSamplerMean();
    ...
    public void resetState() {
        ...
        x_Axis.clear();
        y_Axis_List.clear();
        y_WorkCenter_Axis_List.clear();
        sequencingDecisions.clear();
        routingDecisions.clear();
        AllDecisions.clear();
        this.totalDecisionNumber = 0;
        this.throughput = 0;
        this.beforeThroughput = 0;
        this.afterThroughput = 0;
        this.count = 0;
//        this.ensembleRule = null;
//        operationNumAtDecisionPoints.clear();
        setup();
    }
...
//    }
    ...
    public void setUtilLevel(double utilLevel){
        this.utilLevel = utilLevel;
        this.setInterArrivalTimeSamplerMean();
    }
...
//                 System.out.println("]");
    ...
    /**
     * add by mengxu 2024.4.19 for get sequencing decision points for all cases
     * @param minQueueLength
     * @param numCases
     * @return
     */
    public List<List<SequencingDecisionSituation>> sequencingDecisionSituationsAllCases(int minQueueLength, int numCases) {
        List<List<SequencingDecisionSituation>> sequencingDecisionSituations = new ArrayList<>();
        for(int i=0; i<numCases; i++){
            List<SequencingDecisionSituation> sequencingDecisionSituationsEachCase = new ArrayList<>();
            sequencingDecisionSituations.add(sequencingDecisionSituationsEachCase);
        }
        int numJobsEachCase = (this.numJobsRecorded-1000)/numCases;
        ...
        while (!eventQueue.isEmpty() && throughput < numJobsRecorded) {
            ...
            //version 1 by mengxu
//            int index = (int)Math.floor(throughput/numJobsEachCase);
//            if(index >= 25){
//                break;
//            }
//            nextEvent.addSequencingDecisionSituation(this, sequencingDecisionSituations.get(index), minQueueLength);

            //version 2 by mengxu
            nextEvent.addSequencingDecisionSituationAllCases(this, sequencingDecisionSituations, minQueueLength,numJobsEachCase);
...
//                 System.out.println("]");
    ...
    /**
     * add by mengxu 2024.4.19 for get routing decision points for all cases
     * @param minQueueLength
     * @param numCases
     * @return
     */

    public List<List<RoutingDecisionSituation>> routingDecisionSituationsAllCases(int minQueueLength, int numCases) {
        List<List<RoutingDecisionSituation>> routingDecisionSituations = new ArrayList<>();
        for(int i=0; i<numCases; i++){
            List<RoutingDecisionSituation> routingDecisionSituationsEachCase = new ArrayList<>();
            routingDecisionSituations.add(routingDecisionSituationsEachCase);
        }
        int numJobsEachCase = (this.numJobsRecorded-1000)/numCases;
...
//                break;
//            }
//            nextEvent.addRoutingDecisionSituation(this, routingDecisionSituations.get(index), minQueueLength);

            //version 2 by mengxu
            nextEvent.addRoutingDecisionSituationAllCases(this, routingDecisionSituations, minQueueLength, numJobsEachCase);
